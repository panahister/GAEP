...
| Last stable checkpoint | `1ea0141` — typed path-free Codex/Claude readiness observation in Kiro, Rider and Visual Studio, hostile private-field rejection, refreshed exact package evidence and a 38/7/15 incomplete conformance matrix |
| Active work | Safe provider/model selection-contract preparation: add an exact path-free persisted-selection read and governed switch boundary before exposing selection in Kiro, Rider or Visual Studio |
...
| Next exact action | Add a typed engine-host selection observation/switch contract that cannot bypass active-run checks or required versioned handoff, with hostile stale-capability and path/privacy tests; only then wire cancel-default provider/model selection into Kiro, Rider and Visual Studio |
...
7. continuing with a typed engine-host persisted-selection read and governed switch contract that cannot bypass active-run or versioned-handoff requirements, before exposing cancel-default selection in Kiro, Rider or Visual Studio.
