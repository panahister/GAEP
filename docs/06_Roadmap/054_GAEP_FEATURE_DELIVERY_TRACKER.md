...
|---|---|---|---|---|---|
| PLT-01 | Shared GAEP Engine | Phase 0 / 1A | One shared engine with no behavioral forks between hosts | `🟡 In Progress` | `d6a16c6` adds Rider handoff and strengthens exact Kiro handoff receipts through the shared engine-host contract, on top of `21d2202`'s all-non-terminal Run guard; Visual Studio handoff, packaged-engine workflow evidence and complete behavioral parity remain incomplete |
...
| PLT-04 | Visual Studio Native VSIX | Phase 0 / 1A | Native Visual Studio extension installable on Windows | `🟡 In Progress` | `c0dabe9` adds workspace-bound guarded adapter/model/non-sensitive setting editors to the compiled out-of-process Remote UI and raises the private-safe protocol/controller harness to 63 checks; Windows container creation, installation and rendered interaction remain |
| PLT-05 | Four-IDE Behavioral Parity | Phase 0 / 1A | Equivalent behavior in VS Code, Visual Studio, Rider, and Kiro | `🟡 In Progress` | `4b1a14f` expands the honest executable 60-cell matrix to 41 implemented, 7 partial and 12 not implemented by adding guarded provider/model selection in Kiro, Rider and Visual Studio; the phase gate remains incomplete with zero accepted hosts |
...
| PLT-14 | Provider Switching | Phase 0 / 1A | Switch Codex and Claude Code within one Initiative without losing state | `🟡 In Progress` | `d6a16c6` adds a Rider versioned switch alongside Kiro, binding current selection and latest terminal Run to a freshly observed target with preserved portable history and exact returned receipt validation. Visual Studio handoff and a real bidirectional accepted workflow remain incomplete |
| PLT-15 | Model Switching | Phase 0 / 1A | Change models within each provider while recording provenance | `🟡 In Progress` | `d6a16c6` records Kiro/Rider model and setting switches through atomic versioned handoff plus target selection, including exact agent/model/settings/history receipt checks. Visual Studio provenance, packaged-engine interaction and four-host acceptance remain incomplete |
| PLT-16 | Versioned Provider/Model Handoff | Phase 0 / 1A | Transfer context, Run Envelope, and evidence between agents and models | `🟡 In Progress` | `d6a16c6` adds strict Rider source-Run/prior-selection binding, bounded portable completed/unresolved/decision/evidence details, stale-state recheck, fresh target observation and native asynchronous cancel-default confirmation; Kiro validates the same exact receipt. Visual Studio UI, live bidirectional provider acceptance and packaged-engine evidence remain incomplete |
...
| PLT-24 | Agent and Model Dashboard | Phase 0 / 1A | Provider, model, run status, cost/usage, and handoff evidence | `🟡 In Progress` | `d6a16c6` adds Rider and strengthens Kiro handoff receipt projections with source Run, target, workspace observation, preserved-entry counts and capability differences. Full Run status, usage, Visual Studio handoff view and native acceptance remain incomplete |
...
| PLT-27 | Four-IDE Conformance Suite | Phase 0 / 1A | Shared contract tests for four IDEs and two providers | `🟡 In Progress` | `4b1a14f` verifies 41 implemented, 7 partial and 12 not-implemented cells across 15 capabilities and four hosts against source markers, exact package bytes/digests and bounded runtime evidence. Handoff/execution workflows, native matrices and acceptance remain incomplete |
...
| PLT-29 | Per-Phase Package and Acceptance Report | Phase 0 / 1A | Package, checksum, test report, and known gaps for every phase | `🟡 In Progress` | `4b1a14f` records refreshed exact Kiro/Rider/VS Code package hashes in `evidence/local-packages/2026-07-24T102603Z-phase-0-local-selection.json` and the 41/7/12 matrix in `evidence/ide-conformance/2026-07-24T102603Z-phase-0-local-selection.json`, preserving the missing Windows VSIX, native/supported-platform, human-acceptance and non-release gaps. Later phase reports remain |
