...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Cross-platform capabilities | 35 | 4 | 27 | 0 | 0 | 0 | 0 | 4 |
...
| **All tracked features** | **168** | **5** | **86** | **0** | **0** | **0** | **0** | **77** |
...
|---|---|---|---|---|---|
| PLT-01 | Shared GAEP Engine | Phase 0 / 1A | One shared engine with no behavioral forks between hosts | `🟡 In Progress` | Core and shared packages exist; GAEP-P0-CS01 added a shared `computePlatformReadiness()` in `packages/engine` plus an engine-host v2 `platformReadiness` endpoint; the four-host conformance boundary and execution relocation remain incomplete |
...
| PLT-04 | Visual Studio Native VSIX | Phase 0 / 1A | Native Visual Studio extension installable on Windows | `🟡 In Progress` | GAEP-P0-CS02 added the out-of-process VisualStudio.Extensibility project + Windows build lane; not built here (no Windows/dotnet) — host row `not-built`/`pending-environment` |
...
| PLT-11 | Safe Claude Analysis Execution | Phase 0 / 1A | Read-only Claude analysis with a run envelope and evidence | `🟡 In Progress` | GAEP-P0-CS02 wired read-only Claude analysis through the shared `ReadOnlyAnalysisService` (engine-host v3) with unit-tested lifecycle; one-host execution only, no full acceptance |
...
| PLT-21 | Dashboard Shell | Phase 0 / 1A | Shared dashboard shell in all four hosts | `🟡 In Progress` | A basic dashboard exists; GAEP-P0-CS01 added a read-only `gaep.showPlatformReadiness` VS Code surface fed by the shared readiness snapshot; four-host parity/installation remain incomplete |
...
| PLT-24 | Agent and Model Dashboard | Phase 0 / 1A | Provider, model, run status, cost/usage, and handoff evidence | `🟡 In Progress` | The foundation is partial; GAEP-P0-CS01 added host-neutral provider/model readiness fields (detected, models, truth class) to the shared snapshot; execution truth and four-host coverage remain incomplete |
...
| PLT-26 | Freshness and Evidence Cues | Phase 0 / 1A | Show freshness, source, confidence, and stale state | `🟡 In Progress` | Cues are partial; GAEP-P0-CS01 carries `observedAt`, `truthClass`, and `evidenceSource` end-to-end in the readiness snapshot/report so unknown host state stays visibly unknown; policy and validation remain incomplete |
| PLT-27 | Four-IDE Conformance Suite | Phase 0 / 1A | Shared contract tests for four IDEs and two providers | `🟡 In Progress` | GAEP-P0-CS01 added `packages/conformance` with an engine-host RPC boundary check, `composePlatformReadinessReport` merge, and `observeVsCodeHost`, plus tests; the boundary result is kept separate from the Four-IDE matrix. Full four-IDE execution (VS/Rider/Kiro) is not yet implemented |
| PLT-28 | Realistic Example Runner | Phase 0 / 1A | Repeatable realistic example with inspectable outputs | `✅ Done` | Product Owner accepted GAEP-P0-CS01-C1 on 2026-07-24 after installing the VSIX and manually executing `GAEP: Show Platform Readiness`. The command rendered the computed provider, workspace, and exact Four-IDE readiness matrix; automated evidence remains at `examples/phase0-readiness/acceptance/GAEP-P0-CS01_READINESS_REPORT.json` and `examples/phase0-readiness/acceptance/GAEP-P0-CS01_EVIDENCE_MANIFEST.md` (verified report digest `sha256:158853a9e822ccd9b86dafe5f691aa0ae42b4a4f302e01c55611515ecf5d5b3a`). |
| PLT-29 | Per-Phase Package and Acceptance Report | Phase 0 / 1A | Package, checksum, test report, and known gaps for every phase | `🟡 In Progress` | GAEP-P0-CS01 added an Evidence Manifest generator (SHA-256 digest of the normalized report, environment capture, known gaps); a full per-phase package/checksum delivery pipeline is not yet implemented |
...
| PLT-32 | Kiro Installable Package | Phase 0 / 1A | Independent Kiro installation and compatibility validation | `🟡 In Progress` | GAEP-P0-CS02 produces an independent `gaep-kiro-0.2.0.vsix` (identity `gaep.gaep-kiro`); install/workflow require a Kiro install — `pending-environment` |
| PLT-33 | Kiro Compatibility and Smoke Tests | Phase 0 / 1A | Installation, workflow, provider, model, and dashboard smoke tests in Kiro | `🟡 In Progress` | GAEP-P0-CS02 added Kiro install/smoke documentation and package; not executed in Kiro — `pending-environment` |
| PLT-34 | Visual Studio Windows Installation Tests | Phase 0 / 1A | CI or manual evidence for VSIX installation on Windows | `🟡 In Progress` | GAEP-P0-CS02 added `.github/workflows/cs02-visual-studio.yml` (build lane); not executed — `pending-environment` |
| PLT-35 | Rider Sandbox and Installation Tests | Phase 0 / 1A | `runIde`, installable artifact, and workflow smoke testing | `🟡 In Progress` | GAEP-P0-CS02 added `.github/workflows/cs02-rider.yml` (buildPlugin/verifyPlugin); not executed — `pending-environment` |
...
| P3B-23 | Performance and Reliability Testing | QA | Budgets and load, failure, and recovery scenarios | `❌ Backlog` | No methodology or harness exists |
...
| 2026-07-26 | Claude Code | Phase 0 / 1A | PLT-04, PLT-11, PLT-32, PLT-33, PLT-34, PLT-35 (defect-correction pass; no status change) | None — statuses unchanged; CS02 stays `In Implementation`, PLT-03 stays `🟡 In Progress` | GAEP-P0-CS02 defect-correction pass: `npm run typecheck` (clean), `npm test` (61 files, 472 passed/1 skipped), `npm ci --dry-run --ignore-scripts` (exit 0), clean `npm ci` in an isolated full-tree copy (exit 0, builds + typechecks), `npm run test:lifecycle` (PASS), `npm run validate:docs` (PASS), `npm run build` (OK), `npm run release:cs02 -- --verify` (PASS; VS Code + Kiro built, VS + Rider `not-built`), `npm run verify:cs02:packages` (PASS), `npm run test:vscode:extension-host` (PASS), `npm run example:cs02 -- --verify` (**BLOCKED (auth-unavailable)** — exit 0, truthful environment blocker, NOT a pass), `git diff --check` (clean). | Not accepted — awaiting Product Owner test and acceptance | **Correction of the 2026-07-24 GAEP-P0-CS02 row.** The original example was NOT a real bounded analysis: it bypassed protocol v3 (constructed `GaepEngine` and called `readOnlyAnalysis.start()` directly with fabricated context) and finished `provider-error`. Corrected in this pass: (1) real Codex read-only dispatch through `GaepEngine.runReadOnlyProvider` with staging cleanup on every path + integration test (faked supervisor boundary only); (2) full-workspace mutation guard now also runs after cancel/timeout, before the terminal record is finalized; (3) atomic Engine Host lock with an ownership nonce (exclusive-create + rename takeover; non-owner cannot release) and a persistent VS Code client per Product root; (4) the example now drives the entire slice over protocol v3 (real Product + real governed Context Pack → providerCatalog → selectProviderModel → startReadOnlyAnalysis → poll readAnalysisRun), verifies persisted selection/run/pre-run+result evidence and no Product/`.gaep` mutation, and its `--verify` exits non-zero on `provider-error`/`internal`/`protocol-error`/`timeout`/`source-mutation`; the Claude `provider-error` was diagnosed as a misclassified authentication failure now classified `auth-unavailable`; (5) real official Node 22.11.0 archive/executable digests; (6) Rider/VS Tool Window source. **Final-defects pass (also 2026-07-26):** (1) a REAL provider hard-stop — the `ProviderRunner` now returns a `{ completion, forceStop }` handle; cancel/timeout request graceful cancellation, then after the grace `forceStop` kills the real process/supervisor and terminal persistence waits for confirmed termination (tested: a runner that ignores abort and stays alive past the grace is force-stopped, its late `.gaep/evil.json`/source write is detected as `source-mutation`, and no premature cancelled/timed-out record is published); (2) `build-sea.mjs` now verifies the EXACT extracted-executable SHA-256 for BOTH targets (real linux-x64 `08a8fa…` pinned; no archive-digest substitution), with staging + pure-JS ZIP/VSIX content verification, Rider/VS workflows that stage the SEA into build resources (excluded from source identity), discover the real artifact, regenerate the manifest with the real digest, verify the archive embeds the SEA + sidecar, and emit build-only evidence only after the manifest says built — all covered by script-level tests; (3) Rider and Visual Studio actions are executable — real provider/model selection from the live catalog (never hardcoded), ≥1 governed Context Pack ID required (never `[]`), run-ID capture/poll/cancel, VS resolves the Product root via the Workspaces API (never `Environment.CurrentDirectory`), with pure request/validation logic unit-tested in Kotlin (`GaepRequestsTest`) and C# (`GaepRequestsTests`); (4) the example's Engine Host client is hardened — bounded per-request timeout, fail-fast on host startup failure/exit, sanitized stderr, deterministic pending-request cleanup, graceful stdin-close → awaited exit → forced kill after a bounded grace, and machine-local lock-release verification (tested). Native compilation (Rider JDK 21 / Windows + VS 2022) and a `completed` authenticated Claude run remain the only external-lane executions. No feature set to `✅ Done`. |
| 2026-07-24 | Claude Code | Phase 0 / 1A | PLT-04, PLT-11, PLT-32, PLT-33, PLT-34, PLT-35 (implemented, unexecuted in target envs) | PLT-04/11/32/33/34/35 `❌ Backlog -> 🟡 In Progress` | GAEP-P0-CS02: `npm run typecheck` (clean), `npm test` (56 files, 427 passed/1 skipped), `npm run validate:docs` (PASS), `npm run release:cs02 --verify` (VS Code + Kiro built; VS + Rider `not-built`), `npm run release:cs02 -- --verify` (PASS), `npm run verify:cs02:packages` (PASS — extracted-package v3 Engine Host smoke test), `npm run example:cs02 -- --verify` (**superseded — see the 2026-07-26 defect-correction row: the original example bypassed protocol v3 and finished `provider-error`; it was not a real bounded analysis**). Evidence: `dist/phase0/cs02/package-manifest.json` + `SHA256SUMS.txt`, `examples/phase0-provider-model/acceptance/GAEP-P0-CS02_ACCEPTANCE_REPORT.json` (digest `sha256:98d36b75…`) + `GAEP-P0-CS02_EVIDENCE_MANIFEST.md` | Not accepted — awaiting Product Owner test and acceptance | Approved change set GAEP-P0-CS02: engine-host protocol v3 (8 methods), shared `ProviderCatalogService` (server-derived truth, INV-31) + `ReadOnlyAnalysisService` (single execution owner, source-mutation guard, INV-02/03/04), one selection source of truth, non-self-referential `sourceIdentity`, machine-local Engine Host lock, digest-verified bundled Engine Host launch (no PATH fallback), parameterized CS02 evidence verifier keeping the strict C1 wrapper, host-neutral dashboard projection + VS Code Agent/Model surface, 0.2.0 artifacts (VS Code/Kiro built; VS `win32-x64`/Rider `linux-x64` external lanes), and the phase0-provider-model example. PLT-03 stays `🟡 In Progress` (no VS Code install/upgrade/uninstall/workflow lifecycle executed). No feature set to `✅ Done`. |
| 2026-07-24 | Claude Code | Phase 0 / 1A | PLT-27, PLT-28, PLT-29 (evidence hardened; no status change) | None — statuses unchanged | Correction set GAEP-P0-CS01-C1: `npm run typecheck` (clean), `npm test` (42 files, 351 passed/1 skipped), `npm run test:lifecycle` (PASS), `npm run validate:docs` (PASS), `npm run evidence:vscode`, `npm run example:phase0` | Not accepted — awaiting Product Owner test and acceptance | Applied approved correction set GAEP-P0-CS01-C1: exact Four-IDE matrix enforcement (one row per IDE, canonical base defaults, host/observation match) with Base-Snapshot validation before composition; discriminated-union evidence envelope + governed observation-result + exactly-two-artifact manifest with byte-exact digest and path-traversal defenses; current-attempt truth model (executed-passed / executed-failed / not-executed) so a stale `passed` cannot remain effective; strict `PROTOCOL_UPGRADE_REQUIRED` boundary classification with RPC error metadata preserved; VS Code rendering formatter + computation-proof E2E; executable report/register lifecycle validator. No feature set to `✅ Done`. |
| 2026-07-24 | Claude Code | Phase 0 / 1A | PLT-27, PLT-28, PLT-29 (evidence strengthened: PLT-01, PLT-21, PLT-24, PLT-26) | PLT-27 `❌ Backlog -> 🟡 In Progress`; PLT-28 `❌ Backlog -> 🧪 Ready for Test`; PLT-29 `❌ Backlog -> 🟡 In Progress` | GAEP-P0-CS01 implementation: `npm run typecheck` (clean), `npm test` (40 files, 331 passed/1 skipped), `npm run validate:docs` (PASS), `npm run example:phase0`, `npm run test:vscode:extension-host` (PASS). Evidence: `examples/phase0-readiness/acceptance/GAEP-P0-CS01_READINESS_REPORT.json` + `GAEP-P0-CS01_EVIDENCE_MANIFEST.md` (digest `sha256:fd089ebb…`) | Not accepted — awaiting Product Owner test and acceptance | Approved change set GAEP-P0-CS01: shared `computePlatformReadiness()`, engine-host v2 `platformReadiness` endpoint, `packages/conformance` harness + merge, Phase 0 readiness example, and a read-only VS Code `gaep.showPlatformReadiness` command. No feature set to `✅ Done`. |
