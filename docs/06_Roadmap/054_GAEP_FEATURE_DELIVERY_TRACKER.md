...
|---|---|---|---|---|---|
| PLT-01 | Shared GAEP Engine | Phase 0 / 1A | One shared engine with no behavioral forks between hosts | `🟡 In Progress` | `773c05b` adds one shared audit-gated, snapshot-bounded Managed Run summary/detail projection while preserving exact artifact binding and no apply authority; Rider/Visual Studio use and packaged-engine workflow evidence remain incomplete |
...
| PLT-05 | Four-IDE Behavioral Parity | Phase 0 / 1A | Equivalent behavior in VS Code, Visual Studio, Rider, and Kiro | `🟡 In Progress` | `70f4d2a` adds Kiro evidence-dashboard parity and refreshes the 60-cell matrix to 48 implemented, 7 partial and 5 not implemented; the phase gate remains incomplete with zero accepted hosts because Rider/Visual Studio dashboards, effectful review, native and packaged-engine acceptance remain incomplete |
...
| PLT-21 | Dashboard Shell | Phase 0 / 1A | Shared dashboard shell in all four hosts | `🟡 In Progress` | `773c05b` adds an audit-gated bounded Managed Run/evidence surface to Kiro on the same inventory truth used by VS Code. Rider and Visual Studio dashboard parity, interactive Kiro pagination and four-host installation remain incomplete |
...
| PLT-24 | Agent and Model Dashboard | Phase 0 / 1A | Provider, model, run status, cost/usage, and handoff evidence | `🟡 In Progress` | Kiro now adds bounded Run inventory and exact result/evidence detail to the existing cross-host readiness, selection, handoff and managed preview/receipt projections, with provider disposition separated from governed outcome. Rider/Visual Studio Run inventory, usage and native acceptance remain incomplete |
...
| PLT-27 | Four-IDE Conformance Suite | Phase 0 / 1A | Shared contract tests for four IDEs and two providers | `🟡 In Progress` | `70f4d2a` verifies 48 implemented, 7 partial and 5 not-implemented cells across 15 capabilities and four hosts against source markers, exact package bytes/digests and bounded runtime evidence. Rider/Visual Studio dashboards, effectful workflows, native matrices and acceptance remain incomplete |
...
| PLT-29 | Per-Phase Package and Acceptance Report | Phase 0 / 1A | Package, checksum, test report, and known gaps for every phase | `🟡 In Progress` | `70f4d2a` records exact current Kiro/Rider/VS Code package hashes in `evidence/local-packages/2026-07-24T125103Z-phase-0-local-evidence-dashboard-kiro.json` and the 48/7/5 matrix in `evidence/ide-conformance/2026-07-24T125344Z-phase-0-local-evidence-dashboard-kiro.json`, preserving the missing Windows VSIX, native/supported-platform, packaged-engine, human-acceptance and non-release gaps. Later phase reports remain |
