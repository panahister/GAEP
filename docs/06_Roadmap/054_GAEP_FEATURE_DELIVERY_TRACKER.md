...
| PLT-22 | Phase-Scoped Dashboard Framework | Phase 0 / 1A | Compose dashboards according to phase and applicability | `🟡 In Progress` | `fa9b3cd` adds a strict portable seven-phase catalog, exact current-Product revision/digest binding, deterministic composition digest and protocol-v2 `dashboard.framework` query. `89bccd9` binds each Product-backed VS Code Product Studio snapshot to the explicit Phase 0/1A projection and renders the canonical three panels as an accessible table; uninitialized snapshots omit the Product-bound projection, hostile extra/catalog/applicability drift fails closed, and the UI states that no approval, readiness, acceptance or applicability is proved. The 57 focused protocol/data-source/accessibility tests, compact web bundle, full root typecheck/test and extension-host smoke are green. Kiro, Rider and Visual Studio client/presentation adoption, governed applicability decisions, executable-example evidence and human acceptance remain incomplete |
