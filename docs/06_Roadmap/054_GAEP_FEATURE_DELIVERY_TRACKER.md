...
|---|---|---|---|---|---|
| PLT-01 | Shared GAEP Engine | Phase 0 / 1A | One shared engine with no behavioral forks between hosts | `🟡 In Progress` | `c0dabe9`, `1545ef0` and `f125147` route Visual Studio, Rider and Kiro readiness plus guarded selection through the shared engine-host boundary with strict host-local projections; packaged-engine workflow evidence and complete behavioral parity remain incomplete |
...
| PLT-14 | Provider Switching | Phase 0 / 1A | Switch Codex and Claude Code within one Initiative without losing state | `🟡 In Progress` | `c0dabe9`, `1545ef0` and `f125147` expose guarded initial/same-selection Visual Studio, Rider and Kiro surfaces while `995e69d` prevents changes after prior Runs without a versioned handoff and blocks all non-terminal Runs. Handoff collection and a real bidirectional accepted workflow remain incomplete |
...
| PLT-24 | Agent and Model Dashboard | Phase 0 / 1A | Provider, model, run status, cost/usage, and handoff evidence | `🟡 In Progress` | `c0dabe9`, `1545ef0` and `f125147` add selected-state rendering after explicit guarded confirmation in Visual Studio, Rider and Kiro on top of cross-host readiness. Run status, usage, handoff and complete native acceptance remain incomplete |
