...
|---|---|---|---|---|---|
| PLT-01 | Shared GAEP Engine | Phase 0 / 1A | One shared engine with no behavioral forks between hosts | `🟡 In Progress` | `0daba2a` adds shared audit-gated managed.review read/apply/discard RPCs with exact Run revision, preview, staged inventory, write-envelope and predecessor-chain validation; the engine alone owns mutation and Kiro receives no general write API. Rider/Visual Studio parity and packaged-engine workflow evidence remain incomplete |
...
| PLT-04 | Visual Studio Native VSIX | Phase 0 / 1A | Native Visual Studio extension installable on Windows | `🟡 In Progress` | `ebcd36d` adds workspace-bound bounded Managed Run inventory and exact evidence detail to the compiled out-of-process Remote UI and raises the private-safe hostile protocol/controller harness to 124 checks; Windows container creation, installation and rendered interaction remain |
| PLT-05 | Four-IDE Behavioral Parity | Phase 0 / 1A | Equivalent behavior in VS Code, Visual Studio, Rider, and Kiro | `🟡 In Progress` | `ae52108` adds bounded Kiro staged-review parity and refreshes the 60-cell matrix to 51 implemented, 7 partial and 2 not implemented; the phase gate remains incomplete with zero accepted hosts because Rider/Visual Studio effectful review, native and packaged-engine acceptance remain incomplete |
...
| PLT-12 | Staged Codex Effectful Execution | Phase 0 / 1A | Preview, approval, apply, and recovery for Codex changes | `🟡 In Progress` | `0daba2a` exposes one audit-verified complete bounded Kiro review of at most 512 exact changes with two-step cancel-default apply/discard, stale revision/digest rejection and conservative recovery truth. Post-apply gates are recorded not assessed; real-provider packaged execution, multi-step effectful staging and Rider/Visual Studio parity remain incomplete |
...
| PLT-21 | Dashboard Shell | Phase 0 / 1A | Shared dashboard shell in all four hosts | `🟡 In Progress` | `ebcd36d` adds an audit-gated bounded Managed Run/evidence surface to Visual Studio on the same inventory truth used by VS Code, Kiro and Rider. Interactive later-page navigation, native accessibility/rendering and four-host installation remain incomplete |
...
| PLT-24 | Agent and Model Dashboard | Phase 0 / 1A | Provider, model, run status, cost/usage, and handoff evidence | `🟡 In Progress` | Kiro now adds exact staged inventory, apply/discard availability, transition and not-assessed post-apply outcome truth to the existing cross-host readiness, selection, handoff and bounded Run/evidence projections. Usage, Rider/Visual Studio review parity and native acceptance remain incomplete |
...
| PLT-27 | Four-IDE Conformance Suite | Phase 0 / 1A | Shared contract tests for four IDEs and two providers | `🟡 In Progress` | `ae52108` verifies 51 implemented, 7 partial and 2 not-implemented cells across 15 capabilities and four hosts against source markers, exact package bytes/digests and bounded runtime evidence. Rider/Visual Studio effectful review, native matrices and acceptance remain incomplete |
...
| PLT-29 | Per-Phase Package and Acceptance Report | Phase 0 / 1A | Package, checksum, test report, and known gaps for every phase | `🟡 In Progress` | `ae52108` records exact current Kiro/Rider/VS Code package hashes plus the 124-check Windows-package limit in `evidence/local-packages/2026-07-24T135545Z-phase-0-local-staged-review-kiro.json` and the 51/7/2 matrix in `evidence/ide-conformance/2026-07-24T135604Z-phase-0-local-staged-review-kiro.json`, preserving native/supported-platform, packaged-engine, human-acceptance and non-release gaps. Later phase reports remain |
...
| PLT-31 | Rider Native Plugin | Phase 0 / 1A | Native JetBrains/Rider plugin with sandbox and installation tests | `🟡 In Progress` | `09b5eec` adds asynchronous bounded Managed Run inventory and exact evidence-detail actions to the native Product tool window. Clean test/package/structure/prepared-sandbox parity and bounded Rider 2025.3 startup pass; plugin-manager installation, automated interaction, later-page navigation and complete acceptance remain |
...
| PLT-33 | Kiro Compatibility and Smoke Tests | Phase 0 / 1A | Installation, workflow, provider, model, and dashboard smoke tests in Kiro | `🟡 In Progress` | `0daba2a` proves strict hostile staged-review/apply/discard client behavior and the exact installed compatible package activates ten bounded commands without workspace mutation. Native Kiro, a real staged-provider workflow, interactive pagination and packaged-engine interaction remain |
