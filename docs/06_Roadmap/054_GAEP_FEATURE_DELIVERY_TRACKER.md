...
| PLT-31 | Rider Native Plugin | Phase 0 / 1A | Native JetBrains/Rider plugin with sandbox and installation tests | `🟡 In Progress` | `663259f` launches Rider 2025.3 natively on macOS arm64 and proves exact `dev.gaep.productstudio@0.1.0` loading from isolated paths after `7a084b3` package/sandbox parity; plugin-manager installation, tool-window interaction and complete Product workflow parity remain |
