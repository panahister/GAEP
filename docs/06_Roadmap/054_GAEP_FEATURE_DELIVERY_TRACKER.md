...
| PLT-08 | Codex Model Discovery and Selection | Phase 0 / 1A | Display and select the Codex models available to the user | `🟡 In Progress` | `f125147` adds cancel-default Kiro selection of verified Codex adapter/model/non-sensitive portable settings through the `995e69d` shared guards. Rider, Visual Studio, live execution-backed verification and complete acceptance remain incomplete |
| PLT-09 | Claude Model Discovery and Selection | Phase 0 / 1A | Display and select the Claude Code models available to the user | `🟡 In Progress` | `f125147` exposes the same Kiro guarded-selection flow for a detected Claude adapter and fails closed when none is available. Rider/Visual Studio UI, live Claude verification and acceptance remain incomplete |
...
| PLT-14 | Provider Switching | Phase 0 / 1A | Switch Codex and Claude Code within one Initiative without losing state | `🟡 In Progress` | `f125147` exposes the guarded initial/same-selection Kiro surface while `995e69d` prevents changes after prior Runs without a versioned handoff and blocks all non-terminal Runs. Rider/Visual Studio UI, handoff collection and a real bidirectional accepted workflow remain incomplete |
...
| PLT-24 | Agent and Model Dashboard | Phase 0 / 1A | Provider, model, run status, cost/usage, and handoff evidence | `🟡 In Progress` | `f125147` adds Kiro selected-state rendering after explicit guarded confirmation on top of `1ea0141` cross-host readiness. Run status, usage, handoff, Rider/Visual Studio selection and native acceptance remain incomplete |
...
| PLT-31 | Rider Native Plugin | Phase 0 / 1A | Native JetBrains/Rider plugin with sandbox and installation tests | `🟡 In Progress` | `1ea0141` adds typed path-free Codex/Claude readiness to the native Product tool window and passes clean tests/package/prepared-sandbox parity; plugin-manager installation, automated UI interaction, selection/execution and complete parity remain |
