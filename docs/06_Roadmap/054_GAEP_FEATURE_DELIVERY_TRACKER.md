...
|---|---|---|---|---|---|
| PLT-01 | Shared GAEP Engine | Phase 0 / 1A | One shared engine with no behavioral forks between hosts | `🟡 In Progress` | `09b5eec` consumes the shared audit-gated, snapshot-bounded Managed Run summary/detail projection in Rider while preserving exact artifact binding and no apply authority; Visual Studio use and packaged-engine workflow evidence remain incomplete |
...
| PLT-05 | Four-IDE Behavioral Parity | Phase 0 / 1A | Equivalent behavior in VS Code, Visual Studio, Rider, and Kiro | `🟡 In Progress` | `dc19ab3` adds Rider evidence-dashboard parity and refreshes the 60-cell matrix to 49 implemented, 7 partial and 4 not implemented; the phase gate remains incomplete with zero accepted hosts because the Visual Studio dashboard, effectful review, native and packaged-engine acceptance remain incomplete |
...
| PLT-21 | Dashboard Shell | Phase 0 / 1A | Shared dashboard shell in all four hosts | `🟡 In Progress` | `09b5eec` adds an audit-gated bounded Managed Run/evidence surface to Rider on the same inventory truth used by VS Code and Kiro. Visual Studio dashboard parity, interactive later-page navigation and four-host installation remain incomplete |
...
| PLT-24 | Agent and Model Dashboard | Phase 0 / 1A | Provider, model, run status, cost/usage, and handoff evidence | `🟡 In Progress` | Rider now joins Kiro with bounded Run inventory and exact result/evidence detail alongside cross-host readiness, selection, handoff and managed preview/receipt projections, with provider disposition separated from governed outcome. Visual Studio Run inventory, usage and native acceptance remain incomplete |
...
| PLT-27 | Four-IDE Conformance Suite | Phase 0 / 1A | Shared contract tests for four IDEs and two providers | `🟡 In Progress` | `dc19ab3` verifies 49 implemented, 7 partial and 4 not-implemented cells across 15 capabilities and four hosts against source markers, exact package bytes/digests and bounded runtime evidence. The Visual Studio dashboard, effectful workflows, native matrices and acceptance remain incomplete |
...
| PLT-29 | Per-Phase Package and Acceptance Report | Phase 0 / 1A | Package, checksum, test report, and known gaps for every phase | `🟡 In Progress` | `dc19ab3` records exact current Kiro/Rider/VS Code package hashes in `evidence/local-packages/2026-07-24T130549Z-phase-0-local-evidence-dashboard-rider.json` and the 49/7/4 matrix in `evidence/ide-conformance/2026-07-24T130824Z-phase-0-local-evidence-dashboard-rider.json`, preserving the missing Windows VSIX, native/supported-platform, packaged-engine, human-acceptance and non-release gaps. Later phase reports remain |
