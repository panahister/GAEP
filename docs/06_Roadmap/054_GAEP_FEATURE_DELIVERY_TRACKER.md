...
| PLT-20 | Install, Upgrade, and Rollback | Phase 0 / 1A | Testable lifecycle for all four plugins | `🟡 In Progress` | `27973bf` proves exact temporary-profile install, same-version reinstall, uninstall/absence, clean reinstall and activation for VS Code and Kiro-compatible packages. Cross-version transitions, native Kiro/Rider plugin-manager behavior and Visual Studio Windows installation remain |
...
| PLT-29 | Per-Phase Package and Acceptance Report | Phase 0 / 1A | Package, checksum, test report, and known gaps for every phase | `🟡 In Progress` | `c3991aa` adds the deterministic Phase 0 inventory; the latest snapshot at `7a084b3` records exact VS Code, Kiro and Rider hashes plus lifecycle/sandbox verification while preserving the Windows-only Visual Studio gap and non-release claim boundary. Later phase reports and full acceptance evidence remain |
...
| PLT-31 | Rider Native Plugin | Phase 0 / 1A | Native JetBrains/Rider plugin with sandbox and installation tests | `🟡 In Progress` | `7a084b3` produces a clean instrumented plugin ZIP and proves exact packaged-JAR/prepared-sandbox parity with the stable ID, version and tool-window factory; plugin-manager installation, interactive native execution and complete Product workflow parity remain |
...
| PLT-35 | Rider Sandbox and Installation Tests | Phase 0 / 1A | `runIde`, installable artifact, and workflow smoke testing | `🟡 In Progress` | `7a084b3` proves exact archive-to-prepared-sandbox byte parity and descriptor identity with a configuration-cache-clean gate; plugin-manager installation, `runIde` and interactive workflow evidence remain |
