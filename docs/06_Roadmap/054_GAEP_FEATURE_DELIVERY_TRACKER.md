...
| PLT-21 | Dashboard Shell | Phase 0 / 1A | Shared dashboard shell in all four hosts | `🟡 In Progress` | VS Code recovery/evidence observation consumes the bounded Managed Run page contract; `5801b9f` adds the native Rider Product surface and `c65d1ac` adds the equivalent Visual Studio Product/portable-design Remote UI workflow. Complete dashboard parity and four-host installation remain incomplete |
