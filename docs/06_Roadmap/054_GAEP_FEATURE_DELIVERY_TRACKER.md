...
|---|---|---|---|---|---|
| PLT-01 | Shared GAEP Engine | Phase 0 / 1A | One shared engine with no behavioral forks between hosts | `🟡 In Progress` | `1ea0141` routes Kiro, Rider and Visual Studio readiness through the shared `probeAgents` engine-host boundary with strict host-local projections; packaged-engine workflow evidence and complete behavioral parity remain incomplete |
...
| PLT-04 | Visual Studio Native VSIX | Phase 0 / 1A | Native Visual Studio extension installable on Windows | `🟡 In Progress` | `1ea0141` adds observation-only Codex/Claude readiness to the compiled out-of-process Product Remote UI and raises the private-safe protocol/controller harness to 53 checks; Windows container creation, installation and rendered interaction remain |
| PLT-05 | Four-IDE Behavioral Parity | Phase 0 / 1A | Equivalent behavior in VS Code, Visual Studio, Rider, and Kiro | `🟡 In Progress` | `1ea0141` expands the honest executable 60-cell matrix to 38 implemented, 7 partial and 15 not implemented by adding path-free readiness observation in Kiro, Rider and Visual Studio; the phase gate remains incomplete with zero accepted hosts |
...
| PLT-07 | Claude Code Detection | Phase 0 / 1A | Discover Claude Code, version, authentication readiness, and capabilities | `🟡 In Progress` | `1ea0141` adds typed path-free Claude readiness observation and hostile private-field rejection to Kiro, Rider and Visual Studio. Live Claude invocation, native-host and supported-version acceptance remain incomplete |
| PLT-08 | Codex Model Discovery and Selection | Phase 0 / 1A | Display and select the Codex models available to the user | `🟡 In Progress` | `1545ef0` adds cancel-default native Rider selection of verified Codex adapter/model/non-sensitive portable settings after `f125147` added Kiro, both through the `995e69d` shared guards. Visual Studio, live execution-backed verification and complete acceptance remain incomplete |
| PLT-09 | Claude Model Discovery and Selection | Phase 0 / 1A | Display and select the Claude Code models available to the user | `🟡 In Progress` | `1545ef0` exposes the same Rider guarded-selection flow for a detected Claude adapter after `f125147` added Kiro; both fail closed when none is available. Visual Studio UI, live Claude verification and acceptance remain incomplete |
...
| PLT-14 | Provider Switching | Phase 0 / 1A | Switch Codex and Claude Code within one Initiative without losing state | `🟡 In Progress` | `1545ef0` and `f125147` expose guarded initial/same-selection Rider and Kiro surfaces while `995e69d` prevents changes after prior Runs without a versioned handoff and blocks all non-terminal Runs. Visual Studio UI, handoff collection and a real bidirectional accepted workflow remain incomplete |
...
| PLT-16 | Versioned Provider/Model Handoff | Phase 0 / 1A | Transfer context, Run Envelope, and evidence between agents and models | `🟡 In Progress` | `995e69d` makes versioned handoff mandatory at the shared host boundary before a changed selection can follow any prior Run and binds handoff runtime observation for later prepare. Host UI collection and end-to-end acceptance remain incomplete |
...
| PLT-21 | Dashboard Shell | Phase 0 / 1A | Shared dashboard shell in all four hosts | `🟡 In Progress` | VS Code recovery/evidence observation consumes the bounded Managed Run page contract; `1ea0141` adds conservative agent-readiness views to the independent Kiro command surface, native Rider Product tool window and Visual Studio Remote UI. Complete dashboard parity and four-host installation remain incomplete |
...
| PLT-24 | Agent and Model Dashboard | Phase 0 / 1A | Provider, model, run status, cost/usage, and handoff evidence | `🟡 In Progress` | `1545ef0` adds Rider selected-state rendering after explicit guarded confirmation on top of `f125147` Kiro selection and `1ea0141` cross-host readiness. Run status, usage, handoff, Visual Studio selection and complete native acceptance remain incomplete |
...
| PLT-27 | Four-IDE Conformance Suite | Phase 0 / 1A | Shared contract tests for four IDEs and two providers | `🟡 In Progress` | `1ea0141` verifies 38 implemented, 7 partial and 15 not-implemented cells across 15 capabilities and four hosts against source markers, exact package bytes/digests and bounded runtime evidence. Selection/execution workflows, native matrices and acceptance remain incomplete |
...
| PLT-29 | Per-Phase Package and Acceptance Report | Phase 0 / 1A | Package, checksum, test report, and known gaps for every phase | `🟡 In Progress` | `1ea0141` records refreshed exact Kiro/Rider/VS Code package hashes and `evidence/ide-conformance/2026-07-24T092154Z-phase-0-local-readiness.json`, preserving the missing Windows VSIX, native/supported-platform, human-acceptance and non-release gaps. Later phase reports remain |
...
| PLT-31 | Rider Native Plugin | Phase 0 / 1A | Native JetBrains/Rider plugin with sandbox and installation tests | `🟡 In Progress` | `1545ef0` adds asynchronous guarded adapter/model/non-sensitive setting dialogs and selected-state rendering to the native Product tool window. Clean test/package/structure/prepared-sandbox parity and bounded Rider 2025.3 startup pass; plugin-manager installation, automated interaction, execution and complete parity remain |
...
| PLT-33 | Kiro Compatibility and Smoke Tests | Phase 0 / 1A | Installation, workflow, provider, model, and dashboard smoke tests in Kiro | `🟡 In Progress` | `f125147` proves the exact installed compatible package activates six bounded commands including path-free readiness and guarded selection, machine-only configuration and static Product Studio without workspace mutation. Native Kiro, packaged real-engine interaction and complete dashboard parity remain |
