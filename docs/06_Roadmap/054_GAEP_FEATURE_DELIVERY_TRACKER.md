...
|---|---|---|---|---|---|
| PLT-01 | Shared GAEP Engine | Phase 0 / 1A | One shared engine with no behavioral forks between hosts | `🟡 In Progress` | `bf90ac6` completes source-level managed read-only preview/attestation/receipt parity in Visual Studio, Rider and Kiro through the same shared engine-host contract; packaged-engine workflow evidence and complete behavioral parity remain incomplete |
...
| PLT-04 | Visual Studio Native VSIX | Phase 0 / 1A | Native Visual Studio extension installable on Windows | `🟡 In Progress` | `bf90ac6` adds workspace-bound managed read-only preview/attestation/receipt execution to the compiled out-of-process Remote UI and raises the private-safe hostile protocol/controller harness to 98 checks; Windows container creation, installation and rendered interaction remain |
| PLT-05 | Four-IDE Behavioral Parity | Phase 0 / 1A | Equivalent behavior in VS Code, Visual Studio, Rider, and Kiro | `🟡 In Progress` | `1a7ddcb` verifies source-level managed read-only parity in Visual Studio, Rider and Kiro and refreshes the 60-cell matrix to 47 implemented, 7 partial and 6 not implemented; the phase gate remains incomplete with zero accepted hosts because dashboard, effectful review, native and packaged-engine acceptance remain incomplete |
...
| PLT-24 | Agent and Model Dashboard | Phase 0 / 1A | Provider, model, run status, cost/usage, and handoff evidence | `🟡 In Progress` | Visual Studio, Rider and Kiro now render private-safe readiness, selection, handoff and managed preview/receipt projections with provider disposition separated from governed outcome. Full bounded Run/evidence inventory, usage and native acceptance remain incomplete |
...
| PLT-27 | Four-IDE Conformance Suite | Phase 0 / 1A | Shared contract tests for four IDEs and two providers | `🟡 In Progress` | `1a7ddcb` verifies 47 implemented, 7 partial and 6 not-implemented cells across 15 capabilities and four hosts against source markers, exact package bytes/digests and bounded runtime evidence. Dashboard/effectful workflows, native matrices and acceptance remain incomplete |
...
| PLT-29 | Per-Phase Package and Acceptance Report | Phase 0 / 1A | Package, checksum, test report, and known gaps for every phase | `🟡 In Progress` | `1a7ddcb` records exact current Kiro/Rider/VS Code package hashes in `evidence/local-packages/2026-07-24T123302Z-phase-0-local-managed-readonly-visual-studio.json` and the 47/7/6 matrix in `evidence/ide-conformance/2026-07-24T123440Z-phase-0-local-managed-readonly-visual-studio.json`, preserving the missing Windows VSIX, native/supported-platform, packaged-engine, human-acceptance and non-release gaps. Later phase reports remain |
...
| PLT-31 | Rider Native Plugin | Phase 0 / 1A | Native JetBrains/Rider plugin with sandbox and installation tests | `🟡 In Progress` | `454c4a7` adds asynchronous cancel-default managed read-only preview/attestation/receipt execution to the native Product tool window. Clean test/package/structure/prepared-sandbox parity and bounded Rider 2025.3 startup pass; plugin-manager installation, automated interaction, dashboard and complete acceptance remain |
...
| PLT-33 | Kiro Compatibility and Smoke Tests | Phase 0 / 1A | Installation, workflow, provider, model, and dashboard smoke tests in Kiro | `🟡 In Progress` | `f683648` proves the exact installed compatible package activates eight bounded commands including path-free readiness, guarded selection, versioned handoff and digest-attested managed read-only execution without workspace mutation. Native Kiro, packaged real-engine interaction and complete dashboard parity remain |
