...
| PLT-13 | Staged Claude Effectful Execution | Phase 0 / 1A | Preview, approval, apply, and recovery for Claude changes | `🟡 In Progress` | `b9f96cf` adds an experimental provider-neutral SDK staging foundation: bounded isolated source copy, exact staged Claude argv, streamed events, complete digest/metadata inspection, source-preserving review, apply/discard and durable shared recovery. Twenty-five focused tests and the 45-file aggregate with 488 passed plus 1 conditional skip are green. No live provider request was made. Local Claude Code `2.1.153` confirms the flags but bare mode skips OAuth/keychain reads; no API key or `apiKeyHelper` is injected and higher-priority managed policy is unattested. The production adapter remains context-only with Tool selection false and the engine remains `claude-context-only` until a fail-closed runtime preflight is ready |
