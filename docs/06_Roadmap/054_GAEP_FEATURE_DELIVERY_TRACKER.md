...
| PLT-29 | Per-Phase Package and Acceptance Report | Phase 0 / 1A | Package, checksum, test report, and known gaps for every phase | `🟡 In Progress` | `c3991aa` adds the deterministic Phase 0 inventory; the latest snapshot at `9f1af97` records exact VS Code, Kiro and Rider hashes plus cross-version lifecycle/sandbox verification while preserving the Windows-only Visual Studio gap and non-release claim boundary. Later phase reports and full acceptance evidence remain |
