...
| PLT-08 | Codex Model Discovery and Selection | Phase 0 / 1A | Display and select the Codex models available to the user | `🟡 In Progress` | `995e69d` adds the shared guarded selection-state/switch boundary after `1ea0141` added cross-host model observation. Kiro, Rider and Visual Studio selection UIs and complete acceptance remain incomplete |
| PLT-09 | Claude Model Discovery and Selection | Phase 0 / 1A | Display and select the Claude Code models available to the user | `🟡 In Progress` | `995e69d` adds the same guarded shared selection boundary for Claude after cross-host observation. Selection UI, live execution-backed verification and acceptance remain incomplete |
...
| PLT-14 | Provider Switching | Phase 0 / 1A | Switch Codex and Claude Code within one Initiative without losing state | `🟡 In Progress` | `995e69d` prevents cross-host selection from changing provider/model/settings after prior Runs without the versioned handoff path and blocks all non-terminal Runs. Three host UIs and a real bidirectional accepted workflow remain incomplete |
...
| PLT-24 | Agent and Model Dashboard | Phase 0 / 1A | Provider, model, run status, cost/usage, and handoff evidence | `🟡 In Progress` | `1ea0141` adds bounded adapter/runtime/model/capability observation to Kiro, Rider and Visual Studio without selection or execution authority. Run status, usage, handoff, selection and native acceptance remain incomplete |
...
| PLT-33 | Kiro Compatibility and Smoke Tests | Phase 0 / 1A | Installation, workflow, provider, model, and dashboard smoke tests in Kiro | `🟡 In Progress` | `1ea0141` proves the exact installed compatible package activates five bounded commands including path-free Codex/Claude readiness, machine-only configuration and static Product Studio without workspace mutation. Native Kiro, provider/model switching and complete dashboard parity remain |
