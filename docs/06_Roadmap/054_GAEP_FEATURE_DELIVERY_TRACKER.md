...
|---|---|---|---|---|---|
| PLT-01 | Shared GAEP Engine | Phase 0 / 1A | One shared engine with no behavioral forks between hosts | `🟡 In Progress` | `d67d8bb` adds Visual Studio handoff through the same shared Run/selection/handoff contract already used by Kiro and Rider, on top of `21d2202`'s all-non-terminal Run guard; packaged-engine workflow evidence and complete behavioral parity remain incomplete |
...
| PLT-04 | Visual Studio Native VSIX | Phase 0 / 1A | Native Visual Studio extension installable on Windows | `🟡 In Progress` | `d67d8bb` adds workspace-bound versioned handoff to the compiled out-of-process Remote UI and raises the private-safe protocol/controller harness to 77 checks; Windows container creation, installation and rendered interaction remain |
| PLT-05 | Four-IDE Behavioral Parity | Phase 0 / 1A | Equivalent behavior in VS Code, Visual Studio, Rider, and Kiro | `🟡 In Progress` | `94835ba` verifies source-level versioned handoff parity in Visual Studio, Rider and Kiro and refreshes the 60-cell matrix to 44 implemented, 7 partial and 9 not implemented; the phase gate remains incomplete with zero accepted hosts because managed execution, dashboard, native and packaged-engine acceptance remain incomplete |
...
| PLT-24 | Agent and Model Dashboard | Phase 0 / 1A | Provider, model, run status, cost/usage, and handoff evidence | `🟡 In Progress` | `d67d8bb` adds the same private-safe handoff receipt projection to Visual Studio as Rider and Kiro, including source Run, target, workspace observation, preserved-entry counts and capability differences. Full Run status, usage and native acceptance remain incomplete |
...
| PLT-27 | Four-IDE Conformance Suite | Phase 0 / 1A | Shared contract tests for four IDEs and two providers | `🟡 In Progress` | `94835ba` verifies 44 implemented, 7 partial and 9 not-implemented cells across 15 capabilities and four hosts against source markers, exact package bytes/digests and bounded runtime evidence. Managed execution/dashboard workflows, native matrices and acceptance remain incomplete |
...
| PLT-29 | Per-Phase Package and Acceptance Report | Phase 0 / 1A | Package, checksum, test report, and known gaps for every phase | `🟡 In Progress` | `94835ba` records exact current Kiro/Rider/VS Code package hashes in `evidence/local-packages/2026-07-24T112425Z-phase-0-local-handoff.json` and the 44/7/9 matrix in `evidence/ide-conformance/2026-07-24T112440Z-phase-0-local-handoff.json`, preserving the missing Windows VSIX, native/supported-platform, packaged-engine, human-acceptance and non-release gaps. Later phase reports remain |
...
| PLT-31 | Rider Native Plugin | Phase 0 / 1A | Native JetBrains/Rider plugin with sandbox and installation tests | `🟡 In Progress` | `d6a16c6` adds asynchronous guarded selection and versioned handoff dialogs with exact returned receipt validation to the native Product tool window. Clean test/package/structure/prepared-sandbox parity and bounded Rider 2025.3 startup pass; plugin-manager installation, automated interaction, execution and complete parity remain |
...
| PLT-33 | Kiro Compatibility and Smoke Tests | Phase 0 / 1A | Installation, workflow, provider, model, and dashboard smoke tests in Kiro | `🟡 In Progress` | `21d2202` proves the exact installed compatible package activates seven bounded commands including path-free readiness, guarded selection and versioned handoff, machine-only configuration and static Product Studio without workspace mutation. Native Kiro, managed execution, packaged real-engine interaction and complete dashboard parity remain |
