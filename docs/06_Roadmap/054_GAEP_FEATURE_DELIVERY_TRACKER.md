...
|---|---|---|---|---|---|
| PLT-01 | Shared GAEP Engine | Phase 0 / 1A | One shared engine with no behavioral forks between hosts | `🟡 In Progress` | `ebcd36d` consumes the shared audit-gated, snapshot-bounded Managed Run summary/detail projection in Visual Studio alongside Kiro and Rider while preserving exact artifact binding and no apply authority; packaged-engine workflow evidence remains incomplete |
...
| PLT-05 | Four-IDE Behavioral Parity | Phase 0 / 1A | Equivalent behavior in VS Code, Visual Studio, Rider, and Kiro | `🟡 In Progress` | `8721d13` adds Visual Studio evidence-dashboard parity and refreshes the 60-cell matrix to 50 implemented, 7 partial and 3 not implemented; the phase gate remains incomplete with zero accepted hosts because effectful review, native and packaged-engine acceptance remain incomplete |
...
| PLT-24 | Agent and Model Dashboard | Phase 0 / 1A | Provider, model, run status, cost/usage, and handoff evidence | `🟡 In Progress` | Visual Studio now joins Kiro and Rider with bounded Run inventory and exact result/evidence detail alongside cross-host readiness, selection, handoff and managed preview/receipt projections, with provider disposition separated from governed outcome. Usage and native acceptance remain incomplete |
...
| PLT-27 | Four-IDE Conformance Suite | Phase 0 / 1A | Shared contract tests for four IDEs and two providers | `🟡 In Progress` | `8721d13` verifies 50 implemented, 7 partial and 3 not-implemented cells across 15 capabilities and four hosts against source markers, exact package bytes/digests and bounded runtime evidence. Effectful workflows, native matrices and acceptance remain incomplete |
...
| PLT-29 | Per-Phase Package and Acceptance Report | Phase 0 / 1A | Package, checksum, test report, and known gaps for every phase | `🟡 In Progress` | `8721d13` records exact current Kiro/Rider/VS Code package hashes plus the 124-check Windows-package limit in `evidence/local-packages/2026-07-24T132236Z-phase-0-local-evidence-dashboard-visual-studio.json` and the 50/7/3 matrix in `evidence/ide-conformance/2026-07-24T132516Z-phase-0-local-evidence-dashboard-visual-studio.json`, preserving native/supported-platform, packaged-engine, human-acceptance and non-release gaps. Later phase reports remain |
...
| PLT-33 | Kiro Compatibility and Smoke Tests | Phase 0 / 1A | Installation, workflow, provider, model, and dashboard smoke tests in Kiro | `🟡 In Progress` | `773c05b` proves the exact installed compatible package activates nine bounded commands including an audit-gated Managed Run evidence dashboard without workspace mutation. Native Kiro, interactive pagination beyond the first bounded page and packaged real-engine interaction remain |
