...
|---|---|---|---|---|---|
| PLT-01 | Shared GAEP Engine | Phase 0 / 1A | One shared engine with no behavioral forks between hosts | `🟡 In Progress` | `21d2202` adds Kiro handoff through the shared engine-host contract and closes the engine's all-non-terminal Run guard, on top of `c0dabe9`, `1545ef0` and `f125147` guarded selection; Rider/Visual Studio handoff, packaged-engine workflow evidence and complete behavioral parity remain incomplete |
...
| PLT-14 | Provider Switching | Phase 0 / 1A | Switch Codex and Claude Code within one Initiative without losing state | `🟡 In Progress` | `21d2202` adds a Kiro versioned switch that binds the current selection and latest terminal Run to a freshly observed target with preserved portable history; `995e69d` and the strengthened shared engine reject every non-terminal Run. Rider/Visual Studio handoff and a real bidirectional accepted workflow remain incomplete |
| PLT-15 | Model Switching | Phase 0 / 1A | Change models within each provider while recording provenance | `🟡 In Progress` | `21d2202` records Kiro model/setting switches through an atomic versioned handoff and target selection. Rider/Visual Studio provenance flows, packaged-engine interaction and four-host acceptance remain incomplete |
| PLT-16 | Versioned Provider/Model Handoff | Phase 0 / 1A | Transfer context, Run Envelope, and evidence between agents and models | `🟡 In Progress` | `21d2202` adds strict Kiro source-Run/prior-selection binding, bounded portable completed/unresolved/decision/evidence details, stale-state recheck, fresh target observation and cancel-default atomic handoff/selection. Rider/Visual Studio UI, live bidirectional provider acceptance and packaged-engine evidence remain incomplete |
...
| PLT-24 | Agent and Model Dashboard | Phase 0 / 1A | Provider, model, run status, cost/usage, and handoff evidence | `🟡 In Progress` | `21d2202` adds a Kiro handoff receipt projection with source Run, target, workspace observation and preserved-entry counts on top of cross-host readiness/selection. Full Run status, usage, cross-host handoff views and native acceptance remain incomplete |
