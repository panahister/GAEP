...
|---|---|---|---|---|---|---|---|
| `gaep.rel.targets` | `targets` | bounded work intends to affect a durable subject | Initiative, Change -> Managed Asset | GAEP-CORE-001 | 1.0 | source 1..n; target 0..n | no automatic validity effect; inverse `targeted-by` |
| `gaep.rel.decomposes-into` | `decomposes-into` | work is planned as smaller work | Initiative, Change, Work Item -> Work Item | GAEP-CORE-001 | 1.0 | source 0..n; child exactly one direct parent unless profile permits shared work | not transitive by default; inverse `part-of-work` |
...
| `gaep.rel.supersedes` | `supersedes` | source replaces target authority for declared scope and time | Resource Revision, Baseline Set -> same type | GAEP-CORE-003 | 1.0 | source 0..n; target 0..n by disjoint scopes | may affect revision disposition only for declared scope; inverse `superseded-by` |
...
| `gaep.rel.derived-from` | `derived-from` | source content was transformed from target | Resource Revision -> Resource Revision, External Resource | GAEP-CORE-003 | 1.0 | source 1..n; target 0..n | provenance required; change may create freshness review; inverse `source-of-derivation` |
...
| GAEP-REL-REQ-002 | A relationship SHALL NOT assert approval, authorization, conformance, causation or evidence strength beyond its registered semantics. | Misuse scenarios |
