...
- evidence IDs, as-of date, bounded rationale, and limitation;
- delivery state: shipped, preview/beta, announced roadmap, community extension, inference, or not assessed.

`unknown` is never rendered as No. `unsupported-by-reviewed-evidence` requires reviewed evidence that affirmatively establishes the bounded lack; absence of documentation is insufficient. A shipped state cannot be inferred from roadmap wording.

## 6. GAEP maturity

GAEP uses repository evidence at the accepted P01 starting commit. Each capability is separately classified as:
...
Every candidate statement has a stable ID, audience/context, exact wording, claim class, evidence and GAEP-maturity bindings, disposition, qualifiers, freshness trigger, accountable owner role, and approval/publication state. Allowed wording is bounded to reviewed evidence. The validator blocks unsupported superiority, ROI, time-saving, quality, security, compliance, certification, readiness, guarantee, and competitor-absence claims.
...
Validation fails closed for duplicate/unknown IDs, missing cells, stale bindings, non-HTTPS official URIs, impossible chronology, conflicting states, noncanonical ordering/serialization, broken supersession, unsupported claims, status inflation, type confusion, orphan evidence, unauthorized approval/publication, invented ranking, unresolved seed ambiguity disguised as selection, cross-catalog duplicate truth, and stale projections.
...
| GAEP-MKT-REQ-003 | Every Product/capability pair SHALL have exactly one evidence-bound or explicitly unknown/not-applicable cell. | Matrix completeness test |
...
| GAEP-MKT-REQ-007 | Every factual or comparative executive claim SHALL bind sufficient reviewed evidence and applicable GAEP maturity. | Claim validator |
