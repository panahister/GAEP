...
| GAEP-CLAIM-011 | The exact candidate corpus conforms to its declared structural and semantic rules | exact-revision metadata, requirement, registry, link, dependency, semantic-scenario and independent-review evidence | exact Batch 1 structural checks are partial evidence; semantic and independent review remain missing | `not-assessed` |
...
| GAEP-CLAIM-004 | Independent ontology, dependency, identity/cardinality/lifetime/state-allocation, negative-case, and cross-domain scenarios with no material contradiction would support coherent profile-independent semantics. | Option B, its relationship/state registry consequences, and `S19` through `S36` are recorded at the exact Batch 1 checkpoint; the validator is only a structural input; branch `not-assessed` | the pending semantic decision becomes effective through valid authority records, Core contraction preserves invariants, and scenarios cover materially different domains and realizations | scenario definitions have not been executed or independently reviewed; 12 contracts, 353 requirements, 70 Core Open Decisions, four normative conflicts, and 13 extraction/defer rows remain; constitutional and cross-document propagation are incomplete |
...
| GAEP-CLAIM-009 | Version-bound legacy/candidate and external mappings plus federation scenarios preserving authority, source meaning, mapping fidelity, compatibility by dimension, freshness, loss, rollback, and degraded states would support reliable distributed trace. | staged dual-read, no-initial-dual-write, mapping-fidelity, consumer, rollback, and historical-reconstruction rules and scenarios exist only as candidate structure; branch `not-assessed` | exact source/target revisions, lawful snapshots, stable identifiers, declared read authority, single-write authority, resolver availability, and conflict rules | no migration rehearsal or federation pilot; mapping outputs do not exist; external sources are not version- or rights-assessed; inaccessible and divergent-source behavior is untested |
...
| GAEP-CLAIM-011 | Deterministic validation plus independent semantic, usability, rights, and migration review against an exact revision would support corpus conformance for the declared class. | two structurally equivalent validator executions against commit `0719efb75b09c8704d08385d6cd61541fd9c614c` pass 79 documents and 955 requirement definitions with zero warnings; no distinct strict-mode behavior exists; branch still `not-assessed` | validator scope and implementation are fit, exact evidence is reproducible, and independent review can reproduce and extend the result | structural checks cannot prove semantic correctness, authority validity, Product value, compatibility, or readiness; no immutable Candidate Revision Set, independent conclusion, rights review, migration rehearsal, or assurance evaluation exists |
...
- defined but unexecuted Product/Initiative paper scenarios, and no independent semantic review, migration rehearsal, GAEP-on-GAEP evidence result, participant pilot, or implementation-slice definition.
