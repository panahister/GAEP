...
| GAEP-GAP-005 | high | Static metadata, ID, dependency, link, and requirement integrity require repeatable validation; static checks cannot prove semantic correctness. | `scripts/validate_next_docs.rb`; 2026-07-20 working-tree run passed for 79 documents and 955 requirement definitions with zero warnings; 44 legacy documents mapped; 59 Product Strategy decisions crosswalked; 70 Core Open Decisions and 50 Repository Gaps registered; `GAEP-CLAIM-011` | broken references or duplicated IDs may enter a candidate package; a green check may be overstated | retain exact-revision validator evidence and add independent semantic review | GAEP Specification Steward and GAEP Assurance Authority; Candidate Baseline Gate | open-needs-validation |
...
| GAEP-GAP-050 | blocker | The legacy Constitution, principles, lifecycle model, navigation, glossary, and terminology define or repeat Product as an Engineering Initiative type, while the candidate normatively defines Product as a durable Managed Asset targeted by bounded Initiatives; `GAEP-DEC-007` remains unresolved. | legacy `000_READ_FIRST.md`; `01_Foundation/001_GAEP_CONSTITUTION.md`, `01_Foundation/004_CORE_PRINCIPLES.md`, `01_Foundation/006_ADAPTIVE_ENGINEERING_PRINCIPLES.md`; `02_Platform/018_STATE_MODEL.md`, `02_Platform/019_DYNAMIC_ENGINEERING_MODEL.md`; `03_Product_Engineering/021_PRODUCT_LIFECYCLE.md`; `99_References/991_GLOSSARY.md`, `99_References/992_TERMINOLOGY.md`; `GAEP-CORE-001`; `GAEP-PROF-001`; `GAEP-SELF-003`; `GAEP-RM-004` | Product identity, lifecycle state ownership, Profile applicability, cardinality, schemas, trace, navigation, terminology, compatibility, rollback, and historical interpretation may break or silently diverge | complete `GAEP-DEC-007` with legacy, candidate, dual-model, no-action, and valid-deferral outcomes plus a separate compatibility treatment; bind exact revisions and human authority; approve clause-level reconciliation and any authoritative migration before baseline designation | GAEP Core Specification Steward, GAEP Constitutional Owner, and a separately assigned semantic decision authority; Candidate Baseline Gate | open-needs-decision |
