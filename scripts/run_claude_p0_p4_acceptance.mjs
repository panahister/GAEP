...
async function runTest() {
  ...
  if (summaries.length !== 1) {
    throw new Error(`P1-31 integration emitted ${summaries.length} semantic summaries instead of exactly one`)
  }
  if (!/Test Files\s+1 passed \(1\)/u.test(stdout) || !/Tests\s+1 passed/u.test(stdout)) {
    throw new Error("P1-31 integration did not report exactly one passing focused test file and test")
  }
  if (stderr.trim().length > 0) throw new Error(`P1-31 integration wrote unexpected stderr: ${stderr.trim()}`)
  return summaries[0]
}
...
export async function runClaudeP0P4Acceptance() {
  ...
  const receipt = {
    schemaVersion: 1,
    kind: "gaep-claude-p0-p4-acceptance-receipt",
    scenario: {
      id: "P1-31",
      testFile,
      testName: claudeP0P4TestName,
    ...
    limitations: claudeP0P4Limitations,
  }
...
function parseArguments(args) {
  ...
  throw new Error("Usage: node scripts/run_claude_p0_p4_acceptance.mjs [--output <new-receipt.json>]")
}

async function main() {
  const options = parseArguments(process.argv.slice(2))
  if (options.help) {
    process.stdout.write("Usage: node scripts/run_claude_p0_p4_acceptance.mjs [--output <new-receipt.json>]\n")
    return
  }
