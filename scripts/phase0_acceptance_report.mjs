...
import { verifyPhase3aClaudeReadinessWorkflowArtifactDirectory } from "./phase3a_claude_readiness_workflow_artifacts.mjs"
import { verifyPhase3aRealisticReadinessExampleArtifactDirectory } from "./phase3a_realistic_readiness_example_artifacts.mjs"
...
const defaultPaths = {
  ...
  packages: "evidence/local-packages/20260731T183635Z-phase-3b-controlled-design-to-code-generation-packages.json",
  conformance: "evidence/ide-conformance/20260731T183635Z-phase-3b-controlled-design-to-code-generation.json",
  example: "evidence/examples/20260731T184200Z-phase-3a-realistic-readiness-example/receipt.json",
}
...
async function verifiedSources(root, paths) {
  ...
  const receipt = example.value?.kind === "gaep-phase3a-realistic-readiness-example-receipt"
    ? (await verifyPhase3aRealisticReadinessExampleArtifactDirectory(dirname(example.resolved))).receipt
    : example.value?.kind === "gaep-phase3a-claude-readiness-workflow-receipt"
    ? (await verifyPhase3aClaudeReadinessWorkflowArtifactDirectory(dirname(example.resolved))).receipt
    : example.value?.kind === "gaep-phase3a-codex-readiness-workflow-receipt"
...
function knownGaps(inputs, { controlledDesignToCodeGeneration, stagingWorkspace, proposedChangePreview, changedUnitInventory, designApplicability, designPersonasRoles, userJourneys, informationArchitecture, screenStateInventory, designRequirements, backlogHierarchy, mvpSliceDefinition, prioritizationModel, acceptanceCriteria, definitionOfReady, definitionOfDone, implementationUnitModel, dependencyMapping, technologyProfile, boilerplateRegistry, boilerplateSelectionBinding, boilerplateCompatibilityValidation, figmaToBoilerplateMapping, designToCodeBindingRegistry, routeScreenComponentMapping, testMethodology, testInventory, highLevelDesign, lowLevelDesign, implementationReadinessGate, phase3aRealisticReadinessExample, phase3aDashboard, phase3aCodexReadinessWorkflow, phase3aClaudeReadinessWorkflow, designSystemTokenContract, accessibilityDesignRules, responsiveMultiPlatformTargets, manualFigmaExecutionPath, figmaMcpCapabilityDiscovery, figmaReadSnapshot, figmaContextImport, outboundDesignBriefPackage, governedFigmaWrite, finalizedFigmaSnapshotImport, designToRequirementBinding, designerReadyGate, designDelta, designConflictResolution, humanDesignApproval, designBaseline, designDriftDetection, phase2UxFigmaDashboard, phase2ChangeImpactAgentModelDashboard, phase2RealisticFigmaLoop }) {
  ...
  return [
    ...
    controlledDesignToCodeGeneration
      ? {
          id: "phase-3b-controlled-design-to-code-generation-closure",
          state: "not-established",
          basis: "the exact immutable Controlled Design-to-Code Generation plan candidate binds the current Product, Initiative, approved Figma context retrieval, design baseline, design-to-requirement, Figma-to-boilerplate, design-to-code, route-screen-component, implementation-unit, technology, boilerplate selection and compatibility, proposed change, isolated staging, controlled Codex and Claude, provider-switch and model-switch candidates; one selected provider and model, exact repository-relative target paths, expected trace and test outputs, deterministic dependency, provider, design-context, target, expected-output, lifecycle, prerequisite and assessment receipts, explicit no-effect generation stop lines, evidence, Product Studio and four host projections are implemented locally; actual Figma access, context materialization or transfer, provider execution, generated output or inspection, stage creation, source mutation, approval, authorization, acceptance, generation readiness, native-host, live-provider, security or Product Owner acceptance, release and deployment remain unestablished",
        }
      : approvedFigmaContextRetrieval
      ? {
          id: "phase-3b-approved-figma-context-retrieval-closure",
          state: "not-established",
          basis: "the exact immutable Approved Figma Context Retrieval candidate binds the current Product, Initiative, Design Applicability, Finalized Figma Snapshot Import, Human Design Approval, Design Baseline, Design-to-Requirement Binding, Design-to-Code Binding Registry, Route Screen Component Mapping, Proposed Change Preview, Isolated Staging Workspace and Model Switch Implementation; exact external file identity, returned version, approved scope, decision, baseline lineage, design and implementation continuity metadata, deterministic dependency, snapshot, generation-context, lifecycle and assessment receipts, explicit no-effect retrieval stop lines, evidence, Product Studio and four host projections are implemented locally; actual Figma access, remote fetch, content materialization or transfer, snapshot or design content, generation, provider execution, stage effects, source mutation, design approval or baseline establishment, generation readiness, native-host, live-provider, security or Product Owner acceptance, release and deployment remain unestablished",
        }
      ...
      ? {
          ...
          state: "not-established",
          basis: "the exact immutable Provider Switching During Implementation candidate binds the current Product, Initiative, Proposed Change Preview, Isolated Staging Workspace, Controlled Codex and Controlled Claude candidates, Agent Selections, plans and recovery receipts; explicit switch direction, exact provider, agent, model, unit and path continuity, deterministic binding, provider, continuity and handoff receipts, candidate-only handoff metadata, lifecycle stop lines, prerequisites, evidence, Product Studio and four host projections are implemented locally; actual provider execution or transition, handoff recording, stage-ownership transfer, resume, supported Claude runtime or access, approval, authorization, source mutation, apply, discard, recovery execution, native-host, live-provider, security or Product Owner acceptance, release and deployment remain unestablished",
        }
      ...
      ? {
          ...
          state: "not-established",
          basis: "the exact immutable Proposed Change Preview candidate lifecycle binds the current Changed Unit Inventory and its exact units and repository-relative paths; source and target endpoint metadata, diff metadata, plan operations, traces, attributable evidence, deterministic receipts, Product Studio and four host projections are implemented locally; repository, path, source, target or diff truth, approved change scope or approval, file or diff content, mutation, staging, apply or discard, assignment, execution, acceptance, merge, release, deployment, native-host interaction and Product Owner acceptance remain unestablished",
        }
      ...
      ? {
          ...
          state: "not-established",
          basis: "the exact deterministic realistic Phase 3A readiness example byte-binds 20 current governed candidates, 81 evidence-reference metadata entries, five canonical dashboard views, two sealed offline Codex and Claude provider workflows, two reopen observations, three fail-closed recovery cases and four current host projections; fixture evidence does not establish real Product, artifact, evidence or semantic truth, completeness, priority, waiver or owner authority, implementation readiness, native-host interaction, live-provider acceptance, Product Owner acceptance, security completion, release authorization or deployment approval",
        }
      : phase3aDashboard
      ? {
      ...
      ? {
          ...
          state: "not-established",
          basis: "the exact deterministic Claude Phase 3A readiness workflow binds semantic parity to the sealed Codex scenario, 20 ordered P3A-01 through P3A-20 candidate stages, a fresh isolated tool-free context-only Claude receipt, two exact reopen observations, three fail-closed stale, gap and conflict recovery cases, the Product Studio Implementation Readiness table, four current host projections and zero live requests, credentials, policy bypasses or implementation effects; fixture evidence does not establish a supported Claude runtime, authentication or policy readiness, semantic quality, real Product, artifact or evidence truth, waiver or owner authority, implementation readiness, native-host interaction, live-provider acceptance, Product Owner acceptance, security completion, release or deployment authority",
        }
      ...
      ? {
          ...
          state: "not-established",
          basis: "the exact versioned Low-Level Design candidate lifecycle per Implementation Unit, bounded modules, classes, components, interfaces, data contracts, algorithms, states, error recovery, authorization, observability and test hooks, exact current High-Level Design plus all 15 prior dependencies, deterministic structure, dependency, trace, coverage, ownership and assessment receipts, immutable revisions, privacy-safe Product Studio table and four host projections are implemented locally; candidates do not establish design, repository, source, runtime or deployment truth or completeness, design baseline or approval, privacy or security approval, owner appointment, implementation readiness, acceptance, release, deployment, native-host interaction or Product Owner acceptance",
        }
      : highLevelDesign
      ? {
          id: "phase-3a-high-level-design-closure",
          state: "not-established",
          basis: "the exact versioned High-Level Design candidate lifecycle, bounded elements, relations, decisions, quality attributes and deployment views, all 15 exact current architecture, context, technology, dependency, implementation-unit, boilerplate, design, test, risk and security/privacy bindings, deterministic structure, dependency, trace, coverage, ownership and assessment receipts, immutable revisions, privacy-safe Product Studio table and four host projections are implemented locally; candidates do not establish architecture, repository, runtime or deployment truth or completeness, architecture baseline or approval, privacy or security approval, owner appointment, implementation readiness, acceptance, release, deployment, native-host interaction or Product Owner acceptance",
        }
      : testInventory
      ? {
      ...
      ? {
          ...
          state: "not-established",
          basis: "the exact versioned Test Methodology candidate lifecycle, one exact scope per current Implementation Unit, bounded method, level, representation, automation, environment, platform, test-data policy, evidence expectation, entry, exit and owner candidates, deterministic scope, methodology, environment, data-policy, ownership, trace and assessment receipts, exact current Acceptance Criteria, Definition of Ready, Definition of Done, Implementation Unit Model, Dependency Mapping, Security and Privacy Threat Assessment and Route, Screen, and Component Mapping bindings, immutable revisions, privacy-safe Product Studio table and four host projections are implemented locally; candidates do not establish Requirement or Acceptance Criteria truth, methodology validity or completeness, environment availability, test-data fitness, privacy or security approval, owner appointment, test execution or results, evidence or coverage truth, quality, implementation readiness, acceptance, release, deployment, native-host interaction or Product Owner acceptance",
        }
      : routeScreenComponentMapping
      ? {
          id: "phase-3a-route-screen-component-mapping-closure",
          state: "not-established",
          basis: "the exact versioned Route, Screen, and Component Mapping candidate lifecycle, one exact route, screen, state, or component subject per current Information Architecture and Screen and State Inventory source subject, typed route-screen, screen-state, screen-component and component-component relationships, mapped, conflict, unmapped and not-assessed candidate outcomes, exact Requirement, Acceptance Criteria, Implementation Unit, Design-to-Code Binding and optional test-hook trace candidates, evidence and attributable review candidates, deterministic subject-catalog, relationship-catalog, trace, mapping and assessment receipts, exact current Information Architecture, Screen and State Inventory, Design Requirements, Design Baseline, Design-to-Requirement Binding, Figma-to-Boilerplate Mapping, Design-to-Code Binding Registry, Implementation Unit Model and Acceptance Criteria bindings, immutable revisions, privacy-safe Product Studio table and four host projections are implemented locally; candidates do not connect to or call Figma, establish returned Figma content, navigation or mapping truth, UI or design validity, requirement, acceptance-criteria, test-coverage, repository, path or symbol truth, create or change code or design targets, establish implementation readiness or completeness, assignment, execution, acceptance, merge, release, deployment, native-host interaction or Product Owner acceptance",
        }
      : designToCodeBindingRegistry
      ? {
          id: "phase-3a-design-to-code-binding-registry-closure",
          state: "not-established",
          basis: "the exact versioned Design-to-Code Binding Registry candidate lifecycle, one exact binding subject per current Figma-to-Boilerplate Mapping subject, bounded component, configuration, layout, module, route, style, symbol, test-hook and token binding kinds, bound, conflict, unbound and not-assessed candidate outcomes, exact Implementation Unit, Requirement and repository, module, path and optional symbol target candidates, evidence and attributable human binding candidates, deterministic binding-subject, code-target, trace, binding and assessment receipts, exact current Design Baseline, Finalized Figma Snapshot Import, Design-to-Requirement Binding, Figma-to-Boilerplate Mapping, Implementation Unit Model, Technology Profile, Boilerplate Selection and Binding and Boilerplate Compatibility Validation bindings, immutable revisions, privacy-safe Product Studio table and four host projections are implemented locally; candidates do not connect to or call Figma, establish returned Figma content, design validity, approval or baseline, mapping or binding truth or completeness, repository, path or symbol truth, create or change code targets, retrieve, import, instantiate, generate or execute assets, establish implementation readiness or completeness, assignment, execution, acceptance, merge, release, deployment, native-host interaction or Product Owner acceptance",
        }
...
export async function buildPhase0AcceptanceReport({
  ...
  const designToCodeBindingRegistry = inputs.conformance.hosts.every((host) =>
    host.capabilities.some((capability) =>
      capability.capabilityId === "design-to-code-binding-registry" && capability.state === "implemented"))
  const routeScreenComponentMapping = inputs.conformance.hosts.every((host) =>
    host.capabilities.some((capability) =>
      capability.capabilityId === "route-screen-component-mapping" && capability.state === "implemented"))
  ...
  const highLevelDesign = inputs.conformance.hosts.every((host) =>
    host.capabilities.some((capability) =>
      capability.capabilityId === "high-level-design" && capability.state === "implemented"))
  ...
  const approvedFigmaContextRetrieval = inputs.conformance.hosts.every((host) =>
    host.capabilities.some((capability) =>
      capability.capabilityId === "approved-figma-context-retrieval" && capability.state === "implemented"))
  const controlledDesignToCodeGeneration = inputs.conformance.hosts.every((host) =>
    host.capabilities.some((capability) =>
      capability.capabilityId === "controlled-design-to-code-generation" && capability.state === "implemented"))
  const modelSwitchImplementation = inputs.conformance.hosts.every((host) =>
    host.capabilities.some((capability) =>
      capability.capabilityId === "model-switch-implementation" && capability.state === "implemented"))
  ...
  const phase3aRealisticReadinessExample = inputs.exampleKind === "gaep-phase3a-realistic-readiness-example-receipt"
  ...
  const report = {
    ...
    evidenceScope: controlledDesignToCodeGeneration
      ...
      : designToCodeBindingRegistry
      ? "phase-3a-design-to-code-binding-registry-local"
      : figmaToBoilerplateMapping
    ...
    claimBoundary: controlledDesignToCodeGeneration
      ? "This report binds the exact governed Controlled Design-to-Code Generation offline plan candidate lifecycle, immutable revision history, exact current Product, Initiative, Approved Figma Context Retrieval, Design Baseline, Design-to-Requirement Binding, Figma-to-Boilerplate Mapping, Design-to-Code Binding Registry, Route Screen Component Mapping, Implementation Unit Model, Technology Profile, Boilerplate Selection and Binding, Boilerplate Compatibility Validation, Proposed Change Preview, Isolated Staging Workspace, Controlled Codex and Claude Implementation, Provider Switch Implementation and Model Switch Implementation bindings, one exact selected provider, agent and model, metadata-and-digests-only design context, exact repository-relative target paths, expected trace keys and test outputs, deterministic dependency, provider, design-context, target, expected-output, lifecycle, prerequisite and assessment receipts, explicit no-effect generation stop lines, evidence, portable protocol-v2 transport, privacy-safe Product Studio table and four host projections to current package, test, host and conformance evidence. It does not establish actual Figma access, protected-context materialization or transfer, provider execution, generated output or inspection, stage creation, source mutation, approval, authorization, acceptance, generation readiness, merge, release, deployment, native-host interaction, live-provider acceptance, Product Owner acceptance, security completion or action authority."
      : approvedFigmaContextRetrieval
      ? "This report binds the exact governed Approved Figma Context Retrieval candidate lifecycle, immutable revision history, exact current Product, Initiative, Design Applicability, Finalized Figma Snapshot Import, Human Design Approval, Design Baseline, Design-to-Requirement Binding, Design-to-Code Binding Registry, Route Screen Component Mapping, Proposed Change Preview, Isolated Staging Workspace and Model Switch Implementation bindings, exact external file identity and returned version digests, approved snapshot scope and human-decision receipts, baseline lineage and version metadata, design and implementation continuity counts and digests, deterministic dependency, snapshot, generation-context, lifecycle and assessment receipts, explicit no-effect retrieval stop lines, evidence, portable protocol-v2 transport, privacy-safe Product Studio table and four host projections to current package, test, host and conformance evidence. It does not establish actual Figma access, remote fetch, content materialization or transfer, snapshot or design content, generation readiness, provider execution, code generation, stage effects, source mutation, design approval or baseline establishment, approval, authorization, acceptance, merge, release, deployment, native-host interaction, live-provider acceptance, Product Owner acceptance, security completion or action authority."
      : modelSwitchImplementation
      ...
      : designToCodeBindingRegistry
      ? "This report binds the exact governed Design-to-Code Binding Registry candidate lifecycle, immutable revision history, one exact binding subject per current Figma-to-Boilerplate Mapping subject, bounded component, configuration, layout, module, route, style, symbol, test-hook and token binding kinds, bound, conflict, unbound and not-assessed candidate outcomes, exact Implementation Unit, Requirement and repository, module, path and optional symbol target candidates, evidence and attributable human binding candidates, deterministic binding-subject, code-target, trace, binding and assessment receipts, exact current Product, Initiative, Design Baseline, Finalized Figma Snapshot Import, Design-to-Requirement Binding, Figma-to-Boilerplate Mapping, Implementation Unit Model, Technology Profile, Boilerplate Selection and Binding and Boilerplate Compatibility Validation bindings, portable protocol-v2 transport, privacy-safe Product Studio table and four host projections to current package, test, host and conformance evidence. Design-to-Code bindings remain governed candidates only. This report does not connect to or call Figma; establish returned Figma content, design validity, approval or baseline, mapping or binding truth or completeness, repository, path or symbol truth; create or change code targets; retrieve, import, instantiate, generate or execute assets; establish implementation readiness or completeness, assignment, execution, acceptance, merge, release, deployment, implementation, write or action authority; prove native-host or Product Owner acceptance, Product readiness, security approval, release authorization or deployment approval."
      : figmaToBoilerplateMapping
