...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-acceptance-criteria-local")
  assert.match(report.claimBoundary, /structured precondition, stimulus and expected-result criteria/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-acceptance-criteria-closure")
