...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-2-change-impact-agent-model-dashboard-local")
  assert.match(report.claimBoundary, /synchronization-change, bounded-impact/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-2-change-impact-agent-model-dashboard-closure")
