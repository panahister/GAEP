...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-2-design-to-requirement-binding-local")
  assert.match(report.claimBoundary, /exact Design-to-Requirement Binding review-candidate lifecycle/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-2-design-to-requirement-binding-closure")
