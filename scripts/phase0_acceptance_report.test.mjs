...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.phase, "phase-3b-implementation")
  assert.equal(report.evidenceScope, "phase-3b-model-switch-implementation-local")
  assert.match(report.claimBoundary, /Model Switching Within One Provider candidate lifecycle/u)
  assert.match(report.claimBoundary, /exact current Product, Initiative, Proposed Change Preview, Isolated Staging Workspace, Provider Switch Implementation, Controlled Codex and Controlled Claude candidate bindings/u)
  assert.match(report.claimBoundary, /does not establish target-model availability, capability refresh, actual provider execution or model transition/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3b-model-switch-implementation-closure")
