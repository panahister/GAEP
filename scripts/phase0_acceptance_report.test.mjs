...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-route-screen-component-mapping-local")
  assert.match(report.claimBoundary, /one exact route, screen, state, or component subject per current source subject/u)
  assert.match(report.claimBoundary, /Route, Screen, and Component mappings remain governed candidates only/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-route-screen-component-mapping-closure")
