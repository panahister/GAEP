...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-2-finalized-figma-snapshot-import-local")
  assert.match(report.claimBoundary, /exact Finalized Figma Snapshot Import review-candidate lifecycle/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-2-finalized-figma-snapshot-import-closure")
