...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-dependency-mapping-local")
  assert.match(report.claimBoundary, /complete current Implementation Unit node catalog/u)
  assert.match(report.claimBoundary, /Dependency maps and critical paths remain governed candidates only/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-dependency-mapping-closure")
