...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3b-controlled-codex-implementation-local")
  assert.match(report.claimBoundary, /Controlled Codex Implementation candidate lifecycle/u)
  assert.match(report.claimBoundary, /exact current Product, Initiative, Proposed Change Preview, Isolated Staging Workspace and portable Codex Agent Selection bindings/u)
  assert.match(report.claimBoundary, /does not establish actual provider execution, real stage creation/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3b-controlled-codex-implementation-closure")
