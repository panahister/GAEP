...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3b-approved-figma-context-retrieval-local")
  assert.match(report.claimBoundary, /Approved Figma Context Retrieval candidate lifecycle/u)
  assert.match(report.claimBoundary, /exact current Product, Initiative, Design Applicability, Finalized Figma Snapshot Import, Human Design Approval, Design Baseline/u)
  assert.match(report.claimBoundary, /does not establish actual Figma access, remote fetch, content materialization or transfer/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3b-approved-figma-context-retrieval-closure")
