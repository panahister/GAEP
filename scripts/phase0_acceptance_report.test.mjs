...
function passingTestEvidence() {
  ...
  return [
    ...
    },
    {
      ...
      result: "pass",
      summary: { filesPassed: 51, testsPassed: 540, testsSkipped: 1, testsFailed: 0 },
      outputDigest,
    },
    {
...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-2-design-personas-roles-local")
  assert.match(report.claimBoundary, /exact governed Design Personas and Roles candidate lifecycle/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-2-design-personas-roles-closure")
