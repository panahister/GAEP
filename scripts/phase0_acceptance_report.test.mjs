...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-2-realistic-figma-loop-example-local")
  assert.match(report.claimBoundary, /12 ordered candidate stages/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-2-realistic-figma-loop-example-closure")
