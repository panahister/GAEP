...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-test-methodology-local")
  assert.match(report.claimBoundary, /one exact scope per current Implementation Unit/u)
  assert.match(report.claimBoundary, /Test Methodologies remain governed candidates only/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-test-methodology-closure")
