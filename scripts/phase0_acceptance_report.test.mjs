...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-boilerplate-selection-binding-local")
  assert.match(report.claimBoundary, /one evidence-backed decision per current Implementation Unit/u)
  assert.match(report.claimBoundary, /Boilerplate selection and binding decisions remain governed candidates only/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-boilerplate-selection-binding-closure")
