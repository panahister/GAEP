...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3b-provider-switch-implementation-local")
  assert.match(report.claimBoundary, /Provider Switching During Implementation candidate lifecycle/u)
  assert.match(report.claimBoundary, /exact current Product, Initiative, Proposed Change Preview, Isolated Staging Workspace, Controlled Codex and Controlled Claude Implementation candidate bindings/u)
  assert.match(report.claimBoundary, /does not establish actual provider execution or transition, handoff recording, stage-ownership transfer, resume/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3b-provider-switch-implementation-closure")
