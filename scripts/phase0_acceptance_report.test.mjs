...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-figma-to-boilerplate-mapping-local")
  assert.match(report.claimBoundary, /one exact mapping subject per current Design-to-Requirement Binding/u)
  assert.match(report.claimBoundary, /Figma-to-Boilerplate mappings remain governed candidates only/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-figma-to-boilerplate-mapping-closure")
