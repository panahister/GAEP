...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-high-level-design-local")
  assert.match(report.claimBoundary, /all 15 exact current architecture/u)
  assert.match(report.claimBoundary, /High-Level Designs remain governed candidates only/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-high-level-design-closure")
