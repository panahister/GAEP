...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-implementation-unit-model-local")
  assert.match(report.claimBoundary, /complete current MVP Story and Task membership/u)
  assert.match(report.claimBoundary, /Implementation units remain governed candidates only/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-implementation-unit-model-closure")
