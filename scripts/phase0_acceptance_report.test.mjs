...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3b-controlled-claude-implementation-local")
  assert.match(report.claimBoundary, /Controlled Claude Implementation candidate lifecycle/u)
  assert.match(report.claimBoundary, /exact current Product, Initiative, Proposed Change Preview, Isolated Staging Workspace and portable Claude Agent Selection bindings/u)
  assert.match(report.claimBoundary, /does not establish a supported Claude runtime, authentication, effective administrator policy/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3b-controlled-claude-implementation-closure")
