...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-boilerplate-registry-local")
  assert.match(report.claimBoundary, /bounded candidate organizational boilerplate entries/u)
  assert.match(report.claimBoundary, /Boilerplate entries remain governed candidates only/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-boilerplate-registry-closure")
