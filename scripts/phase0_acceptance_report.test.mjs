...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-prioritization-model-local")
  assert.match(report.claimBoundary, /deterministic weighted-sum-v1 scoring/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-prioritization-model-closure")
