...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-test-inventory-local")
  assert.match(report.claimBoundary, /immutable revision history and test identities/u)
  assert.match(report.claimBoundary, /Test Inventories remain governed candidates only/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-test-inventory-closure")
