...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-definition-of-done-local")
  assert.match(report.claimBoundary, /versioned completion policy/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-definition-of-done-closure")
