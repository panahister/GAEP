...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-definition-of-ready-local")
  assert.match(report.claimBoundary, /versioned prerequisite policy/u)
  assert.match(report.claimBoundary, /candidate pass remains an evaluation result only/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-definition-of-ready-closure")
