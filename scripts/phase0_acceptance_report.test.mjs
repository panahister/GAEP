...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-boilerplate-compatibility-validation-local")
  assert.match(report.claimBoundary, /one exact validation subject per selected Boilerplate Selection and Binding decision/u)
  assert.match(report.claimBoundary, /Compatibility validations remain governed candidates only/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-boilerplate-compatibility-validation-closure")
