...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-low-level-design-local")
  assert.match(report.claimBoundary, /exact current High-Level Design plus all 15 prior/u)
  assert.match(report.claimBoundary, /Low-Level Designs remain governed candidates only/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-low-level-design-closure")
