...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-backlog-hierarchy-local")
  assert.match(report.claimBoundary, /strict Epic-to-Feature-to-Story-to-Task topology/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-backlog-hierarchy-closure")
