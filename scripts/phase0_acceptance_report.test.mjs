...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3b-changed-unit-inventory-local")
  assert.match(report.claimBoundary, /Changed Unit Inventory candidate lifecycle/u)
  assert.match(report.claimBoundary, /eight governed dependency bindings/u)
  assert.match(report.claimBoundary, /repository-relative path and change-kind candidates/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3b-changed-unit-inventory-closure")
