...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-technology-profile-local")
  assert.match(report.claimBoundary, /complete current Implementation Unit profile coverage/u)
  assert.match(report.claimBoundary, /Technology Profiles remain governed candidates only/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-technology-profile-closure")
