...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-2-ux-figma-dashboard-local")
  assert.match(report.claimBoundary, /exact derived Phase 2 UX\/Figma dashboard/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-2-ux-figma-dashboard-closure")
