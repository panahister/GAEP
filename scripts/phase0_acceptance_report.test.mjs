...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3b-isolated-staging-workspace-local")
  assert.match(report.claimBoundary, /Isolated Staging Workspace candidate lifecycle/u)
  assert.match(report.claimBoundary, /exact current Product, Initiative and Proposed Change Preview binding/u)
  assert.match(report.claimBoundary, /does not establish real stage existence/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3b-isolated-staging-workspace-closure")
