...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3b-proposed-change-preview-local")
  assert.match(report.claimBoundary, /Proposed Change Preview candidate lifecycle/u)
  assert.match(report.claimBoundary, /exact current Product, Initiative and Changed Unit Inventory binding/u)
  assert.match(report.claimBoundary, /without file or diff content/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3b-proposed-change-preview-closure")
