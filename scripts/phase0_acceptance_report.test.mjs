...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-design-to-code-binding-registry-local")
  assert.match(report.claimBoundary, /one exact binding subject per current Figma-to-Boilerplate Mapping subject/u)
  assert.match(report.claimBoundary, /Design-to-Code bindings remain governed candidates only/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-design-to-code-binding-registry-closure")
