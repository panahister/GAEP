...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3a-mvp-slice-definition-local")
  assert.match(report.claimBoundary, /exact one-time assignment of every MVP Story and Task/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3a-mvp-slice-definition-closure")
