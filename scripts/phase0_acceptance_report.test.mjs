...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-2-responsive-multi-platform-targets-local")
  assert.match(report.claimBoundary, /exact governed Responsive and Multi-Platform Targets candidate lifecycle/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-2-responsive-multi-platform-targets-closure")
