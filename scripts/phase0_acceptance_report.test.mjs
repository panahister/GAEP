...
test("binds exact Phase 0 package, conformance, provider, host, example, test and gap truth", async () => {
  ...
  assert.equal(report.evidenceScope, "phase-3b-controlled-design-to-code-generation-local")
  assert.match(report.claimBoundary, /Controlled Design-to-Code Generation offline plan candidate lifecycle/u)
  assert.match(report.claimBoundary, /exact current Product, Initiative, Approved Figma Context Retrieval, Design Baseline/u)
  assert.match(report.claimBoundary, /does not establish actual Figma access, protected-context materialization or transfer/u)
  ...
  assert.equal(report.knownGaps.at(-1).id, "phase-3b-controlled-design-to-code-generation-closure")
