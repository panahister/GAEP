...
describe("Design Applicability service", () => {
  ...
  it("persists, assesses, projects, and revises exact immutable candidate guidance without authority", async () => {
    ...
    expect(bundle.manifest.members.map((member) => member.path)).toEqual(expect.arrayContaining([
      `design-applicability/${candidate.id}.json`,
      `design-applicability-history/design-applicability-${candidate.id}-r1.json`,
      `design-applicability-history/design-applicability-${candidate.id}-r2.json`,
    ]))
