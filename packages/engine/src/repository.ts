...
export class GaepRepository {
  ...
  private async listGovernedPaths(): Promise<string[]> {
    ...
    for (const [directory, pattern] of [
      ...
      ["dependency-mappings", /^[0-9a-f-]+\.json$/i],
      ...
      ["information-architecture-models", /^[0-9a-f-]+\.json$/i],
      ...
      ["responsive-multi-platform-targets", /^[0-9a-f-]+\.json$/i],
      ...
      ["design-drift-detections", /^[0-9a-f-]+\.json$/i],
      ["design-drift-detection-history", /^design-drift-detection-[0-9a-f-]+-r[1-9][0-9]*\.json$/i],
  ...
  private async readGovernedRecord(relativePath: string): Promise<unknown> {
    ...
    if (/^dependency-mappings\/[0-9a-f-]+\.json$/i.test(relativePath) ||
        /^dependency-mapping-history\/dependency-mapping-[0-9a-f-]+-r[1-9][0-9]*\.json$/i.test(relativePath)) {
      return this.readJsonUnlocked(path, dependencyMappingSchema)
    }
    ...
    if (/^design-to-code-binding-registries\/[0-9a-f-]+\.json$/i.test(relativePath) ||
        /^design-to-code-binding-registry-history\/design-to-code-binding-registry-[0-9a-f-]+-r[1-9][0-9]*\.json$/i.test(relativePath)) {
      return this.readJsonUnlocked(path, designToCodeBindingRegistrySchema)
    }
    ...
    if (/^information-architecture-models\/[0-9a-f-]+\.json$/i.test(relativePath) ||
        /^information-architecture-model-history\/information-architecture-[0-9a-f-]+-r[1-9][0-9]*\.json$/i.test(relativePath)) {
      return this.readJsonUnlocked(path, informationArchitectureModelSchema)
    }
    ...
    if (/^responsive-multi-platform-targets\/[0-9a-f-]+\.json$/i.test(relativePath) ||
        /^responsive-multi-platform-targets-history\/responsive-multi-platform-targets-[0-9a-f-]+-r[1-9][0-9]*\.json$/i.test(relativePath)) {
      return this.readJsonUnlocked(path, responsiveMultiPlatformTargetsSchema)
    }
    ...
    if (/^design-drift-detections\/[0-9a-f-]+\.json$/i.test(relativePath) ||
        /^design-drift-detection-history\/design-drift-detection-[0-9a-f-]+-r[1-9][0-9]*\.json$/i.test(relativePath)) {
      return this.readJsonUnlocked(path, designDriftDetectionSchema)
    }
