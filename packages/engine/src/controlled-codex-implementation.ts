...
import {
  controlledCodexImplementationInputSchema, controlledCodexImplementationProjectionSchema,
  controlledCodexImplementationSchema, controlledCodexImplementationStatusSchema,
  type AgentSelectionState, type BusinessContextBinding, type ControlledCodexImplementation,
  type ControlledCodexImplementationInput, type ControlledCodexImplementationProjection,
  type ControlledCodexImplementationStatus, type Initiative, type Product, type ProposedChangePreview,
...
export class ControlledCodexImplementationService {
  constructor(
  ...
  async create(inputValue: ControlledCodexImplementationInput, actorId: string): Promise<ControlledCodexImplementation> {
    const input = controlledCodexImplementationInputSchema.parse(inputValue)
    return this.repository.withLock(async () => {
      ...
      if (await this.readCurrent(initiative.id)) throw new Error("An Initiative can have only one current Controlled Codex Implementation candidate")
      const now = new Date().toISOString()
      const record = controlledCodexImplementationSchema.parse({
        schemaVersion: 1, kind: "controlled-codex-implementation-candidate", id: randomUUID(), productId: product.id,
      ...
      await this.commitVersionedRecord(record, "controlled-codex-implementation.created", actorId)
      return record
    })
  }

  async revise(id: string, expectedRevision: number, inputValue: ControlledCodexImplementationInput, actorId: string): Promise<ControlledCodexImplementation> {
    const input = controlledCodexImplementationInputSchema.parse(inputValue)
    return this.repository.withLock(async () => {
      await this.assertIntegrity()
      const current = await this.read(id)
      if (current.revision !== expectedRevision) throw new Error("Controlled Codex Implementation revision changed before update")
      if (current.initiativeId !== input.initiativeId) throw new Error("Controlled Codex Implementation Initiative cannot change")
      const { product, initiative } = await this.requireMutableInitiative(input.initiativeId)
      this.validateContext(input.context, product, initiative)
      const dependencies = await this.requireExactDependencies(input)
      this.validateCandidate(input, dependencies.preview, dependencies.staging, dependencies.selection, product, initiative)
      ...
      await this.commitVersionedRecord(record, "controlled-codex-implementation.revised", actorId)
      return record
    })
  }

  async read(id: string): Promise<ControlledCodexImplementation> {
    return this.repository.readJson(this.currentPath(this.requireUuid(id, "Controlled Codex Implementation ID")), controlledCodexImplementationSchema)
  }

  async readCurrent(initiativeId: string): Promise<ControlledCodexImplementation | undefined> {
    const targetId = this.requireUuid(initiativeId, "Initiative ID")
    const matches = (await this.listRecords("controlled-codex-implementations", currentRecordPattern, controlledCodexImplementationSchema))
      .filter((record) => record.initiativeId === targetId)
    if (matches.length > 1) throw new Error("Initiative has more than one current Controlled Codex Implementation candidate")
    return matches[0]
  }

  async readRevision(id: string, revision: number): Promise<ControlledCodexImplementation> {
    if (!Number.isInteger(revision) || revision < 1) throw new Error("Controlled Codex Implementation history revision must be positive")
    const recordId = this.requireUuid(id, "Controlled Codex Implementation ID")
    const record = await this.repository.readJson(this.historyPath(recordId, revision), controlledCodexImplementationSchema)
    if (record.id !== recordId || record.revision !== revision) throw new Error("Controlled Codex Implementation history identity or revision does not match")
    return record
  }

  async listHistory(id: string): Promise<ControlledCodexImplementation[]> {
    const recordId = this.requireUuid(id, "Controlled Codex Implementation ID")
    const records = await this.listRecords("controlled-codex-implementation-history",
      new RegExp(`^controlled-codex-implementation-${recordId}-r[1-9][0-9]*\\.json$`, "iu"), controlledCodexImplementationSchema)
    const ascending = [...records].sort((left, right) => left.revision - right.revision)
    for (const [index, record] of ascending.entries()) {
      if (record.id !== recordId || record.revision !== index + 1 ||
          (index === 0 && record.predecessorDigest !== undefined) ||
          (index > 0 && record.predecessorDigest !== canonicalDigest(ascending[index - 1]))) {
        throw new Error("Controlled Codex Implementation history is incomplete or has an invalid predecessor chain")
      }
    }
    return ascending.reverse()
  }

  async assess(initiativeId: string): Promise<ControlledCodexImplementationStatus> {
    const targetId = this.requireUuid(initiativeId, "Initiative ID")
    ...
    const providerGapCount = candidate && (candidate.provider.adapterId !== "gaep.codex-cli" || candidate.provider.agentId !== "codex-cli" ||
    ...
    if (!candidate) reasons.push("No versioned Controlled Codex Implementation candidate exists for this Initiative")
    ...
    if (providerGapCount) reasons.push("The provider binding is not the exact selected managed Codex identity")
    ...
    return controlledCodexImplementationStatusSchema.parse({
      schemaVersion: 1, kind: "controlled-codex-implementation-status", productId: product.id, productRevision: revisionOf(product),
  ...
  async project(initiativeId: string): Promise<ControlledCodexImplementationProjection> {
    const targetId = this.requireUuid(initiativeId, "Initiative ID")
    const [product, initiative, status, candidate] = await Promise.all([
      this.readProduct(), this.readInitiative(targetId), this.assess(targetId), this.readCurrent(targetId),
    ])
    const withoutDigest = {
      schemaVersion: 1 as const, kind: "controlled-codex-implementation-projection" as const,
      ...
      ...(candidate ? { candidate: {
        ...
        provider: candidate.provider, plan: candidate.plan, lifecycle: candidate.lifecycle, resourceScopes: candidate.resourceScopes, permissions: candidate.permissions,
        ...
        bindingReceiptDigest: candidate.bindingReceiptDigest, providerReceiptDigest: candidate.providerReceiptDigest, scopeReceiptDigest: candidate.scopeReceiptDigest,
    ...
    return controlledCodexImplementationProjectionSchema.parse({ ...withoutDigest, snapshotDigest: canonicalDigest(withoutDigest) })
  }

  async healthIssues(): Promise<WorkspaceHealthIssue[]> {
    const issues: WorkspaceHealthIssue[] = []
    for (const candidate of await this.listRecords("controlled-codex-implementations", currentRecordPattern, controlledCodexImplementationSchema)) {
      try {
        ...
        if ((await this.assess(candidate.initiativeId)).state === "attention-required") issues.push({ code: "controlled-codex-implementation.review-required", severity: "warning",
          message: `Initiative ${candidate.initiativeId} has a stale, incomplete, or unresolved Controlled Codex Implementation candidate.`,
        ...
        issues.push({ code: "controlled-codex-implementation.invalid", severity: "error", message: `Controlled Codex Implementation ${candidate.id}: ${error instanceof Error ? error.message : "validation failed"}`,
  ...
  private async requireExactDependencies(input: ControlledCodexImplementationInput) {
    ...
    if (!preview || !sameReference(input.proposedChangePreview, preview)) throw new Error("Controlled Codex Implementation must reference the exact current Proposed Change Preview")
    if (!staging || !sameReference(input.stagingWorkspace, staging)) throw new Error("Controlled Codex Implementation must reference the exact current Isolated Staging Workspace")
    if (selectionState.status !== "selected" || canonicalDigest(input.selection) !== canonicalDigest(selectionState.selection)) throw new Error("Controlled Codex Implementation must bind the exact current portable Agent Selection")
    return { preview, staging, selection: selectionState.selection }
  }

  private validateCandidate(input: ControlledCodexImplementationInput, preview: ProposedChangePreview, staging: StagingWorkspace,
    selection: ControlledCodexImplementationInput["selection"], product: Product, initiative: Initiative): void {
    if (input.provider.adapterId !== "gaep.codex-cli" || input.provider.agentId !== "codex-cli") throw new Error("Controlled implementation requires the managed Codex adapter and agent identity")
    if (canonicalDigest(input.provider) !== canonicalDigest(this.expectedProvider(selection, input.provider.runtimeVersion))) throw new Error("Controlled Codex provider binding must match the exact Agent Selection")
    ...
    if (!this.permissionsCover(input)) throw new Error("Controlled tool permissions must uniquely and explicitly cover every path capability and scope")
    ...
    if (canonicalDigest(input.context) !== canonicalDigest(this.exactContext(product, initiative))) throw new Error("Controlled Codex Implementation context changed")
  }

  private expectedProvider(selection: ControlledCodexImplementationInput["selection"], runtimeVersion?: string) {
  ...
  private permissionsCover(input: Pick<ControlledCodexImplementationInput, "permissions" | "units">): boolean {
    const permissions = new Map(input.permissions.map((permission) => [permission.capability, permission]))
    if (permissions.size !== input.permissions.length) return false
    return input.units.every((unit) => unit.paths.every((path) => path.toolCapabilities.every((capability) => {
      const permission = permissions.get(capability)
      return permission !== undefined && permission.mode !== "deny" && permission.scope.includes(path.pathCandidate)
    })))
  }
  private expectedPathEffectReceipt(path: ControlledCodexImplementationInput["units"][number]["paths"][number]): string {
  ...
  private expectedPlanReceipt(input: Pick<ControlledCodexImplementationInput, "plan" | "provider" | "proposedChangePreview" | "stagingWorkspace">): string {
    return canonicalDigest({ strategy: input.plan.strategy, planKey: input.plan.planKey, workflowPlan: input.plan.workflowPlan,
      provider: input.provider, proposedChangePreview: input.proposedChangePreview, stagingWorkspace: input.stagingWorkspace })
  }
  private expectedStagedEffectReceipt(input: Pick<ControlledCodexImplementationInput, "units" | "resourceScopes" | "permissions">): string {
  ...
  private expectedRecoveryCheckpoint(input: Pick<ControlledCodexImplementationInput, "plan" | "proposedChangePreview" | "stagingWorkspace" | "resourceScopes">): string {
  ...
  private expectedRecoveryReceipt(input: Pick<ControlledCodexImplementationInput, "recoveryJournal">): string {
  ...
  private composeDigests(input: ControlledCodexImplementationInput) {
    ...
    const assessmentReceiptDigest = canonicalDigest({ bindingReceiptDigest, providerReceiptDigest, scopeReceiptDigest, planReceiptDigestVerified,
    ...
    return { bindingReceiptDigest, providerReceiptDigest, scopeReceiptDigest, planReceiptDigestVerified, stagedEffectReceiptDigestVerified,
  ...
  private validateContext(binding: BusinessContextBinding, product: Product, initiative: Initiative): void {
    if (initiative.productId !== product.id || canonicalDigest(binding) !== canonicalDigest(this.exactContext(product, initiative))) throw new Error("Controlled Codex Implementation must bind exact current Product and Initiative revisions and digests")
  }
  ...
  private async requireMutableInitiative(initiativeId: string): Promise<{ product: Product; initiative: Initiative }> {
    ...
    if (initiative.productId !== product.id) throw new Error("Initiative does not target the current Product")
    if (["completed", "cancelled"].includes(initiative.state)) throw new Error(`Terminal Initiative ${initiative.state} Controlled Codex Implementation is immutable`)
    return { product, initiative }
  }
  private async commitVersionedRecord(record: ControlledCodexImplementation, eventType: string, actorId: string): Promise<void> {
    await this.repository.commitMutation({ writes: [this.governed(this.currentPath(record.id), record, controlledCodexImplementationSchema),
      this.governed(this.historyPath(record.id, record.revision), record, controlledCodexImplementationSchema)],
    audit: { eventType, actor: { kind: "human", id: actorId }, subjectId: record.id, payload: {
      ...
      bindingReceiptDigest: record.bindingReceiptDigest, providerReceiptDigest: record.providerReceiptDigest, scopeReceiptDigest: record.scopeReceiptDigest,
  ...
  private currentPath(id: string): string { return this.repository.resolve("controlled-codex-implementations", `${id}.json`) }
  private historyPath(id: string, revision: number): string { return this.repository.resolve("controlled-codex-implementation-history", `controlled-codex-implementation-${id}-r${revision}.json`) }
  ...
  private async listRecords<T>(directory: string, pattern: RegExp, schema: ZodType<T>): Promise<T[]> {
    ...
    if (names.length > inventoryLimit) throw new Error(`Controlled Codex Implementation directory ${directory} exceeds the safety limit`)
