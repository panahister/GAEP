...
import { controlledCodexImplementationInputSchema, type ControlledCodexImplementationInput } from "@gaep/contracts"
...
function fixture() {
  ...
  const evidence = { kind: "evidence" as const, sourceId: "controlled-codex-inspection", revision: 1, digest, evidenceState: "human-reviewed" as const }
  ...
  const selection = { schemaVersion: 2 as const, adapterId: "gaep.codex-cli", agentId: "codex-cli", modelId: "configured-codex-model",
  ...
  const pathBase = { id: randomUUID(), ordinal: 1, proposedPreviewPathId: previewPathId, stagingPathId: stagingPath.id,
    pathCandidate: previewPath.pathCandidate, resourceScopeId: "scope-unit-a", toolCapabilities: ["workspace-write"], expectedEffect: "reversible-change" as const,
  ...
  const permissions = [{ capability: "workspace-write", mode: "ask" as const, scope: resourceScopes }]
  const planBase = { strategy: "managed-codex-staged-candidate" as const, planKey: "controlled-codex-plan-1",
    workflowPlan: { recordId: randomUUID(), revision: 1, digest } }
  const planReceiptDigest = canonicalDigest({ ...planBase, provider, proposedChangePreview, stagingWorkspace })
  ...
  const recoveryBase = { strategy: "write-ahead-journal-candidate" as const, journalKey: "controlled-codex-journal-1",
  ...
  const input: ControlledCodexImplementationInput = {
    ...
    informationClassification: "internal", title: "Controlled Codex implementation candidate", proposedChangePreview, stagingWorkspace,
    selection, provider, plan, permissions, resourceScopes, units: [unit],
  ...
  const service = new ControlledCodexImplementationService(repository as unknown as GaepRepository, async () => product as never,
...
describe("Controlled Codex Implementation lifecycle", () => {
  it("persists immutable exact-bound candidates while all authoritative actions remain unperformed", async () => {
    ...
    const revised = await service.revise(created.id, 1, { ...input, title: "Reviewed Controlled Codex candidate" }, "implementation-author")
    ...
    expect(projection.candidate).toMatchObject({ provider: { adapterId: "gaep.codex-cli", agentId: "codex-cli" }, unitCount: 1, pathCount: 1 })
  ...
  it("fails closed on forged scope, provider, receipts, and secret-shaped metadata", async () => {
    ...
    await expect(service.create({ ...input, provider: { ...input.provider, adapterId: "gaep.claude-code-cli" } }, "implementation-author")).rejects.toThrow(/managed Codex/iu)
    ...
    expect(controlledCodexImplementationInputSchema.safeParse({ ...input, limitations: ["api_key=abcdefghijklmnopqrstuvwxyz123456"] }).success).toBe(false)
  })
})
