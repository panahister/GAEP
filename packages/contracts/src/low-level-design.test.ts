...
import { lowLevelDesignInputSchema, lowLevelDesignStatusSchema } from "./low-level-design.js"
...
const evidence = (kind: "architecture" | "bounded-context" | "dependency" | "high-level-design" | "implementation-unit" | "risk" | "security-privacy" | "technology" | "test-inventory") => ({
...
function input() {
  ...
  return {
    ...
    informationClassification: "internal" as const, title: "Candidate Low-Level Design",
    implementationUnitId: ids[22]!, highLevelDesign: reference(17),
    ...
    elements: [
      { id: left, ordinal: 1, key: "checkout.module", kind: "module" as const, title: "Checkout Module",
        responsibility: "Coordinates checkout candidate interactions", boundedContextKeys: ["checkout"],
        highLevelDesignElementIds: [ids[18]!],
      ...
      { id: right, ordinal: 2, key: "payment.adapter", kind: "component" as const, title: "Payment Adapter",
        responsibility: "Represents a bounded payment adapter candidate", boundedContextKeys: ["checkout"],
        highLevelDesignElementIds: [ids[19]!],
    ...
    reviewState: "ready-for-human-review" as const, designTruthState: "not-established" as const,
    designCompletenessState: "not-established" as const, repositoryTruthState: "not-established" as const,
    sourceTruthState: "not-established" as const,
...
describe("Low-Level Design contracts", () => {
  it("accepts an exact attributed candidate and reconciled status", () => {
    const candidate = lowLevelDesignInputSchema.parse(input())
    expect(candidate.elements).toHaveLength(2)
    expect(lowLevelDesignStatusSchema.parse({
      schemaVersion: 1, kind: "low-level-design-status", productId: ids[28], productRevision: 1,
      initiativeId: ids[0], initiativeRevision: 1, implementationUnitId: candidate.implementationUnitId,
      candidate: reference(29), highLevelDesign: candidate.highLevelDesign, dependencyCount: 16,
      systemSolutionArchitecture: candidate.systemSolutionArchitecture, boundedContextModel: candidate.boundedContextModel,
      technologyProfile: candidate.technologyProfile, dependencyMapping: candidate.dependencyMapping,
      implementationUnitModel: candidate.implementationUnitModel, boilerplateRegistry: candidate.boilerplateRegistry,
      boilerplateSelectionBinding: candidate.boilerplateSelectionBinding,
      boilerplateCompatibilityValidation: candidate.boilerplateCompatibilityValidation,
      designBaseline: candidate.designBaseline, designToCodeBindingRegistry: candidate.designToCodeBindingRegistry,
      routeScreenComponentMapping: candidate.routeScreenComponentMapping, testMethodology: candidate.testMethodology,
      testInventory: candidate.testInventory, riskRegister: candidate.riskRegister,
      securityPrivacyAssessment: candidate.securityPrivacyAssessment,
      presentDependencyCount: 16, elementCount: 2, definedElementCount: 2, relationCount: 1,
  ...
  it("rejects duplicate elements, orphan relations, missing attribution, and secrets", () => {
    const value = input()
    expect(lowLevelDesignInputSchema.safeParse({ ...value, elements: [value.elements[0]!, value.elements[0]!] }).success).toBe(false)
    expect(lowLevelDesignInputSchema.safeParse({ ...value, relations: [{ ...value.relations[0]!, toElementId: ids[40]! }] }).success).toBe(false)
    expect(lowLevelDesignInputSchema.safeParse({ ...value, elements: [{ ...value.elements[0]!, designedBy: undefined }, value.elements[1]!] }).success).toBe(false)
    expect(lowLevelDesignInputSchema.safeParse({ ...value, limitations: ["apiKey=low-level-design-secret-value"] }).success).toBe(false)
  })
})
