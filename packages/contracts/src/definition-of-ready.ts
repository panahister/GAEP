...
function rejectSecrets<T extends z.ZodType>(schema: T): T {
  return schema.refine((value) => !containsSecretShapedValue(value), {
    message: "Portable Definition of Ready candidates cannot contain secret-shaped values",
  }) as unknown as T
}
...
export const definitionOfReadyPrerequisiteKindSchema = z.enum([
  "acceptance-criteria",
  "accountable-owner",
  "architecture",
  "dependency",
...
export const definitionOfReadyEvidenceReferenceSchema = z.object({
  kind: z.enum([
    "acceptance-criteria",
    "architecture",
    "decision",
...
export const definitionOfReadyPolicyEntrySchema = z.object({
  key: identifierSchema,
  kind: definitionOfReadyPrerequisiteKindSchema,
...
export const definitionOfReadyItemEvaluationSchema = z.object({
  id: z.string().uuid(),
  ...
  subjectLevel: backlogHierarchyLevelSchema.refine(
    ...
    "Definition of Ready subjects must be Story or Task nodes",
  ),
  ...
  evidenceReferences: z.array(definitionOfReadyEvidenceReferenceSchema).max(256)
    .refine((values) => unique(values.map((value) =>
      `${value.kind}:${value.recordId}:${value.revision}:${value.digest}`)), "Ready evaluation evidence references must be unique"),
  assessedBy: humanActorSchema,
  assessedAt: z.string().datetime(),
}).strict()

const definitionOfReadyInputBaseSchema = z.object({
  ...
  policyEntries: z.array(definitionOfReadyPolicyEntrySchema).min(1).max(1_024),
  itemEvaluations: z.array(definitionOfReadyItemEvaluationSchema).min(1).max(100_000),
  ...
  prerequisiteTruthState: z.literal("not-established"),
  ...
  phaseEntryState: z.literal("not-established"),
  ...
  if (!unique(policyKeys)) context.addIssue({ code: "custom", path: ["policyEntries"], message: "Definition of Ready policy keys must be unique" })
  if (!canonical(policyKeys)) context.addIssue({ code: "custom", path: ["policyEntries"], message: "Definition of Ready policy entries must use canonical key ordering" })
  ...
  if (!unique(identities) || !unique(pairs)) {
    context.addIssue({ code: "custom", path: ["itemEvaluations"], message: "Ready evaluation identities and subject-prerequisite pairs must be unique" })
  }
  for (const [index, evaluation] of candidate.itemEvaluations.entries()) {
    if (evaluation.ordinal !== index + 1) {
      context.addIssue({ code: "custom", path: ["itemEvaluations", index, "ordinal"], message: "Ready evaluations must use contiguous canonical ordinal ordering" })
    }
    const policy = policyByKey.get(evaluation.prerequisiteKey)
    if (!policy) {
      context.addIssue({ code: "custom", path: ["itemEvaluations", index, "prerequisiteKey"], message: "Ready evaluations must reference a declared policy prerequisite" })
      continue
    }
    ...
    if (evaluation.applicability === "required" && evaluation.assessmentState === "candidate-satisfied" &&
      ...
      context.addIssue({ code: "custom", path: ["itemEvaluations", index, "evidenceReferences"], message: "Candidate-satisfied required prerequisites must carry exact evidence when policy requires it" })
    }
  }
  if (candidate.reviewState === "ready-for-human-review" &&
      (candidate.unresolvedQuestions.length > 0 || candidate.itemEvaluations.some((evaluation) =>
        !["candidate-satisfied"].includes(evaluation.assessmentState)))) {
    context.addIssue({ code: "custom", path: ["reviewState"], message: "Review-ready Definition of Ready requires candidate-satisfied evaluations and no unresolved questions" })
  }
})

export const definitionOfReadyInputSchema = rejectSecrets(definitionOfReadyInputBaseSchema)

export const definitionOfReadySchema = definitionOfReadyInputSchema.safeExtend({
  schemaVersion: z.literal(1),
  kind: z.literal("definition-of-ready-candidate"),
  id: z.string().uuid(),
  ...
  gateBoundary: z.literal("a-passing-definition-of-ready-candidate-is-an-evaluation-result-not-admission-readiness-assignment-execution-or-implementation-permission"),
  authorityBoundary: z.literal("definition-of-ready-is-a-versioned-item-evaluation-candidate-and-does-not-establish-prerequisite-truth-criterion-validity-completeness-requirement-satisfaction-priority-commitment-approval-ready-done-exception-waiver-authority-phase-entry-implementation-readiness-assignment-execution-acceptance-or-action-authority"),
}).strict().superRefine((record, context) => {
  if ((record.revision === 1) !== (record.predecessorDigest === undefined)) {
    context.addIssue({ code: "custom", path: ["predecessorDigest"], message: "Only Definition of Ready revisions after revision one require an exact predecessor digest" })
  }
})

export const exactDefinitionOfReadyReferenceSchema = z.object({
  recordId: z.string().uuid(), revision: z.number().int().positive(), digest: digestSchema,
}).strict()

export const definitionOfReadyStatusSchema = z.object({
  schemaVersion: z.literal(1),
  kind: z.literal("definition-of-ready-status"),
  ...
  candidate: exactDefinitionOfReadyReferenceSchema.optional(),
  ...
  assessedAt: z.string().datetime(),
  gateBoundary: z.literal("a-passing-definition-of-ready-candidate-is-an-evaluation-result-not-admission-readiness-assignment-execution-or-implementation-permission"),
  authorityBoundary: z.literal("definition-of-ready-status-is-observational-and-does-not-establish-prerequisite-truth-criterion-validity-completeness-requirement-satisfaction-priority-commitment-approval-ready-done-exception-waiver-authority-phase-entry-implementation-readiness-assignment-execution-acceptance-or-action-authority"),
  ...
  if (status.candidateSatisfiedCount + status.notSatisfiedCount + status.exceptionCandidateCount +
    ...
    context.addIssue({ code: "custom", path: ["evaluationCount"], message: "Definition of Ready evaluation counts must reconcile" })
  }
  const gaps = status.notSatisfiedCount + status.exceptionCandidateCount + status.notAssessedCount +
    ...
    status.staleAcceptanceCriteriaCount + status.expiredCount + status.unresolvedQuestionCount
  if (status.result === "candidate-passed" &&
      (gaps > 0 || !status.candidate || !status.hierarchy || !status.mvpSliceDefinition ||
       !status.prioritizationModel || !status.acceptanceCriteria || status.reviewState !== "ready-for-human-review" ||
       status.reasons.length > 0 || status.evaluationCount !== status.expectedEvaluationCount)) {
  ...
  if (status.result === "attention-required" && status.reasons.length === 0) {
    context.addIssue({ code: "custom", path: ["reasons"], message: "Attention-required Definition of Ready status must expose reasons" })
  }
})

export const definitionOfReadyProjectionSchema = z.object({
  schemaVersion: z.literal(1),
  kind: z.literal("definition-of-ready-projection"),
  ...
  status: definitionOfReadyStatusSchema,
  candidate: z.object({
    id: z.string().uuid(), revision: z.number().int().positive(), digest: digestSchema,
  ...
  gateBoundary: z.literal("a-passing-definition-of-ready-candidate-is-an-evaluation-result-not-admission-readiness-assignment-execution-or-implementation-permission"),
  authorityBoundary: z.literal("definition-of-ready-projection-is-read-only-and-does-not-establish-prerequisite-truth-criterion-validity-completeness-requirement-satisfaction-priority-commitment-approval-ready-done-exception-waiver-authority-phase-entry-implementation-readiness-assignment-execution-acceptance-or-action-authority"),
  ...
  if (projection.product.id !== projection.status.productId || projection.product.revision !== projection.status.productRevision ||
    ...
    context.addIssue({ code: "custom", path: ["status"], message: "Definition of Ready projection must bind exact Product and Initiative revisions" })
  }
})

export type DefinitionOfReadyInput = z.infer<typeof definitionOfReadyInputSchema>
export type DefinitionOfReady = z.infer<typeof definitionOfReadySchema>
export type DefinitionOfReadyStatus = z.infer<typeof definitionOfReadyStatusSchema>
export type DefinitionOfReadyProjection = z.infer<typeof definitionOfReadyProjectionSchema>
export type ExactDefinitionOfReadyReference = z.infer<typeof exactDefinitionOfReadyReferenceSchema>
