...
function mvpSliceDefinitionProjection(hierarchy = backlogHierarchyProjection()): MvpSliceDefinitionProjection {
  const exactHierarchy = hierarchy.candidate!
  const status = {
    schemaVersion: 1 as const,
    kind: "mvp-slice-definition-status" as const,
    productId: product.id,
    productRevision: product.revision ?? 1,
    initiativeId: initiative.id,
    initiativeRevision: initiative.revision ?? 1,
    candidate: {
      recordId: "e5e5e5e5-e5e5-45e5-85e5-e5e5e5e5e5e5",
      revision: 2,
      digest: `sha256:${"c".repeat(64)}` as const,
    },
    ...
    invalidSliceCount: 1,
    unresolvedQuestionCount: 2,
    scopeCompletenessState: "not-assessed" as const,
    reviewState: "held" as const,
    state: "attention-required" as const,
    reasons: ["One or more MVP scope or Vertical Slice candidates require review"],
    assessedAt: "2026-07-30T10:20:00.000Z",
    authorityBoundary: "mvp-slice-definition-status-is-observational-and-does-not-establish-priority-commitment-scope-approval-acceptance-criteria-validity-ready-done-implementation-readiness-assignment-execution-or-action-authority" as const,
  }
  const body = {
    schemaVersion: 1 as const,
    ...
    candidate: {
      ...
      state: "candidate" as const,
      membershipDigest: `sha256:${"d".repeat(64)}` as const,
      hierarchyDigest: exactHierarchy.digest,
      scopeNodeCount: 24,
      mvpNodeCount: 16,
      laterNodeCount: 5,
      excludedNodeCount: 3,
    ...
    observedAt: status.assessedAt,
    privacyBoundary: "projection-contains-record-identities-scope-and-slice-counts-statuses-and-digests-only-not-slice-titles-rationales-objectives-criteria-scope-content-requirement-content-personal-data-secrets-credentials-or-machine-paths" as const,
    authorityBoundary: "mvp-slice-definition-projection-is-read-only-and-does-not-prioritize-commit-approve-scope-admit-assign-execute-or-authorize-implementation-or-action" as const,
  }
  return { ...body, snapshotDigest: canonicalDigest(body) }
}
...
function figmaToBoilerplateMappingProjection(
  designApplicability: DesignApplicabilityProjection,
  designSystemTokenContract: DesignSystemTokenContractProjection,
  responsiveMultiPlatformTargets: ResponsiveMultiPlatformTargetsProjection,
  ...
  boilerplateRegistry: BoilerplateRegistryProjection,
  boilerplateSelectionBinding: BoilerplateSelectionBindingProjection,
  ...
  const status = {
    ...
    candidate: { recordId: "f3f3f3f3-f3f3-43f3-83f3-f3f3f3f3f3f3", revision: 2, digest: `sha256:${"4".repeat(64)}` as const },
    designApplicability: exact(designApplicability.candidate!),
    designSystemTokenContract: exact(designSystemTokenContract.candidate!),
    responsiveMultiPlatformTargets: exact(responsiveMultiPlatformTargets.candidate!),
    ...
    designBaseline: exact(designBaseline.candidate!), implementationUnitModel: exact(units.candidate!),
    technologyProfile: exact(technologyProfile.candidate!), boilerplateRegistry: exact(boilerplateRegistry.candidate!),
    ...
    designBindingCount: 2, subjectCount: 2, mappedCandidateCount: 1, conflictCandidateCount: 1,
    unmappedCandidateCount: 0, notAssessedCount: 0, componentMappingCount: 1, tokenMappingCount: 1,
    layoutMappingCount: 0, responsiveBehaviorMappingCount: 0, platformTargetMappingCount: 0,
    missingSubjectCount: 0, invalidSubjectCount: 1, targetGapCount: 1, traceGapCount: 1,
    evidenceGapCount: 1, staleBindingCount: 0, staleDependencyCount: 0, invalidCandidateCount: 1,
  ...
  const body = {
    ...
    candidate: {
      ...
      state: "candidate" as const, mappingSubjectCatalogDigest: `sha256:${"5".repeat(64)}` as const,
      targetCatalogDigest: `sha256:${"6".repeat(64)}` as const,
      traceReceiptDigest: `sha256:${"7".repeat(64)}` as const,
      mappingReceiptDigest: `sha256:${"8".repeat(64)}` as const,
      assessmentReceiptDigest: `sha256:${"9".repeat(64)}` as const,
      subjectCount: 2, mappedCandidateCount: 1, conflictCandidateCount: 1,
      unmappedCandidateCount: 0, notAssessedCount: 0, reviewState: "held" as const,
...
function highLevelDesignProjection(dependencies: {
  system: SystemSolutionArchitectureProjection
  contexts: BoundedContextModelProjection
  technology: TechnologyProfileProjection
  dependencyMapping: DependencyMappingProjection
  units: ImplementationUnitModelProjection
  registry: BoilerplateRegistryProjection
  selection: BoilerplateSelectionBindingProjection
  compatibility: BoilerplateCompatibilityValidationProjection
  designBaseline: DesignBaselineProjection
  designBinding: DesignToCodeBindingRegistryProjection
  routeMapping: RouteScreenComponentMappingProjection
  methodology: TestMethodologyProjection
  inventory: TestInventoryProjection
  risks: RiskRegisterProjection
  securityPrivacy: SecurityPrivacyAssessmentProjection
  ...
  const status = {
    schemaVersion: 1 as const, kind: "high-level-design-status" as const,
    productId: product.id, productRevision: product.revision ?? 1,
    initiativeId: initiative.id, initiativeRevision: initiative.revision ?? 1,
    candidate: { recordId: "c6c6c6c6-c6c6-46c6-86c6-c6c6c6c6c6c6", revision: 2, digest: `sha256:${"1".repeat(64)}` as const },
    systemSolutionArchitecture: exact(dependencies.system.architecture!), boundedContextModel: exact(dependencies.contexts.model!),
    technologyProfile: exact(dependencies.technology.candidate!), dependencyMapping: exact(dependencies.dependencyMapping.candidate!),
    implementationUnitModel: exact(dependencies.units.candidate!), boilerplateRegistry: exact(dependencies.registry.candidate!),
    boilerplateSelectionBinding: exact(dependencies.selection.candidate!),
    boilerplateCompatibilityValidation: exact(dependencies.compatibility.candidate!),
    designBaseline: {
      ...exact(dependencies.designBaseline.candidate!),
      membershipDigest: dependencies.designBaseline.candidate!.membershipDigest,
      baselineLineageId: dependencies.designBaseline.candidate!.baselineLineageId,
      semanticVersion: dependencies.designBaseline.candidate!.semanticVersion,
    }, designToCodeBindingRegistry: exact(dependencies.designBinding.candidate!),
    routeScreenComponentMapping: exact(dependencies.routeMapping.candidate!), testMethodology: exact(dependencies.methodology.candidate!),
    testInventory: exact(dependencies.inventory.candidate!), riskRegister: exact(dependencies.risks.register!),
    securityPrivacyAssessment: exact(dependencies.securityPrivacy.assessment!),
    ...
    uncoveredUnitCount: 1, staleBindingCount: 0, staleDependencyCount: 0, invalidCandidateCount: 1,
    unresolvedQuestionCount: 2, reviewState: "held" as const, state: "attention-required" as const,
    reasons: ["One or more High-Level Design candidates require human review"], assessedAt: "2026-07-31T03:00:00.000Z",
  ...
  const body = {
    ...
    candidate: {
      id: status.candidate.recordId, revision: status.candidate.revision, digest: status.candidate.digest,
      state: "candidate" as const, structureReceiptDigest: `sha256:${"2".repeat(64)}` as const,
      dependencyReceiptDigest: `sha256:${"3".repeat(64)}` as const, traceReceiptDigest: `sha256:${"4".repeat(64)}` as const,
      coverageReceiptDigest: `sha256:${"5".repeat(64)}` as const, ownershipReceiptDigest: `sha256:${"6".repeat(64)}` as const,
      assessmentReceiptDigest: `sha256:${"7".repeat(64)}` as const, elementCount: 8, relationCount: 11,
      decisionCount: 4, reviewState: "held" as const, updatedAt: "2026-07-31T02:59:00.000Z",
    },
    observedAt: status.assessedAt,
...
describe("current-engine Product Studio data source", () => {
  ...
  it("projects exact privacy-safe MVP and Vertical Slice metadata on Delivery", async () => {
    const hierarchy = backlogHierarchyProjection()
    const projection = mvpSliceDefinitionProjection(hierarchy)
    const { source } = harness({ backlogHierarchyProjection: hierarchy, mvpSliceDefinitionProjection: projection })
    const snapshot = await source.readSnapshot("delivery")

    expect(isStudioSnapshot(snapshot)).toBe(true)
    expect(snapshot.page.kind === "delivery" && snapshot.page.mvpSliceDefinitions).toMatchObject({
      ...
      rows: [{
        ...
        cells: {
          ...
          revision: "2",
          membership: projection.candidate?.membershipDigest,
          hierarchy: hierarchy.candidate?.digest,
          scope: "24 nodes · 16 MVP · 5 later · 3 excluded",
          slices: "4 slices · 7 Stories · 9 Tasks · 3 dependencies",
          assessment: "attention-required · held · not-assessed",
          gaps: "1 unassigned MVP Stories or Tasks · 2 questions · 0 stale bindings · 0 stale hierarchies · 0 invalid scope entries · 1 invalid slices",
        },
      }],
    })
    expect(JSON.stringify(snapshot)).not.toMatch(
  ...
  it("projects exact privacy-safe Figma-to-Boilerplate Mapping metadata only across all 11 current dependencies", async () => {
    const designApplicability = designApplicabilityProjection()
    const designSystemTokenContract = designSystemTokenContractProjection()
    ...
    const projection = figmaToBoilerplateMappingProjection(
      designApplicability, designSystemTokenContract, responsiveTargets, finalizedSnapshot, designBinding,
    ...
    const options = {
      ...
      designSystemTokenContractProjection: designSystemTokenContract,
      ...
      figmaToBoilerplateMappingProjection: projection,
    }
    const { source } = harness(options)
    const snapshot = await source.readSnapshot("delivery")

    expect(isStudioSnapshot(snapshot)).toBe(true)
    expect(snapshot.page.kind === "delivery" && snapshot.page.figmaToBoilerplateMappings).toMatchObject({
      id: "figma-to-boilerplate-mapping",
      rows: [{
        id: projection.candidate?.id,
        cells: {
          initiative: initiative.id, revision: "2",
          subjects: projection.candidate?.mappingSubjectCatalogDigest,
          targets: projection.candidate?.targetCatalogDigest,
          traceReceipt: projection.candidate?.traceReceiptDigest,
          mappingReceipt: projection.candidate?.mappingReceiptDigest,
          assessmentReceipt: projection.candidate?.assessmentReceiptDigest,
          coverage: "2 design bindings · 2 mapping subjects",
          outcomes: "1 mapped candidates · 1 conflicts · 0 unmapped · 0 not assessed",
          kinds: "1 component · 1 token · 0 layout · 0 responsive · 0 platform-target",
          assessment: "attention-required · held",
          mappingGaps: "0 missing subjects · 1 invalid subjects · 1 target gaps · 1 trace gaps · 1 evidence gaps",
          staleGaps: "0 stale bindings · 0 stale dependencies · 1 invalid candidates · 2 questions",
          boundary: expect.stringContaining("no Figma content, design-item, binding, unit, profile"),
        },
      }],
    })
    expect(JSON.stringify(snapshot)).not.toMatch(
      /private figma|private target|private design item|private requirement|private reviewer|customer@example\.com|api_key/iu,
    )

    const hostileBody = { ...projection, status: {
      ...projection.status,
      designApplicability: { ...projection.status.designApplicability!, digest: `sha256:${"a".repeat(64)}` as const },
    } }
    ...
    const hostileSnapshot = await harness({ ...options, figmaToBoilerplateMappingProjection: hostile }).source.readSnapshot("delivery")
  ...
  it("projects exact privacy-safe Route, Screen, and Component Mapping metadata only across all 9 current dependencies", async () => {
    ...
    const system = systemSolutionArchitectureProjection()
    const contexts = boundedContextModelProjection()
    const highLevelDesign = highLevelDesignProjection({
      system, contexts, technology: technologyProfile, dependencyMapping, units, registry: boilerplateRegistry,
      selection: selectionBinding, compatibility: compatibilityValidation, designBaseline,
    ...
    expect(snapshot.page.kind === "delivery" && snapshot.page.highLevelDesigns).toMatchObject({
      id: "high-level-design",
      rows: [{
        id: highLevelDesign.candidate?.id,
        cells: {
          initiative: initiative.id, revision: "2",
          structureReceipt: highLevelDesign.candidate?.structureReceiptDigest,
          dependencyReceipt: highLevelDesign.candidate?.dependencyReceiptDigest,
          traceReceipt: highLevelDesign.candidate?.traceReceiptDigest,
          coverageReceipt: highLevelDesign.candidate?.coverageReceiptDigest,
          ownershipReceipt: highLevelDesign.candidate?.ownershipReceiptDigest,
          assessmentReceipt: highLevelDesign.candidate?.assessmentReceiptDigest,
          dependencies: "15/15 exact current candidates",
          structure: "6/8 elements · 8/11 relations · 3/4 decisions",
          views: "5 quality attributes · 3 deployment views",
          assessment: "attention-required · held",
          structuralGaps: "1 conflicts · 1 missing · 1 orphan relations",
          integrityGaps: "2 trace · 1 evidence · 1 ownership · 1 uncovered units",
          staleGaps: "0 stale bindings · 0 stale dependencies · 1 invalid candidates · 2 questions",
          boundary: expect.stringContaining("no design narratives, diagrams, interfaces, data flows, technologies, owners"),
        },
      }],
    })
