...
export interface CurrentStudioEngineReader {
  ...
  definitionOfDone?: {
    project(initiativeId: string): Promise<DefinitionOfDoneProjection>
  }
  implementationUnitModel?: {
    project(initiativeId: string): Promise<ImplementationUnitModelProjection>
  }
  dependencyMapping?: {
...
function humanDesignApprovalTable(state: ObservedStudioState): StudioTableSnapshot {
  const rows = [...state.humanDesignApprovalProjections.values()].flatMap((projection) => {
    const record = projection.candidate
    if (!record) return []
    const status = projection.status
    return [{
      ...
      cells: {
        ...
        evidence: `definition ${record.decisionDefinitionDigest} · receipt ${record.decisionReceiptDigest}`,
        decision: record.decisionKind
          ? `${record.decisionKind} · ${record.decisionLifecycleState} · ${record.decisionDigest}`
          : "not-recorded",
        result: `${status.candidateResult} · ${status.state} · ${status.reviewState}`,
        prerequisites: `${status.completePrerequisiteCount}/${status.prerequisiteCount} complete`,
        governance: `approver authority ${status.approverAuthorityState} · separation enforcement ${status.separationOfDutiesEnforcementState}`,
        gaps: `${status.expiredDecisionCount} expired · ${status.revokedDecisionCount} revoked · ${status.unresolvedQuestionCount} questions · ${status.staleBindingCount} stale bindings · ${status.staleSourceReferenceCount} stale Source references`,
        boundary: "Candidate identities, exact prerequisite and finalized-snapshot bindings, scope and receipt digests, decision kind, lifecycle, counts, and recorded states only; no design, decision rationale, condition, evidence, Source, human-attribution, personal, secret, credential, or permission content. This view does not verify approver authority, enforce separation of duties, establish design approval, baseline, readiness, or phase entry, call Figma, request credentials, grant permissions, execute imports or writes, or grant implementation or action authority.",
      },
      state: status.state,
      actions: [],
    }]
  ...
  return {
    ...
    title: "Human Design Approval Decision Candidate",
    columns: [
      { key: "initiative", label: "Initiative", identifier: true },
      { key: "record", label: "Candidate" },
      { key: "revision", label: "Revision" },
      { key: "digest", label: "Exact digest" },
      { key: "membership", label: "Membership digest" },
      { key: "subject", label: "Exact approval subject" },
      { key: "scope", label: "Approval scope" },
      { key: "evidence", label: "Decision receipts" },
      { key: "decision", label: "Recorded decision" },
      { key: "result", label: "Candidate result" },
      { key: "prerequisites", label: "Prerequisite state" },
    ...
    ...(rows.length === 0 ? {
      emptyState: emptySurface(
        "No Human Design Approval candidate",
        "Record an exact attributable design-decision candidate through the governed engine workflow. This view does not verify approver authority, enforce separation of duties, establish design approval, baseline, readiness, or phase entry, access Figma, execute imports or writes, or grant implementation or action authority.",
      ),
    } : {}),
  }
}
...
function definitionOfDoneTable(state: ObservedStudioState): StudioTableSnapshot {
  const rows = [...state.definitionOfDoneProjections.values()].flatMap((projection) => {
    const record = projection.candidate
    if (!record) return []
    const status = projection.status
    return [{
      ...
      cells: {
        ...
        boundary: "Candidate identities, counts, statuses, validity time, and subject, policy, evaluation, receipt, and snapshot digests only; no rules, rationales, evidence identities, assessor identities, personal data, evidence truth, test success, quality or Requirement satisfaction, Acceptance Criteria satisfaction, approval, ready or done, exception or waiver authority, implementation completeness, merge, release or deployment readiness, assignment, execution, acceptance, or action authority. A candidate pass is not completion, approval, merge, release, deployment, or action permission.",
      },
      state: status.result,
      actions: [],
    }]
  ...
  return {
    ...
    title: "Governed Definition of Done Candidate",
    columns: [
      { key: "initiative", label: "Initiative", identifier: true },
      { key: "record", label: "Candidate" },
      { key: "revision", label: "Revision" },
    ...
    ...(rows.length === 0 ? {
      emptyState: emptySurface(
        "No governed Definition of Done candidate",
        "Create the candidate through the governed engine workflow after exact Backlog Hierarchy, MVP and Vertical Slice, Prioritization Model, Acceptance Criteria, and Definition of Ready candidates exist. This view does not infer completion, acceptance, approval, merge, release, deployment, or action authority.",
      ),
    } : {}),
  }
}

function implementationUnitModelTable(state: ObservedStudioState): StudioTableSnapshot {
  const rows = [...state.implementationUnitModelProjections.values()].flatMap((projection) => {
    const record = projection.candidate
    if (!record) return []
    const status = projection.status
    return [{
      ...
      cells: {
        ...
        digest: record.digest,
        membership: record.membershipDigest,
        placement: record.placementDigest,
        ...
        dependencies: `${status.dependencyEdgeCount} dependency edges · ${status.candidateAssessedBlastRadiusCount} blast radii candidate-assessed · ${status.notAssessedBlastRadiusCount} not assessed`,
        assessment: `${status.state} · ${status.reviewState}`,
        ...
        boundary: "Candidate identities, counts, statuses, and membership, placement, assessment-receipt, and snapshot digests only; no unit titles, boundaries, Story, Task, or Requirement identities, repository keys, module paths, owner identities, evidence, rationales, personal data, repository truth, owner appointment, dependency or impact completeness, implementation readiness or completeness, assignment, execution, approval, acceptance, merge, release, deployment, or action authority.",
      },
      state: status.state,
      actions: [],
    }]
  ...
  return {
    ...
    title: "Governed Implementation Unit Model Candidate",
    columns: [
      { key: "initiative", label: "Initiative", identifier: true },
      { key: "record", label: "Candidate" },
      { key: "revision", label: "Revision" },
      { key: "digest", label: "Exact digest" },
      { key: "membership", label: "Membership digest" },
      { key: "placement", label: "Placement digest" },
      { key: "assessmentReceipt", label: "Assessment receipt" },
      { key: "coverage", label: "Privacy-safe coverage" },
      { key: "dependencies", label: "Dependency and impact candidates" },
      { key: "assessment", label: "Candidate assessment" },
      { key: "gaps", label: "Candidate gaps" },
      { key: "boundary", label: "Privacy and authority boundary" },
    ],
    ...
    ...(rows.length === 0 ? {
      emptyState: emptySurface(
        "No governed Implementation Unit Model candidate",
        "Create the candidate through the governed engine workflow after exact Backlog Hierarchy, MVP and Vertical Slice, Acceptance Criteria, Definition of Ready, and Definition of Done candidates exist. This view does not infer repository truth, owner appointment, dependency or impact completeness, implementation readiness or completeness, assignment, execution, approval, acceptance, merge, release, deployment, or action authority.",
      ),
    } : {}),
  }
}

function dependencyMappingTable(state: ObservedStudioState): StudioTableSnapshot {
  const rows = [...state.dependencyMappingProjections.values()].flatMap((projection) => {
    const record = projection.candidate
    if (!record) return []
    const status = projection.status
    return [{
      ...
      cells: {
        ...
        revision: String(record.revision),
        digest: record.digest,
        graph: record.graphDigest,
        criticalPath: record.criticalPathDigest,
        assessmentReceipt: record.assessmentReceiptDigest,
        coverage: `${status.nodeCount} nodes · ${status.edgeCount} edges · ${status.requiredEdgeCount} required · ${status.conditionalEdgeCount} conditional · ${status.advisoryEdgeCount} advisory`,
        path: `${status.criticalPathUnitCount} units · ${status.criticalPathCandidateEffortPoints} candidate effort points · ${status.rootNodeCount} roots · ${status.leafNodeCount} leaves`,
        assessment: `${status.state} · ${status.reviewState}`,
        ...
        boundary: "Candidate identities, counts, statuses, and graph, critical-path, assessment-receipt, and snapshot digests only; no unit, node, edge, evidence, rationale, estimate, owner, repository, module, Requirement, architecture, risk, test, or personal data, dependency truth or completeness, critical-path authority, sequencing commitment, ownership appointment, implementation readiness or completeness, assignment, execution, approval, acceptance, merge, release, deployment, or action authority.",
      },
      state: status.state,
      actions: [],
    }]
  ...
  return {
    ...
    title: "Governed Dependency Mapping Candidate",
    columns: [
      { key: "initiative", label: "Initiative", identifier: true },
      { key: "record", label: "Candidate" },
      { key: "revision", label: "Revision" },
      { key: "digest", label: "Exact digest" },
      { key: "graph", label: "Graph digest" },
      { key: "criticalPath", label: "Critical-path digest" },
      { key: "assessmentReceipt", label: "Assessment receipt" },
      { key: "coverage", label: "Privacy-safe graph coverage" },
      { key: "path", label: "Candidate critical path" },
      { key: "assessment", label: "Candidate assessment" },
      { key: "gaps", label: "Candidate gaps" },
      { key: "boundary", label: "Privacy and authority boundary" },
    ],
...
function boilerplateCompatibilityValidationTable(state: ObservedStudioState): StudioTableSnapshot {
  const rows = [...state.boilerplateCompatibilityValidationProjections.values()].flatMap((projection) => {
    const record = projection.candidate
    if (!record) return []
    const status = projection.status
    return [{
      ...
      cells: {
        ...
        revision: String(record.revision),
        digest: record.digest,
        subjects: record.validationSubjectCatalogDigest,
        dimensions: record.dimensionCatalogDigest,
        evidenceReceipt: record.evidenceReceiptDigest,
        validationReceipt: record.validationReceiptDigest,
        assessmentReceipt: record.assessmentReceiptDigest,
        coverage: `${status.selectedBindingCount} selected bindings · ${status.subjectCount} subjects · ${status.dimensionAssessmentCount} dimension assessments`,
        outcomes: `${status.compatibleCandidateCount} compatible candidates · ${status.incompatibleCandidateCount} incompatible candidates · ${status.exceptionCandidateCount} exception candidates · ${status.notAssessedCount} not assessed`,
        assessment: `${status.state} · ${status.reviewState}`,
        validationGaps: `${status.missingSubjectCount} missing subjects · ${status.invalidSubjectCount} invalid subjects · ${status.missingDimensionCount} missing dimensions · ${status.missingEvidenceCount} missing evidence · ${status.expiredAssessmentCount} expired assessments · ${status.conflictingOutcomeCount} conflicting outcomes · ${status.selectionBindingGapCount} selection-binding gaps`,
        staleGaps: `${status.staleBindingCount} stale bindings · ${status.staleImplementationUnitModelCount} stale Implementation Unit Models · ${status.staleDependencyMappingCount} stale Dependency Mappings · ${status.staleTechnologyProfileCount} stale Technology Profiles · ${status.staleBoilerplateRegistryCount} stale Boilerplate Registries · ${status.staleSelectionBindingCount} stale Selection Bindings · ${status.invalidCandidateCount} invalid candidates · ${status.unresolvedQuestionCount} questions`,
        boundary: "Candidate identities, counts, statuses, and subject, dimension, evidence, validation, assessment, and snapshot digests only; no boilerplate names, locators, versions, unit, profile, entry, or binding identities, claims, evidence, assessors, personal data, compatibility truth or completeness, validation decision, actual asset behavior, test execution, design validity, security, privacy, or licensing approval, exception or waiver, effective selection or binding, source retrieval, import or instantiation, architecture baseline, implementation readiness or completeness, assignment, execution, acceptance, merge, release, deployment, or action authority.",
      },
      state: status.state,
      actions: [],
    }]
  ...
  return {
    ...
    title: "Governed Boilerplate Compatibility Validation Candidate",
    columns: [
      { key: "initiative", label: "Initiative", identifier: true },
      { key: "record", label: "Candidate" },
      { key: "revision", label: "Revision" },
      { key: "digest", label: "Exact digest" },
      { key: "subjects", label: "Validation subject catalog digest" },
      { key: "dimensions", label: "Dimension catalog digest" },
      { key: "evidenceReceipt", label: "Evidence receipt" },
      { key: "validationReceipt", label: "Validation receipt" },
      { key: "assessmentReceipt", label: "Assessment receipt" },
      { key: "coverage", label: "Privacy-safe validation coverage" },
      { key: "outcomes", label: "Candidate outcomes" },
      { key: "assessment", label: "Candidate assessment" },
      { key: "validationGaps", label: "Candidate validation gaps" },
      { key: "staleGaps", label: "Candidate freshness gaps" },
      { key: "boundary", label: "Privacy and authority boundary" },
    ],
    rows,
...
function modelSwitchImplementationTable(state: ObservedStudioState): StudioTableSnapshot {
  const rows = [...state.modelSwitchImplementationProjections.values()].flatMap((projection) => {
    const record = projection.candidate
    if (!record) return []
    const status = projection.status
    return [{ id: record.id, cells: {
      initiative: projection.initiative.id, record: record.id, revision: String(record.revision), digest: record.digest,
      provider: `${record.provider} · ${record.providerSwitchRole}`,
      ...
      capability: `${record.sourceSelection.capabilityDigest} · ${status.continuityGapCount} continuity gaps`,
      transition: `${record.transition.state} · target availability ${record.transition.targetModelAvailabilityState} · capability refresh ${record.transition.capabilityRefreshState} · context transfer ${record.transition.contextTransferState}`,
      lifecycle: `${record.lifecycle.planningState} · model transition ${record.lifecycle.modelTransitionState} · provider execution ${record.lifecycle.providerExecutionState} · handoff ${record.lifecycle.handoffState} · stage ownership ${record.lifecycle.stageOwnershipState} · resume ${record.lifecycle.resumeState}`,
      prerequisites: `${record.prerequisiteCount} required · approval ${record.lifecycle.approvalState} · authorization ${record.lifecycle.authorizationState}`,
      effects: `source mutation ${record.lifecycle.sourceMutationState} · apply ${record.lifecycle.applyState} · discard ${record.lifecycle.discardState} · recovery ${record.lifecycle.recoveryState}`,
      ...
      receipts: `${record.bindingReceiptDigest} · ${record.modelReceiptDigest} · ${record.continuityReceiptDigest} · ${record.transitionReceiptDigest}`,
      assessment: `${status.state} · ${status.reviewState}`,
      boundary: "Privacy-safe same-provider model-switch candidate metadata only. This view does not establish target-model availability, refresh capabilities, execute a provider or model transition, transfer context, record a handoff, transfer stage ownership, resume work, approve, authorize, mutate source, apply/discard, recover, accept, release, deploy, or grant action authority.",
    }, state: status.state, actions: [] }]
  })
  return { id: "model-switch-implementation", title: "Governed Model Switching Within One Provider Candidates",
    columns: [
      { key: "initiative", label: "Initiative", identifier: true }, { key: "record", label: "Candidate" },
      { key: "revision", label: "Revision" }, { key: "digest", label: "Exact digest" },
      { key: "provider", label: "Provider and P3B-06 role" }, { key: "models", label: "Exact source and target models" },
      { key: "capability", label: "Capability snapshot continuity" }, { key: "transition", label: "Transition stop lines" },
      { key: "lifecycle", label: "Distinct lifecycle states" }, { key: "prerequisites", label: "Approval prerequisites" },
      { key: "effects", label: "Effect stop lines" }, { key: "gaps", label: "Candidate gaps" },
      { key: "receipts", label: "Deterministic receipts" }, { key: "assessment", label: "Candidate assessment" },
      { key: "boundary", label: "Privacy and authority boundary" },
...
export function boilerplateConstraintEnforcementTable(projections: Iterable<BoilerplateConstraintEnforcementProjection>): StudioTableSnapshot {
  const rows = [...projections].flatMap((projection) => {
    const record = projection.candidate
    if (!record) return []
    const kindCounts = Object.entries(record.ruleKindCounts).map(([kind, count]) => `${kind} ${count}`).join(" · ")
    return record.targets.map((target) => ({ id: target.id, cells: {
      initiative: projection.initiative.id, policy: record.policyKey, candidate: record.id, revision: String(record.revision),
      target: target.targetKey, trace: target.traceKey, unit: target.implementationUnitId,
      code: `${target.repositoryCandidate}/${target.moduleCandidate}/${target.pathCandidate}`,
      rules: `${target.applicableRuleCount} applicable · ${kindCounts}`,
      violations: `${target.violationCount} candidate violation(s)`, state: target.enforcementState,
      assessment: `${projection.status.state} · ${projection.status.reviewState} · ${projection.status.staleBindingCount} stale · ${projection.status.coverageGapCount} coverage gap · ${projection.status.ruleGapCount} rule gap`,
      receipts: `${record.dependencyReceiptDigest} · ${record.ruleCatalogDigest} · ${record.targetCoverageDigest} · ${record.violationReceiptDigest} · ${record.policyReceiptDigest}`,
      boundary: "Candidate policy metadata only. Repository-relative locations, constraints, conformance and violations are unverified candidates; this view does not inspect repository, source or generated output, execute enforcement, establish compliance, approve exceptions, accept work, release, deploy, or grant action authority.",
    }, state: target.enforcementState === "candidate-conformant" ? projection.status.state : target.enforcementState, actions: [] }))
  })
  return { id: "boilerplate-constraint-enforcement", title: "Governed Boilerplate Constraint Enforcement",
    columns: [
      { key: "initiative", label: "Initiative", identifier: true }, { key: "policy", label: "Policy", identifier: true },
      { key: "candidate", label: "Candidate" }, { key: "revision", label: "Revision" },
      { key: "target", label: "Controlled target" }, { key: "trace", label: "Trace" },
      { key: "unit", label: "Implementation unit" }, { key: "code", label: "Candidate code location" },
      { key: "rules", label: "Stack and architecture rule coverage" }, { key: "violations", label: "Candidate violations" },
      { key: "state", label: "Enforcement candidate state" }, { key: "assessment", label: "Fail-closed assessment" },
...
export class CurrentEngineStudioDataSource implements StudioDataSource {
  ...
  private async observe(route: StudioRoute): Promise<ObservedStudioState> {
    ...
    if (route === "delivery" && engine.definitionOfDone) {
      if (auditSemanticsVerified) {
        const projections = await Promise.allSettled(
          empty.initiatives.map((initiative) => engine.definitionOfDone!.project(initiative.id)),
        )
        projections.forEach((projection, index) => {
          const initiative = empty.initiatives[index]
          if (!initiative) return
          if (projection.status === "fulfilled") {
            ...
            const priority = empty.prioritizationModelProjections.get(initiative.id)?.candidate
            const criteria = empty.acceptanceCriteriaProjections.get(initiative.id)?.candidate
            const ready = empty.definitionOfReadyProjections.get(initiative.id)?.candidate
            const candidate = projection.value.candidate
            const exactDependencies = !candidate || (
              hierarchy !== undefined && projection.value.status.hierarchy?.recordId === hierarchy.id &&
              projection.value.status.hierarchy.revision === hierarchy.revision && projection.value.status.hierarchy.digest === hierarchy.digest &&
              ...
              criteria !== undefined && projection.value.status.acceptanceCriteria?.recordId === criteria.id &&
              projection.value.status.acceptanceCriteria.revision === criteria.revision && projection.value.status.acceptanceCriteria.digest === criteria.digest &&
              ready !== undefined && projection.value.status.definitionOfReady?.recordId === ready.id &&
              projection.value.status.definitionOfReady.revision === ready.revision && projection.value.status.definitionOfReady.digest === ready.digest
            )
            if (
              projection.value.product.id === empty.product?.id &&
              projection.value.product.revision === (empty.product.revision ?? 1) &&
              ...
              empty.definitionOfDoneProjections.set(initiative.id, projection.value)
              return
            }
          }
          this.context.logDiagnostic(
            "Product Studio Definition of Done projection was unavailable or did not bind exact Product, Initiative, Backlog Hierarchy, MVP and Slice Definition, Prioritization Model, Acceptance Criteria, and Definition of Ready revisions",
            projection.status === "rejected" ? projection.reason : undefined,
          )
          empty.issues.push(issue(
            `definition-of-done-${initiative.id}-unavailable`,
            `${initiative.title}: exact privacy-safe Definition of Done metadata is unavailable.`,
            "warning",
            initiative.id,
          ))
        })
        ...
        empty.issues.push(issue(
          "definition-of-done-unavailable",
          "Definition of Done metadata is withheld because the audit chain is invalid or unavailable.",
          "blocker",
        ))
      }
    }
    if (route === "delivery" && engine.implementationUnitModel) {
      if (auditSemanticsVerified) {
        const projections = await Promise.allSettled(
          empty.initiatives.map((initiative) => engine.implementationUnitModel!.project(initiative.id)),
        )
        projections.forEach((projection, index) => {
          const initiative = empty.initiatives[index]
          if (!initiative) return
          if (projection.status === "fulfilled") {
            ...
            const criteria = empty.acceptanceCriteriaProjections.get(initiative.id)?.candidate
            const ready = empty.definitionOfReadyProjections.get(initiative.id)?.candidate
            const done = empty.definitionOfDoneProjections.get(initiative.id)?.candidate
            const candidate = projection.value.candidate
            const exactDependencies = !candidate || (
              hierarchy !== undefined && projection.value.status.hierarchy?.recordId === hierarchy.id &&
              projection.value.status.hierarchy.revision === hierarchy.revision && projection.value.status.hierarchy.digest === hierarchy.digest &&
              ...
              ready !== undefined && projection.value.status.definitionOfReady?.recordId === ready.id &&
              projection.value.status.definitionOfReady.revision === ready.revision && projection.value.status.definitionOfReady.digest === ready.digest &&
              done !== undefined && projection.value.status.definitionOfDone?.recordId === done.id &&
              projection.value.status.definitionOfDone.revision === done.revision && projection.value.status.definitionOfDone.digest === done.digest
            )
            if (
              projection.value.product.id === empty.product?.id &&
              projection.value.product.revision === (empty.product.revision ?? 1) &&
              ...
              empty.implementationUnitModelProjections.set(initiative.id, projection.value)
              return
            }
          }
          this.context.logDiagnostic(
            "Product Studio Implementation Unit Model projection was unavailable or did not bind exact Product, Initiative, Backlog Hierarchy, MVP and Slice Definition, Acceptance Criteria, Definition of Ready, and Definition of Done revisions",
            projection.status === "rejected" ? projection.reason : undefined,
          )
          empty.issues.push(issue(
            `implementation-unit-model-${initiative.id}-unavailable`,
            `${initiative.title}: exact privacy-safe Implementation Unit Model metadata is unavailable.`,
            "warning",
            initiative.id,
          ))
        })
        ...
        empty.issues.push(issue(
          "implementation-unit-model-unavailable",
          "Implementation Unit Model metadata is withheld because the audit chain is invalid or unavailable.",
          "blocker",
        ))
      }
    }
    if (route === "delivery" && engine.dependencyMapping) {
      if (auditSemanticsVerified) {
        const projections = await Promise.allSettled(
          empty.initiatives.map((initiative) => engine.dependencyMapping!.project(initiative.id)),
        )
        projections.forEach((projection, index) => {
          const initiative = empty.initiatives[index]
          if (!initiative) return
          if (projection.status === "fulfilled") {
            ...
            const exactDependencies = !candidate || (
              ...
              projection.value.status.implementationUnitModel.revision === units.revision && projection.value.status.implementationUnitModel.digest === units.digest
            )
            if (
              projection.value.product.id === empty.product?.id &&
              projection.value.product.revision === (empty.product.revision ?? 1) &&
              ...
              empty.dependencyMappingProjections.set(initiative.id, projection.value)
              return
            }
          }
          this.context.logDiagnostic(
            "Product Studio Dependency Mapping projection was unavailable or did not bind exact Product, Initiative, Backlog Hierarchy, MVP and Slice Definition, and Implementation Unit Model revisions",
            projection.status === "rejected" ? projection.reason : undefined,
          )
          empty.issues.push(issue(
            `dependency-mapping-${initiative.id}-unavailable`,
            `${initiative.title}: exact privacy-safe Dependency Mapping metadata is unavailable.`,
            "warning",
            initiative.id,
          ))
        })
    ...
    if (route === "delivery" && engine.boilerplateCompatibilityValidation) {
      if (auditSemanticsVerified) {
        const projections = await Promise.allSettled(
          empty.initiatives.map((initiative) => engine.boilerplateCompatibilityValidation!.project(initiative.id)),
        )
        projections.forEach((projection, index) => {
          const initiative = empty.initiatives[index]
          if (!initiative) return
          if (projection.status === "fulfilled") {
            ...
            const technologyProfile = empty.technologyProfileProjections.get(initiative.id)?.candidate
            const boilerplateRegistry = empty.boilerplateRegistryProjections.get(initiative.id)?.candidate
            const boilerplateSelectionBinding = empty.boilerplateSelectionBindingProjections.get(initiative.id)?.candidate
            const candidate = projection.value.candidate
            const exactDependencies = !candidate || (
              units !== undefined && projection.value.status.implementationUnitModel?.recordId === units.id &&
              projection.value.status.implementationUnitModel.revision === units.revision &&
              ...
              projection.value.status.boilerplateRegistry.revision === boilerplateRegistry.revision &&
              projection.value.status.boilerplateRegistry.digest === boilerplateRegistry.digest &&
              boilerplateSelectionBinding !== undefined &&
              projection.value.status.boilerplateSelectionBinding?.recordId === boilerplateSelectionBinding.id &&
              projection.value.status.boilerplateSelectionBinding.revision === boilerplateSelectionBinding.revision &&
              projection.value.status.boilerplateSelectionBinding.digest === boilerplateSelectionBinding.digest
            )
            if (
              projection.value.product.id === empty.product?.id &&
              projection.value.product.revision === (empty.product.revision ?? 1) &&
              ...
              empty.boilerplateCompatibilityValidationProjections.set(initiative.id, projection.value)
              return
            }
          }
          this.context.logDiagnostic(
            "Product Studio Boilerplate Compatibility Validation projection was unavailable or did not bind exact Product, Initiative, Implementation Unit Model, Dependency Mapping, Technology Profile, Boilerplate Registry, and Boilerplate Selection and Binding revisions",
            projection.status === "rejected" ? projection.reason : undefined,
          )
          empty.issues.push(issue(
            `boilerplate-compatibility-validation-${initiative.id}-unavailable`,
            `${initiative.title}: exact privacy-safe Boilerplate Compatibility Validation metadata is unavailable.`,
            "warning",
            initiative.id,
          ))
        })
    ...
    if (route === "delivery" && engine.modelSwitchImplementation) {
      if (auditSemanticsVerified) {
        const projections = await Promise.allSettled(empty.initiatives.map((initiative) => engine.modelSwitchImplementation!.project(initiative.id)))
        projections.forEach((projection, index) => {
          const initiative = empty.initiatives[index]
          if (!initiative) return
          if (projection.status === "fulfilled") {
            const value = projection.value
            const providerSwitch = empty.providerSwitchImplementationProjections.get(initiative.id)?.candidate
            const codex = empty.controlledCodexImplementationProjections.get(initiative.id)?.candidate
            const claude = empty.controlledClaudeImplementationProjections.get(initiative.id)?.candidate
            const { snapshotDigest, ...projectionBody } = value
            const exactProviderSwitch = providerSwitch !== undefined && value.status.providerSwitchImplementation !== undefined &&
              value.status.providerSwitchImplementation.recordId === providerSwitch.id && value.status.providerSwitchImplementation.revision === providerSwitch.revision &&
              value.status.providerSwitchImplementation.digest === providerSwitch.digest
            const exactCodex = codex !== undefined && value.status.controlledCodexImplementation !== undefined &&
              value.status.controlledCodexImplementation.recordId === codex.id && value.status.controlledCodexImplementation.revision === codex.revision &&
              value.status.controlledCodexImplementation.digest === codex.digest
            const exactClaude = claude !== undefined && value.status.controlledClaudeImplementation !== undefined &&
            ...
            if (value.candidate !== undefined && value.product.id === empty.product?.id && value.product.revision === (empty.product.revision ?? 1) &&
                value.product.digest === canonicalDigest(empty.product) && value.initiative.id === initiative.id &&
                value.initiative.revision === (initiative.revision ?? 1) && value.initiative.digest === canonicalDigest(initiative) &&
                exactProviderSwitch && exactCodex && exactClaude && snapshotDigest === canonicalDigest(projectionBody)) {
              empty.modelSwitchImplementationProjections.set(initiative.id, value)
              return
            }
          }
          this.context.logDiagnostic("Product Studio Model Switch Implementation projection was unavailable or did not bind exact current Provider Switch, Controlled Codex, and Controlled Claude candidates",
            projection.status === "rejected" ? projection.reason : undefined)
          empty.issues.push(issue(`model-switch-implementation-${initiative.id}-unavailable`, `${initiative.title}: exact privacy-safe Model Switch Implementation metadata is unavailable.`, "warning", initiative.id))
        })
      } else if (empty.initiatives.length > 0) {
    ...
    if (route === "delivery" && engine.boilerplateConstraintEnforcement) {
      if (auditSemanticsVerified) {
        const projections = await Promise.allSettled(empty.initiatives.map((initiative) =>
          engine.boilerplateConstraintEnforcement!.project(initiative.id)))
        projections.forEach((projection, index) => {
          const initiative = empty.initiatives[index]
          if (!initiative) return
          if (projection.status === "fulfilled") {
            ...
            const candidates = {
              technologyProfile: empty.technologyProfileProjections.get(initiative.id)?.candidate,
              boilerplateRegistry: empty.boilerplateRegistryProjections.get(initiative.id)?.candidate,
              boilerplateSelectionBinding: empty.boilerplateSelectionBindingProjections.get(initiative.id)?.candidate,
              boilerplateCompatibilityValidation: empty.boilerplateCompatibilityValidationProjections.get(initiative.id)?.candidate,
              figmaToBoilerplateMapping: empty.figmaToBoilerplateMappingProjections.get(initiative.id)?.candidate,
              designToCodeBindingRegistry: empty.designToCodeBindingRegistryProjections.get(initiative.id)?.candidate,
            ...
            if (value.candidate !== undefined && value.product.id === empty.product?.id && value.product.revision === (empty.product.revision ?? 1) &&
                ...
                value.initiative.revision === (initiative.revision ?? 1) && value.initiative.digest === canonicalDigest(initiative) &&
                exactDependencies && snapshotDigest === canonicalDigest(projectionBody)) {
              empty.boilerplateConstraintEnforcementProjections.set(initiative.id, value)
              return
            }
          }
          this.context.logDiagnostic("Product Studio Boilerplate Constraint Enforcement projection was unavailable or did not bind all exact current technology, boilerplate, mapping, generation, traceability, preview, and staging predecessors",
            projection.status === "rejected" ? projection.reason : undefined)
          empty.issues.push(issue(`boilerplate-constraint-enforcement-${initiative.id}-unavailable`, `${initiative.title}: exact privacy-safe Boilerplate Constraint Enforcement metadata is unavailable.`, "warning", initiative.id))
        })
      } else if (empty.initiatives.length > 0) {
        ...
        projections.forEach((projection, index) => {
          ...
          if (projection.status === "fulfilled") {
            ...
            if (
              ...
              empty.humanDesignApprovalProjections.set(initiative.id, projection.value)
              return
            }
          }
          this.context.logDiagnostic(
            "Product Studio Human Design Approval projection was unavailable or did not bind the exact Product and Initiative revisions",
            projection.status === "rejected" ? projection.reason : undefined,
          )
          empty.issues.push(issue(
            `human-design-approval-${initiative.id}-unavailable`,
            `${initiative.title}: exact privacy-safe Human Design Approval metadata is unavailable.`,
            "warning",
            initiative.id,
          ))
        })
