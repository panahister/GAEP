...
CLI Charters contain no per-command `ask` mode because the current non-interactive transport has no GAEP approval callback. The supplied profile allows workspace analysis and local commands inside Codex's read-only sandbox. It denies all workspace modification, network access, and high-impact actions such as commit, push, deploy, publish, spend, privilege change, and deletion.
