...
class GaepEngineClient(
    ...
    @Synchronized
    fun readUserJourneyModel(initiativeId: UUID): UserJourneyProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("design.journeys.snapshot", params) { envelope ->
            PortableDesignProtocol.parseUserJourneyEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readInformationArchitectureModel(initiativeId: UUID): InformationArchitectureProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("design.informationArchitecture.snapshot", params) { envelope ->
            PortableDesignProtocol.parseInformationArchitectureEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readScreenStateInventory(initiativeId: UUID): ScreenStateInventoryProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("design.screenStateInventory.snapshot", params) { envelope ->
            PortableDesignProtocol.parseScreenStateInventoryEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readDesignRequirements(initiativeId: UUID): DesignRequirementsProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("design.requirements.snapshot", params) { envelope ->
            PortableDesignProtocol.parseDesignRequirementsEnvelope(envelope, initiativeId)
        }
    }
    ...
    @Synchronized
    fun readAcceptanceCriteria(initiativeId: UUID): AcceptanceCriteriaProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("planning.acceptanceCriteria.snapshot", params) { envelope ->
            PortableDesignProtocol.parseAcceptanceCriteriaEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readDefinitionOfReady(initiativeId: UUID): DefinitionOfReadyProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("planning.definitionOfReady.snapshot", params) { envelope ->
            PortableDesignProtocol.parseDefinitionOfReadyEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readDefinitionOfDone(initiativeId: UUID): DefinitionOfDoneProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("planning.definitionOfDone.snapshot", params) { envelope ->
            PortableDesignProtocol.parseDefinitionOfDoneEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readImplementationUnitModel(initiativeId: UUID): ImplementationUnitModelProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("planning.implementationUnits.snapshot", params) { envelope ->
            PortableDesignProtocol.parseImplementationUnitModelEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readDependencyMapping(initiativeId: UUID): DependencyMappingProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("planning.dependencyMapping.snapshot", params) { envelope ->
            PortableDesignProtocol.parseDependencyMappingEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readTechnologyProfile(initiativeId: UUID): TechnologyProfileProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("planning.technologyProfile.snapshot", params) { envelope ->
            PortableDesignProtocol.parseTechnologyProfileEnvelope(envelope, initiativeId)
        }
    }
    ...
    @Synchronized
    fun readBoilerplateSelectionBinding(initiativeId: UUID): BoilerplateSelectionBindingProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("planning.boilerplateSelectionBinding.snapshot", params) { envelope ->
            PortableDesignProtocol.parseBoilerplateSelectionBindingEnvelope(envelope, initiativeId)
        }
    }
    ...
    @Synchronized
    fun readDesignToCodeBindingRegistry(initiativeId: UUID): DesignToCodeBindingRegistryProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("planning.designToCodeBindingRegistry.snapshot", params) { envelope ->
            PortableDesignProtocol.parseDesignToCodeBindingRegistryEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readRouteScreenComponentMapping(initiativeId: UUID): RouteScreenComponentMappingProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("planning.routeScreenComponentMapping.snapshot", params) { envelope ->
            PortableDesignProtocol.parseRouteScreenComponentMappingEnvelope(envelope, initiativeId)
        }
    }
    ...
    @Synchronized
    fun readHighLevelDesign(initiativeId: UUID): HighLevelDesignProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("planning.highLevelDesign.snapshot", params) { envelope ->
            PortableDesignProtocol.parseHighLevelDesignEnvelope(envelope, initiativeId)
        }
    }
    ...
    @Synchronized
    fun readChangedUnitInventory(initiativeId: UUID): ChangedUnitInventoryProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("delivery.changedUnitInventory.snapshot", params) { envelope ->
            PortableDesignProtocol.parseChangedUnitInventoryEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readProposedChangePreview(initiativeId: UUID): ProposedChangePreviewProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("delivery.proposedChangePreview.snapshot", params) { envelope ->
            PortableDesignProtocol.parseProposedChangePreviewEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readStagingWorkspace(initiativeId: UUID): StagingWorkspaceProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("delivery.stagingWorkspace.snapshot", params) { envelope ->
            PortableDesignProtocol.parseStagingWorkspaceEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readControlledCodexImplementation(initiativeId: UUID): ControlledCodexImplementationProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("delivery.controlledCodexImplementation.snapshot", params) { envelope ->
            PortableDesignProtocol.parseControlledCodexImplementationEnvelope(envelope, initiativeId)
        }
    }
    @Synchronized
    fun readControlledClaudeImplementation(initiativeId: UUID): ControlledClaudeImplementationProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("delivery.controlledClaudeImplementation.snapshot", params) { envelope ->
            PortableDesignProtocol.parseControlledClaudeImplementationEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readProviderSwitchImplementation(initiativeId: UUID): ProviderSwitchImplementationProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("delivery.providerSwitchImplementation.snapshot", params) { envelope ->
            PortableDesignProtocol.parseProviderSwitchImplementationEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readModelSwitchImplementation(initiativeId: UUID): ModelSwitchImplementationProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("delivery.modelSwitchImplementation.snapshot", params) { envelope ->
            PortableDesignProtocol.parseModelSwitchImplementationEnvelope(envelope, initiativeId)
        }
    }
    ...
    fun readDesignSystemTokenContract(initiativeId: UUID): DesignSystemTokenContractProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("design.systemTokenContract.snapshot", params) { envelope ->
            PortableDesignProtocol.parseDesignSystemTokenContractEnvelope(envelope, initiativeId)
        }
    }
    ...
    @Synchronized
    fun readResponsiveMultiPlatformTargets(initiativeId: UUID): ResponsiveMultiPlatformTargetsProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("design.responsiveMultiPlatformTargets.snapshot", params) { envelope ->
            PortableDesignProtocol.parseResponsiveMultiPlatformTargetsEnvelope(envelope, initiativeId)
        }
    }
    ...
    @Synchronized
    fun readOutboundDesignBriefPackage(initiativeId: UUID): OutboundDesignBriefPackageProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("design.outboundDesignBriefPackage.snapshot", params) { envelope ->
            PortableDesignProtocol.parseOutboundDesignBriefPackageEnvelope(envelope, initiativeId)
        }
    }
    ...
    @Synchronized
    fun readDesignToRequirementBinding(initiativeId: UUID): DesignToRequirementBindingProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("design.designToRequirementBinding.snapshot", params) { envelope ->
            PortableDesignProtocol.parseDesignToRequirementBindingEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readDesignerReadyGate(initiativeId: UUID): DesignerReadyGateProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("design.designerReadyGate.snapshot", params) { envelope ->
            PortableDesignProtocol.parseDesignerReadyGateEnvelope(envelope, initiativeId)
        }
    }

    @Synchronized
    fun readDesignDelta(initiativeId: UUID): DesignDeltaProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("design.designDelta.snapshot", params) { envelope ->
            PortableDesignProtocol.parseDesignDeltaEnvelope(envelope, initiativeId)
        }
    }
    ...
    @Synchronized
    fun readDesignBaseline(initiativeId: UUID): DesignBaselineProjection {
        require(initiativeId != UUID(0, 0)) { "Initiative ID must be a non-empty UUID" }
        val params = JsonObject().apply { addProperty("initiativeId", initiativeId.toString()) }
        return portableRequest("design.designBaseline.snapshot", params) { envelope ->
            PortableDesignProtocol.parseDesignBaselineEnvelope(envelope, initiativeId)
        }
    }
    ...
    fun readPhase2ChangeImpactAgentModelDashboard(
        ...
        return portableRequest("dashboard.phase2ChangeImpactAgentModel", params) { envelope ->
            PortableDesignProtocol.parsePhase2ChangeImpactAgentModelDashboardEnvelope(
    ...
    fun readPhase1Summary(product: ProductBinding, initiative: InitiativeEntryRecord): Phase1SummaryDashboard {
        ...
        return portableRequest("dashboard.phase1Summary", params) { envelope ->
            PortableDesignProtocol.parsePhase1SummaryEnvelope(envelope, product, initiative)
        }
    }

    @Synchronized
    fun readPhase1ChangeImpact(
        product: ProductBinding,
        initiative: InitiativeEntryRecord,
        ...
        require(Regex("^sha256:[0-9a-f]{64}$").matches(product.digest) &&
            ...
            Regex("^sha256:[0-9a-f]{64}$").matches(change.digest)) { "Exact digests must be SHA-256" }
        val params = JsonObject().apply {
            addProperty("expectedProductId", product.id.toString())
            addProperty("expectedProductRevision", product.revision)
            addProperty("expectedProductDigest", product.digest)
        ...
        return portableRequest("dashboard.phase1ChangeImpact", params) { envelope ->
            PortableDesignProtocol.parsePhase1ChangeImpactEnvelope(envelope, product, initiative, change)
        }
    }
