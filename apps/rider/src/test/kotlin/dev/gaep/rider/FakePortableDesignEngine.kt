...
fun main(arguments: Array<String>) {
    ...
    generateSequence(::readLine).forEach { line ->
        ...
        when (method) {
            ...
            "planning.testInventory.snapshot" -> handleTestInventory(
                id,
                request.getAsJsonObject("params"),
                workspacePath,
            )
...
private fun handleDefinitionOfDone(id: Long, params: JsonObject, workspacePath: String) {
    if (params.keySet() != setOf("initiativeId") || params.get("initiativeId").asString != initiativeId.toString()) {
        writeError(id, -32_602, "INVALID_PARAMS", "PRIVATE DEFINITION OF DONE PARAMS")
        return
    }
    val productRevision = if (workspacePath.endsWith("bad-definition-of-done-snapshot-binding")) 8 else 7
    val assessedAt = "2026-07-30T14:20:00.000Z"
    val candidateDigest = "sha256:${"e".repeat(64)}"
    val content = JsonObject().apply {
        addProperty("schemaVersion", 1)
        addProperty("kind", "definition-of-done-projection")
        add("product", JsonObject().apply {
            addProperty("id", productId.toString())
            addProperty("revision", productRevision)
            addProperty("digest", canonicalDigest(productRecord()))
        ...
        add("status", JsonObject().apply {
            addProperty("schemaVersion", 1)
            addProperty("kind", "definition-of-done-status")
            addProperty("productId", productId.toString())
            addProperty("productRevision", productRevision)
            addProperty("initiativeId", initiativeId.toString())
            addProperty("initiativeRevision", initiativeState.get("revision").asLong)
            ...
            add("hierarchy", JsonObject().apply {
                addProperty("recordId", backlogHierarchyId.toString())
                addProperty("revision", 2)
                addProperty("digest", if (workspacePath.endsWith("bad-definition-of-done-hierarchy-binding")) "sha256:${"7".repeat(64)}" else "sha256:${"8".repeat(64)}")
            })
            add("mvpSliceDefinition", JsonObject().apply {
                addProperty("recordId", mvpSliceDefinitionId.toString())
                addProperty("revision", 2)
                addProperty("digest", if (workspacePath.endsWith("bad-definition-of-done-mvp-binding")) "sha256:${"9".repeat(64)}" else "sha256:${"a".repeat(64)}")
            })
            add("prioritizationModel", JsonObject().apply {
                addProperty("recordId", prioritizationModelId.toString())
                addProperty("revision", 2)
                addProperty("digest", if (workspacePath.endsWith("bad-definition-of-done-prioritization-binding")) "sha256:${"b".repeat(64)}" else "sha256:${"c".repeat(64)}")
            })
            add("acceptanceCriteria", JsonObject().apply {
                addProperty("recordId", acceptanceCriteriaId.toString())
                addProperty("revision", 2)
                addProperty("digest", if (workspacePath.endsWith("bad-definition-of-done-criteria-binding")) "sha256:${"0".repeat(64)}" else "sha256:${"1".repeat(64)}")
            })
            add("definitionOfReady", JsonObject().apply {
                addProperty("recordId", definitionOfReadyId.toString())
                addProperty("revision", 2)
                addProperty("digest", if (workspacePath.endsWith("bad-definition-of-done-ready-binding")) "sha256:${"5".repeat(64)}" else "sha256:${"6".repeat(64)}")
            })
            addProperty("subjectCount", 4)
            addProperty("policyEntryCount", 6)
            addProperty("expectedEvaluationCount", 24)
            addProperty("evaluationCount", 21)
            addProperty("candidateSatisfiedCount", 14)
            addProperty("notSatisfiedCount", 2)
            addProperty("notApplicableCount", 3)
            addProperty("exceptionCandidateCount", 1)
            addProperty("notAssessedCount", 1)
            addProperty("staleEvaluationCount", 1)
            addProperty("invalidEvaluationCount", 2)
            addProperty("missingEvaluationCount", 3)
            addProperty("staleBindingCount", 0)
            addProperty("staleHierarchyCount", 0)
            addProperty("staleMvpSliceDefinitionCount", 0)
            addProperty("stalePrioritizationModelCount", 0)
            ...
            addProperty("reviewState", "held")
            addProperty("result", "attention-required")
            add("reasons", JsonArray().apply { add("One or more completion prerequisites require review") })
            addProperty("assessedAt", assessedAt)
            addProperty("gateBoundary", "a-passing-definition-of-done-candidate-is-an-evaluation-result-not-completion-acceptance-approval-merge-release-deployment-or-action-permission")
            addProperty("authorityBoundary", "definition-of-done-status-is-observational-and-does-not-establish-evidence-truth-test-success-quality-requirement-satisfaction-acceptance-criteria-satisfaction-approval-ready-done-exception-waiver-authority-implementation-completeness-merge-readiness-release-readiness-deployment-readiness-assignment-execution-acceptance-or-action-authority")
        })
        add("candidate", JsonObject().apply {
            addProperty("id", definitionOfDoneId.toString())
            addProperty("revision", 2)
            addProperty("digest", candidateDigest)
            addProperty("state", "candidate")
            addProperty("policyVersion", 4)
            addProperty("validUntil", "2026-08-30T14:19:00.000Z")
            addProperty("subjectCatalogDigest", "sha256:${"1".repeat(64)}")
            addProperty("policyDigest", "sha256:${"2".repeat(64)}")
            addProperty("evaluationDigest", "sha256:${"3".repeat(64)}")
            addProperty("receiptDigest", "sha256:${"4".repeat(64)}")
            addProperty("subjectCount", 4)
            addProperty("policyEntryCount", 6)
            addProperty("evaluationCount", 21)
            addProperty("reviewState", "held")
            addProperty("updatedAt", "2026-07-30T14:19:00.000Z")
        })
        addProperty("observedAt", assessedAt)
        addProperty("privacyBoundary", "projection-contains-record-identities-counts-statuses-and-policy-evaluation-receipt-snapshot-digests-only-not-rules-rationales-evidence-identities-assessor-identities-personal-data-secrets-credentials-or-machine-paths")
        addProperty("gateBoundary", "a-passing-definition-of-done-candidate-is-an-evaluation-result-not-completion-acceptance-approval-merge-release-deployment-or-action-permission")
        addProperty("authorityBoundary", "definition-of-done-projection-is-read-only-and-does-not-establish-evidence-truth-test-success-quality-requirement-satisfaction-acceptance-criteria-satisfaction-approval-ready-done-exception-waiver-authority-implementation-completeness-merge-readiness-release-readiness-deployment-readiness-assignment-execution-acceptance-or-action-authority")
    }
    val value = content.deepCopy().apply { addProperty("snapshotDigest", canonicalDigest(content)) }
    when {
        workspacePath.endsWith("bad-definition-of-done-snapshot-digest") -> {
...
private fun handleTestInventory(id: Long, params: JsonObject, workspacePath: String) {
    if (params.keySet() != setOf("initiativeId") || params.get("initiativeId").asString != initiativeId.toString()) {
        writeError(id, -32_602, "INVALID_PARAMS", "PRIVATE TEST INVENTORY PARAMS")
        return
    }
    val productRevision = if (workspacePath.endsWith("bad-test-inventory-snapshot-binding")) 8 else 7
    val assessedAt = "2026-07-31T02:00:00.000Z"
    val candidateDigest = "sha256:${"9".repeat(64)}"
    fun reference(recordId: UUID, revision: Int, digest: String) = JsonObject().apply {
        addProperty("recordId", recordId.toString())
        addProperty("revision", revision)
    ...
    val content = JsonObject().apply {
        addProperty("schemaVersion", 1)
        addProperty("kind", "test-inventory-projection")
        add("product", JsonObject().apply {
            addProperty("id", productId.toString())
            addProperty("revision", productRevision)
            addProperty("digest", canonicalDigest(productRecord()))
        ...
        add("status", JsonObject().apply {
            addProperty("schemaVersion", 1)
            addProperty("kind", "test-inventory-status")
            addProperty("productId", productId.toString())
            addProperty("productRevision", productRevision)
            addProperty("initiativeId", initiativeId.toString())
            addProperty("initiativeRevision", initiativeState.get("revision").asLong)
            add("candidate", reference(testInventoryId, 2, candidateDigest))
            add("acceptanceCriteria", reference(acceptanceCriteriaId, 2, "sha256:${"1".repeat(64)}"))
            add("riskRegister", reference(riskRegisterId, 3, "sha256:${"e".repeat(64)}"))
            add("implementationUnitModel", reference(implementationUnitModelId, 2, "sha256:${"5".repeat(64)}"))
            add("routeScreenComponentMapping", reference(routeScreenComponentMappingId, 2, "sha256:${"2".repeat(64)}"))
            add(
                "testMethodology",
                reference(
                    testMethodologyId,
                    2,
                    "sha256:${if (workspacePath.endsWith("bad-test-inventory-dependency-binding")) "0".repeat(64) else "8".repeat(64)}",
                ),
            )
            addProperty("sourceCriterionCount", 12)
            addProperty("sourceRiskCount", 9)
            addProperty("sourceUnitCount", 4)
            addProperty("sourceMappingSubjectCount", 14)
            addProperty("sourceMethodologyScopeCount", 4)
            addProperty("assetCount", 18)
            addProperty("catalogedAssetCount", 14)
            addProperty("conflictAssetCount", 1)
            addProperty("missingAssetCount", 1)
            addProperty("deferredAssetCount", 1)
            addProperty("notAssessedAssetCount", 1)
            addProperty("observedAssetCount", 11)
            addProperty("plannedAssetCount", 5)
            addProperty("automatedAssetCount", 10)
            addProperty("manualAssetCount", 4)
            addProperty("duplicateIdentityCount", 1)
            addProperty("orphanAssetCount", 1)
            addProperty("uncoveredCriterionCount", 2)
            addProperty("uncoveredRiskCount", 1)
            addProperty("uncoveredUnitCount", 1)
            addProperty("uncoveredMappingSubjectCount", 2)
            addProperty("uncoveredMethodologyScopeCount", 1)
            addProperty("ownershipGapCount", 1)
            addProperty("traceGapCount", 2)
            addProperty("evidenceGapCount", 1)
            addProperty("staleBindingCount", 0)
            ...
            addProperty("reviewState", "held")
            addProperty("state", "attention-required")
            add("reasons", JsonArray().apply { add("One or more Test Inventory candidates require human review") })
            addProperty("assessedAt", assessedAt)
            addProperty(
                "authorityBoundary",
        ...
        add("candidate", JsonObject().apply {
            addProperty("id", testInventoryId.toString())
            addProperty("revision", 2)
            addProperty("digest", candidateDigest)
            addProperty("state", "candidate")
            addProperty("catalogReceiptDigest", "sha256:${"1".repeat(64)}")
            addProperty("coverageReceiptDigest", "sha256:${"2".repeat(64)}")
            addProperty("traceReceiptDigest", "sha256:${"3".repeat(64)}")
            addProperty("ownershipReceiptDigest", "sha256:${"4".repeat(64)}")
            addProperty("assessmentReceiptDigest", "sha256:${"5".repeat(64)}")
            addProperty("assetCount", 18)
            addProperty("catalogedAssetCount", 14)
            addProperty("conflictAssetCount", 1)
            ...
            addProperty("plannedAssetCount", 5)
            addProperty("reviewState", "held")
            addProperty("updatedAt", "2026-07-31T01:59:00.000Z")
        })
        addProperty("observedAt", assessedAt)
        addProperty(
            "privacyBoundary",
    ...
    val value = content.deepCopy().apply { addProperty("snapshotDigest", canonicalDigest(content)) }
    when {
        workspacePath.endsWith("bad-test-inventory-snapshot-digest") ->
            value.getAsJsonObject("candidate").addProperty("assetCount", 19)
        workspacePath.endsWith("bad-test-inventory-snapshot-private") -> {
            value.addProperty("testPath", "$privateRoot/$privateCredential")
            val digestBody = value.deepCopy().also { it.remove("snapshotDigest") }
            value.addProperty("snapshotDigest", canonicalDigest(digestBody))
        }
    }
