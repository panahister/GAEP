...
async function run() {
  ...
  process.stdout.write(`PASS activation: nineteen bounded commands, machine-only configuration, static Product Studio, exact package-local engine ${packagedEngineSha256}, provider/model dashboard, empty audit-gated evidence workflow, and no workspace mutation\n`)
}
