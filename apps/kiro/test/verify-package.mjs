...
const maximumTotalBytes = 9 * 1024 * 1024
...
export async function verifyKiroPackage(packagePath = resolve(kiroRoot, "dist/gaep-kiro.vsix")) {
  ...
  if (!Array.isArray(commands) || commands.length !== 95 ||
      ...
      !commands.some((command) => command.command === "gaepKiro.dashboard.phase3a") ||
      ...
      !commands.some((command) => command.command === "gaepKiro.implementationUnitModel.inspect") ||
      !commands.some((command) => command.command === "gaepKiro.dependencyMapping.inspect") ||
      !commands.some((command) => command.command === "gaepKiro.technologyProfile.inspect") ||
      ...
      !commands.some((command) => command.command === "gaepKiro.testInventory.inspect") ||
      !commands.some((command) => command.command === "gaepKiro.highLevelDesign.inspect") ||
      !commands.some((command) => command.command === "gaepKiro.lowLevelDesign.inspect") ||
      !commands.some((command) => command.command === "gaepKiro.implementationReadinessGate.inspect") ||
      !commands.some((command) => command.command === "gaepKiro.changedUnitInventory.inspect") ||
      !commands.some((command) => command.command === "gaepKiro.proposedChangePreview.inspect") ||
      !commands.some((command) => command.command === "gaepKiro.stagingWorkspace.inspect") ||
      !commands.some((command) => command.command === "gaepKiro.controlledCodexImplementation.inspect") ||
      !commands.some((command) => command.command === "gaepKiro.controlledClaudeImplementation.inspect") ||
      !commands.some((command) => command.command === "gaepKiro.providerSwitchImplementation.inspect") ||
      !commands.some((command) => command.command === "gaepKiro.modelSwitchImplementation.inspect") ||
      ...
      !commands.some((command) => command.command === "gaepKiro.designToRequirementBinding.inspect") ||
      !commands.some((command) => command.command === "gaepKiro.designerReadyGate.inspect") ||
    ...
    fail("package command inventory must contain 95 unique commands including the Phase 3B Changed Unit Inventory, Proposed Change Preview, Isolated Staging Workspace, Controlled Codex Implementation, Controlled Claude Implementation, Provider Switching During Implementation, and Model Switching Within One Provider, Phase 3A dashboard, both Phase 2 dashboards, all three Phase 1 dashboards, Backlog Hierarchy, MVP and Vertical Slice Definition, Prioritization Model, Acceptance Criteria, Definition of Ready, Definition of Done, Implementation Unit Model, Dependency Mapping, Technology Profile, Boilerplate Registry, Boilerplate Selection and Binding, Boilerplate Compatibility Validation, Figma-to-Boilerplate Mapping, Design-to-Code Binding Registry, Route, Screen, and Component Mapping, Test Methodology, Test Inventory, High-Level Design, Low-Level Design, Implementation Readiness Gate, and governed Design phase candidates through Design Drift Detection")
  }
