...
export class GaepEngineClient {
  ...
  readPrioritizationModel(initiativeValue: string): Promise<PrioritizationModelProjection> {
    return this.enqueue(async () => {
      const initiativeId = normalizeUuid(initiativeValue, "Initiative ID")
      const parsed = prioritizationModelProjectionSchema.safeParse(
        await this.request("planning.prioritization.snapshot", { initiativeId }),
      )
      if (!parsed.success) throw invalidHostResponse()
      const projection = parsed.data
      const { snapshotDigest, ...projectionBody } = projection
  ...
  readAcceptanceCriteria(initiativeValue: string): Promise<AcceptanceCriteriaProjection> {
    return this.enqueue(async () => {
      const initiativeId = normalizeUuid(initiativeValue, "Initiative ID")
      const parsed = acceptanceCriteriaProjectionSchema.safeParse(
        await this.request("planning.acceptanceCriteria.snapshot", { initiativeId }),
      )
      if (!parsed.success) throw invalidHostResponse()
      const projection = parsed.data
      const { snapshotDigest, ...projectionBody } = projection
  ...
  readDefinitionOfReady(initiativeValue: string): Promise<DefinitionOfReadyProjection> {
    return this.enqueue(async () => {
      const initiativeId = normalizeUuid(initiativeValue, "Initiative ID")
  ...
  readImplementationUnitModel(initiativeValue: string): Promise<ImplementationUnitModelProjection> {
    return this.enqueue(async () => {
      const initiativeId = normalizeUuid(initiativeValue, "Initiative ID")
  ...
  readBoilerplateSelectionBinding(initiativeValue: string): Promise<BoilerplateSelectionBindingProjection> {
    return this.enqueue(async () => {
      const initiativeId = normalizeUuid(initiativeValue, "Initiative ID")
  ...
  readStagingWorkspace(initiativeValue: string): Promise<StagingWorkspaceProjection> {
    return this.enqueue(async () => {
      const initiativeId = normalizeUuid(initiativeValue, "Initiative ID")
      const parsed = stagingWorkspaceProjectionSchema.safeParse(
        await this.request("delivery.stagingWorkspace.snapshot", { initiativeId }),
      )
      if (!parsed.success) throw invalidHostResponse()
      const projection = parsed.data
      const { snapshotDigest, ...projectionBody } = projection
  ...
  readControlledCodexImplementation(initiativeValue: string): Promise<ControlledCodexImplementationProjection> {
    return this.enqueue(async () => {
      const initiativeId = normalizeUuid(initiativeValue, "Initiative ID")
      const parsed = controlledCodexImplementationProjectionSchema.safeParse(
        await this.request("delivery.controlledCodexImplementation.snapshot", { initiativeId }),
      )
      if (!parsed.success) throw invalidHostResponse()
      const projection = parsed.data
      const { snapshotDigest, ...projectionBody } = projection
  ...
  readControlledClaudeImplementation(initiativeValue: string): Promise<ControlledClaudeImplementationProjection> {
    return this.enqueue(async () => {
      const initiativeId = normalizeUuid(initiativeValue, "Initiative ID")
  ...
  readModelSwitchImplementation(initiativeValue: string): Promise<ModelSwitchImplementationProjection> {
    return this.enqueue(async () => {
      const initiativeId = normalizeUuid(initiativeValue, "Initiative ID")
  ...
  async readPhase2ChangeImpactAgentModelDashboard(
    ...
    return this.enqueue(async () => {
      ...
      const result = await this.request("dashboard.phase2ChangeImpactAgentModel", {
        expectedProductId: productId,
        expectedProductRevision: productRevision,
        expectedProductDigest: productDigest,
      ...
      const parsed = phase2ChangeImpactAgentModelDashboardSchema.safeParse(result)
      if (!parsed.success) throw invalidHostResponse()
      const dashboard = parsed.data
      const { snapshotDigest, ...content } = dashboard
      if (snapshotDigest !== canonicalDigest(content) ||
          dashboard.product.recordId.toLowerCase() !== productId || dashboard.product.revision !== productRevision ||
          ...
          dashboard.initiative.state !== initiative.state) throw invalidHostResponse()
      return dashboard
    })
  }
