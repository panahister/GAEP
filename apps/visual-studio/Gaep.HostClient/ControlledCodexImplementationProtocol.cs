...
{
    private const string ControlledCodexImplementationProjectionPrivacyBoundary =
    ...
    private const string ControlledCodexImplementationProjectionAuthorityBoundary =
    ...
    private const string ControlledCodexImplementationStatusAuthorityBoundary =
    ...
    internal static ControlledCodexImplementationProjection ParseControlledCodexImplementationResponse(JsonElement envelope, Guid expectedInitiativeId)
    {
        ...
        if (!HasRequiredAndAllowedProperties(projection, ["schemaVersion", "kind", "product", "initiative", "status", "observedAt", "privacyBoundary", "authorityBoundary", "snapshotDigest"], ["candidate"]) ||
            ParseBoundedNonNegativeInt(projection, "schemaVersion", 1) != 1 || ParseRequiredEnum(projection, "kind", "controlled-codex-implementation-projection") != "controlled-codex-implementation-projection" ||
            ParseRequiredEnum(projection, "privacyBoundary", ControlledCodexImplementationProjectionPrivacyBoundary) != ControlledCodexImplementationProjectionPrivacyBoundary ||
            ParseRequiredEnum(projection, "authorityBoundary", ControlledCodexImplementationProjectionAuthorityBoundary) != ControlledCodexImplementationProjectionAuthorityBoundary) throw InvalidResponse();
        ...
        if (!HasRequiredAndAllowedProperties(status, ["schemaVersion", "kind", "productId", "productRevision", "initiativeId", "initiativeRevision", .. counts, "reviewState", "state", "reasons", "assessedAt", "authorityBoundary"], ["candidate", "proposedChangePreview", "stagingWorkspace"]) ||
            ParseBoundedNonNegativeInt(status, "schemaVersion", 1) != 1 || ParseRequiredEnum(status, "kind", "controlled-codex-implementation-status") != "controlled-codex-implementation-status" ||
            ...
            ParseRequiredEnum(status, "authorityBoundary", ControlledCodexImplementationStatusAuthorityBoundary) != ControlledCodexImplementationStatusAuthorityBoundary) throw InvalidResponse();
        ...
        ControlledCodexImplementationRecordView? candidate = null;
        if (projection.TryGetProperty("candidate", out var value)) {
            if (!HasOnlyProperties(value, "id", "revision", "digest", "state", "provider", "plan", "lifecycle", "resourceScopes", "permissions", "unitCount", "pathCount", "prerequisiteCount", "bindingReceiptDigest", "providerReceiptDigest", "scopeReceiptDigest", "planReceiptDigest", "stagedEffectReceiptDigest", "lifecycleReceiptDigest", "prerequisiteReceiptDigest", "recoveryReceiptDigest", "assessmentReceiptDigest", "reviewState", "updatedAt") || ParseRequiredEnum(value, "state", "candidate") != "candidate") throw InvalidResponse();
            ...
            if (adapterId != "gaep.codex-cli" || agentId != "codex-cli") throw InvalidResponse(); if (provider.TryGetProperty("runtimeVersion", out var runtime)) ParseSourceText(runtime, 1, 1_024);
            var plan = value.GetProperty("plan"); if (!HasOnlyProperties(plan, "strategy", "planKey", "workflowPlan", "planReceiptDigest", "stagedEffectReceiptDigest") || ParseRequiredEnum(plan, "strategy", "managed-codex-staged-candidate") != "managed-codex-staged-candidate") throw InvalidResponse();
            ...
            var scopes = value.GetProperty("resourceScopes"); var permissions = value.GetProperty("permissions"); if (scopes.ValueKind != JsonValueKind.Array || scopes.GetArrayLength() > 65_536 || permissions.ValueKind != JsonValueKind.Array || permissions.GetArrayLength() > 256) throw InvalidResponse();
            foreach (var scope in scopes.EnumerateArray()) ParseWorkspaceRelativePath(scope); foreach (var permission in permissions.EnumerateArray()) { if (!HasOnlyProperties(permission, "capability", "mode", "scope")) throw InvalidResponse(); ParseSourceText(permission.GetProperty("capability"), 1, 256); ParseRequiredEnum(permission, "mode", "allow", "ask", "deny"); if (permission.GetProperty("scope").ValueKind != JsonValueKind.Array) throw InvalidResponse(); }
            foreach (var name in new[] { "bindingReceiptDigest", "providerReceiptDigest", "scopeReceiptDigest", "planReceiptDigest", "stagedEffectReceiptDigest", "lifecycleReceiptDigest", "prerequisiteReceiptDigest", "recoveryReceiptDigest", "assessmentReceiptDigest" }) ParseRequiredDigest(value, name);
            candidate = new(id, revision, digest, adapterId, agentId, modelId, capabilityDigest, planKey, providerExecution, realStage, sourceMutation, apply, discard, ParseBoundedNonNegativeInt(value, "unitCount", 65_536), ParseBoundedNonNegativeInt(value, "pathCount", 65_536), ParseBoundedNonNegativeInt(value, "prerequisiteCount", 4), ParseRequiredEnum(value, "reviewState", "draft", "held", "ready-for-human-review"));
