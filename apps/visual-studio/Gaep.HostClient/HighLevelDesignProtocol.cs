...
{
    private const string HighLevelDesignProjectionPrivacyBoundary =
        "projection-contains-record-identities-counts-statuses-and-structure-dependency-trace-coverage-ownership-assessment-snapshot-digests-only-not-design-narratives-diagrams-interfaces-data-flows-technologies-owners-evidence-source-content-personal-data-secrets-credentials-or-machine-paths";
    private const string HighLevelDesignProjectionAuthorityBoundary =
        "high-level-design-projection-is-read-only-and-does-not-establish-architecture-repository-runtime-or-deployment-truth-or-completeness-architecture-baseline-or-approval-privacy-or-security-approval-owner-appointment-implementation-readiness-acceptance-release-deployment-or-action-authority";
    private const string HighLevelDesignStatusAuthorityBoundary =
        "high-level-design-status-is-observational-and-does-not-establish-architecture-repository-runtime-or-deployment-truth-or-completeness-architecture-baseline-or-approval-privacy-or-security-approval-owner-appointment-implementation-readiness-acceptance-release-deployment-or-action-authority";

    internal static HighLevelDesignProjection ParseHighLevelDesignResponse(JsonElement envelope, Guid expectedInitiativeId)
    {
        ...
        if (!HasRequiredAndAllowedProperties(projection,
            ...
            ParseRequiredEnum(projection, "kind", "high-level-design-projection") != "high-level-design-projection" ||
            ParseRequiredEnum(projection, "privacyBoundary", HighLevelDesignProjectionPrivacyBoundary) != HighLevelDesignProjectionPrivacyBoundary ||
            ParseRequiredEnum(projection, "authorityBoundary", HighLevelDesignProjectionAuthorityBoundary) != HighLevelDesignProjectionAuthorityBoundary)
        ...
        string[] dependencyNames = [
            "systemSolutionArchitecture", "boundedContextModel", "technologyProfile", "dependencyMapping", "implementationUnitModel",
        ...
        if (!HasRequiredAndAllowedProperties(status,
                ["schemaVersion", "kind", "productId", "productRevision", "initiativeId", "initiativeRevision", .. countNames,
            ...
            ParseRequiredEnum(status, "kind", "high-level-design-status") != "high-level-design-status" ||
            ...
            ParseRequiredEnum(status, "authorityBoundary", HighLevelDesignStatusAuthorityBoundary) != HighLevelDesignStatusAuthorityBoundary)
            throw InvalidResponse();
        ...
        var staleDependencyCount = Count("staleDependencyCount", 15); var invalidCandidateCount = Count("invalidCandidateCount", 1);
        ...
        HighLevelDesignRecordView? candidate = null;
        if (projection.TryGetProperty("candidate", out var candidateElement))
        {
            ...
            candidate = new HighLevelDesignRecordView(id, revision, digest,
        ...
        return new HighLevelDesignProjection(productId, productRevision, productDigest, initiativeId, initiativeRevision, initiativeDigest,
            initiativeState, state, reviewState, Array.AsReadOnly(reasons), dependencyCount, presentDependencyCount, elementCount,
