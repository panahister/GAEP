...
public sealed class EngineClient : IAsyncDisposable
{
    ...
    public async Task<ImplementationReadinessGateProjection> ReadImplementationReadinessGateAsync(Guid initiativeId, CancellationToken cancellationToken = default)
    {
        if (initiativeId == Guid.Empty) throw new ArgumentException("Initiative ID must not be empty.", nameof(initiativeId));
        using var response = await RequestPortableDesignAsync("planning.implementationReadinessGate.snapshot", new Dictionary<string, object?> { ["initiativeId"] = initiativeId }, cancellationToken);
        return ParsePortableDesignResponse(response, envelope => PortableDesignProtocol.ParseImplementationReadinessGateResponse(envelope, initiativeId));
    }

    public async Task<ChangedUnitInventoryProjection> ReadChangedUnitInventoryAsync(Guid initiativeId, CancellationToken cancellationToken = default)
    {
        if (initiativeId == Guid.Empty) throw new ArgumentException("Initiative ID must not be empty.", nameof(initiativeId));
        using var response = await RequestPortableDesignAsync("delivery.changedUnitInventory.snapshot", new Dictionary<string, object?> { ["initiativeId"] = initiativeId }, cancellationToken);
        return ParsePortableDesignResponse(response, envelope => PortableDesignProtocol.ParseChangedUnitInventoryResponse(envelope, initiativeId));
    }

    public async Task<ProposedChangePreviewProjection> ReadProposedChangePreviewAsync(Guid initiativeId, CancellationToken cancellationToken = default)
    {
        if (initiativeId == Guid.Empty) throw new ArgumentException("Initiative ID must not be empty.", nameof(initiativeId));
        using var response = await RequestPortableDesignAsync("delivery.proposedChangePreview.snapshot", new Dictionary<string, object?> { ["initiativeId"] = initiativeId }, cancellationToken);
    ...
    public async Task<ControlledCodexImplementationProjection> ReadControlledCodexImplementationAsync(Guid initiativeId, CancellationToken cancellationToken = default)
    {
        if (initiativeId == Guid.Empty) throw new ArgumentException("Initiative ID must not be empty.", nameof(initiativeId));
        using var response = await RequestPortableDesignAsync("delivery.controlledCodexImplementation.snapshot", new Dictionary<string, object?> { ["initiativeId"] = initiativeId }, cancellationToken);
        return ParsePortableDesignResponse(response, envelope => PortableDesignProtocol.ParseControlledCodexImplementationResponse(envelope, initiativeId));
    }

    public async Task<ControlledClaudeImplementationProjection> ReadControlledClaudeImplementationAsync(Guid initiativeId, CancellationToken cancellationToken = default)
    {
        if (initiativeId == Guid.Empty) throw new ArgumentException("Initiative ID must not be empty.", nameof(initiativeId));
        using var response = await RequestPortableDesignAsync("delivery.controlledClaudeImplementation.snapshot", new Dictionary<string, object?> { ["initiativeId"] = initiativeId }, cancellationToken);
    ...
    public async Task<ModelSwitchImplementationProjection> ReadModelSwitchImplementationAsync(Guid initiativeId, CancellationToken cancellationToken = default)
    {
        if (initiativeId == Guid.Empty) throw new ArgumentException("Initiative ID must not be empty.", nameof(initiativeId));
        using var response = await RequestPortableDesignAsync("delivery.modelSwitchImplementation.snapshot", new Dictionary<string, object?> { ["initiativeId"] = initiativeId }, cancellationToken);
    ...
    public async Task<ControlledDesignToCodeGenerationProjection> ReadControlledDesignToCodeGenerationAsync(Guid initiativeId, CancellationToken cancellationToken = default)
    {
        if (initiativeId == Guid.Empty) throw new ArgumentException("Initiative ID must not be empty.", nameof(initiativeId));
        using var response = await RequestPortableDesignAsync("delivery.controlledDesignToCodeGeneration.snapshot", new Dictionary<string, object?> { ["initiativeId"] = initiativeId }, cancellationToken);
        return ParsePortableDesignResponse(response, envelope => PortableDesignProtocol.ParseControlledDesignToCodeGenerationResponse(envelope, initiativeId));
    }
    ...
    public async Task<Phase3aDashboard> ReadPhase3aDashboardAsync(
        ProductBinding product,
        InitiativeEntryRecord initiative,
        CancellationToken cancellationToken = default)
    {
        ...
        return ParsePortableDesignResponse(
            response,
            envelope => PortableDesignProtocol.ParsePhase3aDashboardResponse(envelope, product, initiative));
    }
