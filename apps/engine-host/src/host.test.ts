...
describe("engine host protocol", () => {
  ...
  it("exposes bounded business-understanding records and projection only through protocol v2", async () => {
    ...
    await expect(host.dispatch({
      jsonrpc: "2.0",
      id: "figma-mcp-capability-discovery-read-empty",
      protocolVersion: 2,
      method: "design.figmaMcpCapabilityDiscovery.read",
      params: { initiativeId },
    })).resolves.toBeNull()
    await expect(host.dispatch({
      jsonrpc: "2.0",
      id: "figma-mcp-capability-discovery-assess-empty",
      protocolVersion: 2,
      method: "design.figmaMcpCapabilityDiscovery.assess",
      params: { initiativeId },
    })).resolves.toMatchObject({
      toolCount: 0,
      ...
      staleSourceReferenceCount: 0,
      unresolvedQuestionCount: 0,
      catalogState: "not-assessed",
      permissionModelState: "not-assessed",
    ...
    const figmaMcpCapabilityDiscoveryProjection = await host.dispatch({
      jsonrpc: "2.0",
      id: "figma-mcp-capability-discovery-snapshot-empty",
      protocolVersion: 2,
      method: "design.figmaMcpCapabilityDiscovery.snapshot",
      params: { initiativeId },
    }) as { snapshotDigest: string; privacyBoundary: string; authorityBoundary: string }
    const {
      snapshotDigest: figmaMcpCapabilityDiscoverySnapshotDigest,
      ...figmaMcpCapabilityDiscoveryProjectionBody
    } = figmaMcpCapabilityDiscoveryProjection
    expect(figmaMcpCapabilityDiscoverySnapshotDigest).toBe(canonicalDigest(figmaMcpCapabilityDiscoveryProjectionBody))
    expect(figmaMcpCapabilityDiscoveryProjection).toMatchObject({
    ...
    await expect(host.dispatch({
      jsonrpc: "2.0",
      id: "finalized-figma-snapshot-import-read-empty",
      protocolVersion: 2,
      method: "design.finalizedFigmaSnapshotImport.read",
      params: { initiativeId },
    })).resolves.toBeNull()
    await expect(host.dispatch({
      jsonrpc: "2.0",
      id: "finalized-figma-snapshot-import-assess-empty",
      protocolVersion: 2,
      method: "design.finalizedFigmaSnapshotImport.assess",
      params: { initiativeId },
    })).resolves.toMatchObject({
      itemCount: 0,
      humanReviewedItemCount: 0,
      ...
      staleSourceReferenceCount: 0,
      unresolvedQuestionCount: 0,
      returnAuthorizationState: "not-assessed",
    ...
    const finalizedFigmaSnapshotImportProjection = await host.dispatch({
      jsonrpc: "2.0",
      id: "finalized-figma-snapshot-import-snapshot-empty",
      protocolVersion: 2,
      method: "design.finalizedFigmaSnapshotImport.snapshot",
      params: { initiativeId },
    }) as { snapshotDigest: string; privacyBoundary: string; authorityBoundary: string }
    const {
      snapshotDigest: finalizedFigmaSnapshotImportDigest,
      ...finalizedFigmaSnapshotImportProjectionBody
    } = finalizedFigmaSnapshotImportProjection
    expect(finalizedFigmaSnapshotImportDigest).toBe(canonicalDigest(finalizedFigmaSnapshotImportProjectionBody))
    expect(finalizedFigmaSnapshotImportProjection).toMatchObject({
